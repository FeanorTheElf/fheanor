...
///  - "encoded" plaintexts, i.e. plaintexts that have already been lifted to the ciphertext ring
///    (and prepared for fast multiplication) to avoid this cost at operation time (see
///    [`EncodedBGVPlaintextRingBase`])
///
...
pub trait AsBGVPlaintext<Params: BGVInstantiation>: RingBase + CanHomFrom<BigIntRingBase> {
    ...
    where
        I: IntoIterator<Item = (Boo<'a, Self::Element>, Boo<'b, Ciphertext<Params>>)>,
        Params: 'a + 'b,
        Self: 'a + 'b,
    {
...
// projection in the self-type position is opaque to coherence). Instead we use a
// `NumberRingQuotient` type parameter `R` tied to `Params::PlaintextRing`; the `R:
// NumberRingQuotient` bound lets coherence rule out overlap with the (non-`NumberRingQuotient`)
// `BigIntRingBase` and `EncodedBGVPlaintextRingBase` impls below.
impl<Params, R> AsBGVPlaintext<Params> for R
where
    Params: BGVInstantiation<PlaintextRing = R>,
...
impl<Params: BGVInstantiation> AsBGVPlaintext<Params> for BigIntRingBase {
        ...
        Self: 'a,
    {
        let summands = summands
            .into_iter()
            .map(|(m, ct)| (P.base_ring().coerce(&ZZbig, m.to_owned(|x| ZZbig.clone_el(x))), ct))
            .collect::<Vec<_>>();
        ...
        N: 'b,
    {
        let summands = summands
            .into_iter()
            .map(|(m, ct)| (P.base_ring().coerce(&ZZbig, ZZbig.clone_el(m)), ct))
            .collect::<Vec<_>>();
...
impl<Params: BGVInstantiation> AsBGVPlaintext<Params> for ZnBase {
    fn hom_add_to(
        ...
        ZZbig
            .get_ring()
            .hom_add_to(P, C, &int_cast(self.smallest_lift(*m), ZZbig, ZZi64), ct)
    }
    ...
    fn hom_mul_to(
        ...
        ZZbig
            .get_ring()
            .hom_mul_to(P, C, &int_cast(self.smallest_lift(*m), ZZbig, ZZi64), ct)
    }
    ...
    where
        ...
        Self: 'a,
    {
        ZZbig.get_ring().hom_inner_product(
            P,
            C,
            summands
                .into_iter()
                .map(|(x, ct)| (Boo::Owned(int_cast(self.smallest_lift(*x), ZZbig, ZZi64)), ct)),
        )
    }
    ...
    fn hom_add_to_noise<N: BGVNoiseEstimator<Params>>(
        ...
        ZZbig
            .get_ring()
            .hom_add_to_noise(estimator, P, C, &int_cast(self.smallest_lift(*m), ZZbig, ZZi64), ct)
    }
    ...
    fn hom_mul_to_noise<N: BGVNoiseEstimator<Params>>(
        ...
        ZZbig
            .get_ring()
            .hom_mul_to_noise(estimator, P, C, &int_cast(self.smallest_lift(*m), ZZbig, ZZi64), ct)
    }
        ...
        N: 'b,
    {
        let summands = summands
            .into_iter()
            .map(|(x, ct)| (int_cast(self.smallest_lift(*x), ZZbig, ZZi64), ct))
            .collect::<Vec<_>>();
        ZZbig
            .get_ring()
            .hom_inner_product_noise(estimator, P, C, summands.iter().map(|(x, ct)| (x, *ct)))
    }
}
...
impl<Params: BGVInstantiation> DelegateRing for EncodedBGVPlaintextRingBase<Params> {
    ...
    fn rev_delegate(&self, el: <Self::Base as RingBase>::Element) -> Self::Element {
        ...
        EncodedBGVPlaintextRingEl { encoded, el }
    }
...
impl<Params: BGVInstantiation> CanHomFrom<BigIntRingBase> for EncodedBGVPlaintextRingBase<Params> {
    ...
    fn mul_assign_map_in(
        ...
        self.plaintext_ring().inclusion().mul_assign_map(
            self.delegate_mut(lhs),
            self.plaintext_ring().base_ring().get_ring().map_in(from, rhs, hom),
        );
    ...
    fn map_in(
        ...
        self.rev_delegate(
            self.plaintext_ring()
                .inclusion()
                .map(self.plaintext_ring().base_ring().get_ring().map_in(from, el, hom)),
        )
    }
}
...
impl<Params: BGVInstantiation> EncodedBGVPlaintextRingBase<Params> {
    ...
    fn encoded_for(
        &self,
        C: &CiphertextRing<Params>,
        m: &EncodedBGVPlaintextRingEl<Params>,
    ) -> El<CiphertextRing<Params>> {
        if C.get_ring() == self.C.get_ring() {
            ...
            C.get_ring()
                .drop_rns_factor_element(self.C.get_ring(), &dropped, &m.encoded)
        }
    }
}
...
impl<Params: BGVInstantiation> AsBGVPlaintext<Params> for EncodedBGVPlaintextRingBase<Params> {
        ...
        Self: 'a,
    {
        ...
        let summands = summands
            .into_iter()
            .map(|(m, ct)| (Boo::Owned(self.encoded_for(C, &m)), ct))
            .collect::<Vec<_>>();
        ...
        N: 'b,
    {
        let summands = summands
            .into_iter()
            .map(|(m, ct)| (self.encoded_for(C, m), ct))
            .collect::<Vec<_>>();
        estimator.hom_inner_product_plain_encoded(
            P,
            C,
            summands.iter().map(|(m, ct)| (m, ct.borrow())),
...
fn test_as_bgv_plaintext_ops() {
    ...
    let res = P.get_ring().hom_add_to(
        &P,
        &C,
        &P.int_hom().map(5),
        P.get_ring().hom_mul_to(&P, &C, &P.int_hom().map(3), ct),
    );
    assert_el_eq!(&P, &P.int_hom().map(11), &Pow2BGV::dec(&P, &C, res, &sk));

    // (b) integer constants (BigIntRingBase): 3 * Enc(2) + 5 = 11
    let ct = Pow2BGV::enc_sym(&P, &C, &mut rng, &P.int_hom().map(2), &sk, 3.2);
    let res = ZZbig.get_ring().hom_add_to(
        &P,
        &C,
        &int_cast(5, ZZbig, ZZi64),
        ZZbig.get_ring().hom_mul_to(&P, &C, &int_cast(3, ZZbig, ZZi64), ct),
    );
    assert_el_eq!(&P, &P.int_hom().map(11), &Pow2BGV::dec(&P, &C, res, &sk));
    ...
    let res = P.get_ring().hom_inner_product(
        &P,
        &C,
        coeffs.into_iter().map(Boo::Owned).zip(cts.into_iter().map(Boo::Owned)),
    );
