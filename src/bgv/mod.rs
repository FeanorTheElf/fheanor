...
use feanor_math::rings::zn::{ZnRingStore, zn_rns};
use feanor_math::seq::*;
use rand::*;
...
use crate::bgv::bootstrap::{SparseKeyEncapsulationKey, ThinBootstrapper};
use crate::bgv::modswitch::{BGVModswitchStrategy, DefaultModswitchStrategy, ModulusAwareCiphertext};
...
use crate::ciphertext_ring::{AsSubmatrix, NumberRingRNSQuotient, RNSFactorCongruence, perform_rns_op};
...
use crate::number_ring::galois::{CyclotomicGaloisGroupOps, GaloisGroupEl};
...
use crate::{
    DefaultCiphertextAllocator, DefaultConvolution, DefaultNegacyclicNTT, FheanorAllocator, NiceZn, ZZbig, ZZi64,
};
...
pub enum ImplicitScalePolicy {
    ...
    /// it with the inverse of its implicit scale, see
    /// [`BGVInstantiation::merge_implicit_scale()`]). The result then also has implicit scale
    /// `1`. This always works, but introduces a small amount of additional noise.
    Merge,
...
impl<Params: ?Sized + BGVInstantiation> KeySwitchKey<Params> {
    ...
    pub fn gadget_vector_digits(&self) -> &RNSGadgetVectorDigitIndices { self.k0.gadget_vector_digits() }
    ...
    pub fn k0<'b>(&'b self) -> &'b RNSGadgetProductRhsOperand<Params::CiphertextRing> { &self.k0 }

    /// Returns the linear component of the key-switching key, i.e. `k1` from
    /// the tuple `k0, k1` that satisfies `k0[i] + k1[i] * s_new = g[i] * s_old`
    pub fn k1<'b>(&'b self) -> &'b RNSGadgetProductRhsOperand<Params::CiphertextRing> { &self.k1 }
}
...
pub trait BGVInstantiation {
    ...
    fn gen_sk<R: Rng + CryptoRng>(C: &CiphertextRing<Self>, mut rng: R, hwt: SecretKeyDistribution) -> SecretKey<Self> {
        match hwt {
            SecretKeyDistribution::Custom(_) => {
                panic!("cannot create a secret key if the distribution is not specified; consider creating it manually")
            }
            SecretKeyDistribution::SparseWithHwt(hwt) => {
                assert!(
                    hwt > 0,
                    "if you want to use zero as secret key, use SecretKeyDistribution::Zero instead"
                );
                assert!(
                    hwt * 3 <= C.rank() * 2,
                    "it does not make sense to take more than 2/3 of secret key entries in {{-1, 1}}"
                );
            ...
            SecretKeyDistribution::UniformTernary => C.from_canonical_basis(
                (0..C.rank()).map(|_| C.base_ring().int_hom().map((rng.next_u32() % 3) as i32 - 1)),
            ),
    ...
    #[instrument(skip_all)]
    fn gen_rk<R: Rng + CryptoRng>(
        P: &PlaintextRing<Self>,
        C: &CiphertextRing<Self>,
        rng: R,
        sk: &SecretKey<Self>,
        digits: &RNSGadgetVectorDigitIndices,
        noise_sigma: f64,
    ) -> RelinKey<Self> {
    ...
    fn gen_gk<R: Rng + CryptoRng>(
        ...
        Self::gen_switch_key(
            ...
            C,
            rng,
            &C.get_ring().apply_galois_action(sk, g),
            sk,
            digits,
            noise_sigma,
        )
    ...
    #[instrument(skip_all)]
    fn gen_gks<R: Rng + CryptoRng, I: IntoIterator<Item = GaloisGroupEl>>(
        P: &PlaintextRing<Self>,
        C: &CiphertextRing<Self>,
        mut rng: R,
        sk: &SecretKey<Self>,
        ...
        digits: &RNSGadgetVectorDigitIndices,
        noise_sigma: f64,
    ) -> Vec<(GaloisGroupEl, KeySwitchKey<Self>)> {
        gs.into_iter()
            .map(|g| {
    ...
    /// does not depend on the special modulus, and can be used w.r.t. any special modulus.
    #[instrument(skip_all)]
    fn gen_switch_key<R: Rng + CryptoRng>(
        P: &PlaintextRing<Self>,
        C: &CiphertextRing<Self>,
        mut rng: R,
        old_sk: &SecretKey<Self>,
        new_sk: &SecretKey<Self>,
        digits: &RNSGadgetVectorDigitIndices,
        noise_sigma: f64,
    ) -> KeySwitchKey<Self> {
        ...
        for digit_i in 0..digits.len() {
            ...
            let factor = C
                .base_ring()
                .get_ring()
                .from_congruence((0..C.base_ring().len()).map(|i2| {
    ...
    #[instrument(skip_all)]
    fn rlwe_sample<R: Rng + CryptoRng>(
        C: &CiphertextRing<Self>,
        mut rng: R,
        sk: &SecretKey<Self>,
        noise_sigma: f64,
    ) -> (El<CiphertextRing<Self>>, El<CiphertextRing<Self>>) {
        let a = C.random_element(|| rng.next_u64());
        let mut b = C.negate(C.mul_ref(&a, &sk));
        let e = C.from_canonical_basis((0..C.rank()).map(|_| {
            C.base_ring()
                .int_hom()
                .map((rng.sample::<f64, _>(StandardNormal) * noise_sigma).round() as i32)
        }));
    ...
    #[instrument(skip_all)]
    fn enc_sym_zero<R: Rng + CryptoRng>(
        P: &PlaintextRing<Self>,
        C: &CiphertextRing<Self>,
        rng: R,
        sk: &SecretKey<Self>,
    ...
    /// Returns a fresh encryption of the given element, i.e. a ciphertext `(c0, c1) = (-as + te +
    /// m, a)` where `s = sk` is the given secret key. `a` and `e` are sampled using the
    /// randomness of `rng`. Currently, the standard deviation of the error is fixed to `3.2`.
    #[instrument(skip_all)]
    fn enc_sym<R: Rng + CryptoRng>(
        P: &PlaintextRing<Self>,
        C: &CiphertextRing<Self>,
        rng: R,
        m: &El<PlaintextRing<Self>>,
        sk: &SecretKey<Self>,
        noise_sigma: f64,
    ) -> Ciphertext<Self> {
    ...
    fn noise_budget(
        ...
        .unwrap();
        return ZZbig
            .abs_log2_ceil(C.base_ring().modulus())
            .unwrap()
            .saturating_sub(size_of_critical_quantity + 1);
    }
    ...
    fn dec(
        ...
        ct: Ciphertext<Self>,
        sk: &SecretKey<Self>,
    ) -> El<PlaintextRing<Self>> {
        let noisy_m = C.add(ct.c0, C.mul_ref_snd(ct.c1, sk));
        let mod_t = P.base_ring().can_hom(&ZZbig).unwrap();
        return P.inclusion().mul_map(
            P.from_canonical_basis(
                C.wrt_canonical_basis(&noisy_m)
                    .iter()
                    .map(|x| mod_t.map(C.base_ring().smallest_lift(x))),
            ),
    ...
    fn encode_plain(
        ...
        C: &CiphertextRing<Self>,
        m: &El<PlaintextRing<Self>>,
    ) -> El<CiphertextRing<Self>> {
        let ZZi64_to_Zq = C.base_ring().can_hom(P.base_ring().integer_ring()).unwrap();
        return C.from_canonical_basis(
            P.wrt_canonical_basis(m)
                .iter()
                .map(|c| ZZi64_to_Zq.map(P.base_ring().smallest_lift(c))),
        );
    }
    ...
    #[instrument(skip_all)]
    fn hom_add_plain_scalar(
        P: &PlaintextRing<Self>,
        C: &CiphertextRing<Self>,
        m: &<Self::PlaintextZnRing as RingBase>::Element,
        ct: Ciphertext<Self>,
    ) -> Ciphertext<Self> {
        let m_encoded = C.inclusion().map(C.base_ring().coerce(
            &ZZbig,
            int_cast(
                P.base_ring().smallest_lift(P.base_ring().clone_el(m)),
                ZZbig,
                P.base_ring().integer_ring(),
            ),
        ));
    ...
    /// the addition, and if this is performed in advance (via
    /// [`BGVInstantiation::encode_plain()`]), addition will be faster.
    #[instrument(skip_all)]
    fn hom_add_plain_encoded(
        P: &PlaintextRing<Self>,
        C: &CiphertextRing<Self>,
        m: &El<CiphertextRing<Self>>,
        ct: Ciphertext<Self>,
    ) -> Ciphertext<Self> {
        assert!(P.base_ring().is_unit(&ct.implicit_scale));
        ...
        let implicit_scale = C.base_ring().coerce(
            &ZZbig,
            int_cast(
                P.base_ring().smallest_lift(P.base_ring().clone_el(&ct.implicit_scale)),
                ZZbig,
                P.base_ring().integer_ring(),
            ),
        );
    ...
    #[instrument(skip_all)]
    fn hom_mul_plain_scalar(
        P: &PlaintextRing<Self>,
        C: &CiphertextRing<Self>,
        m: &<Self::PlaintextZnRing as RingBase>::Element,
        mut ct: Ciphertext<Self>,
    ) -> Ciphertext<Self> {
        let factor = C.base_ring().coerce(
            &ZZbig,
            int_cast(
                P.base_ring().smallest_lift(P.base_ring().clone_el(m)),
                ZZbig,
    ...
    /// instead. However, internally, the plaintext will be lifted into the ciphertext ring during
    /// the multiplication, and if this is performed in advance (via
    /// [`BGVInstantiation::encode_plain()`]), multiplication will be faster.
    #[instrument(skip_all)]
    fn hom_mul_plain_encoded(
        P: &PlaintextRing<Self>,
        C: &CiphertextRing<Self>,
        m: &El<CiphertextRing<Self>>,
        ct: Ciphertext<Self>,
    ...
    #[instrument(skip_all)]
    fn hom_inner_product_plain_scalar<'a, I>(
        P: &PlaintextRing<Self>,
        C: &CiphertextRing<Self>,
        summands: I,
    ) -> Ciphertext<Self>
    where
        I: IntoIterator<Item = (<Self::PlaintextZnRing as RingBase>::Element, Boo<'a, Ciphertext<Self>>)>,
        Self: 'a,
    {
        ...
        for (m, ct) in summands {
            ...
            let scalar = to_Cbase.map(int_cast(
                P.base_ring().smallest_lift(scalar),
                ZZbig,
                P.base_ring().integer_ring(),
            ));
    ...
    /// are combined, see [`ImplicitScalePolicy`].
    #[instrument(skip_all)]
    fn hom_inner_product_plain<'a, 'b, I>(
        P: &PlaintextRing<Self>,
        C: &CiphertextRing<Self>,
        summands: I,
    ) -> Ciphertext<Self>
    where
        I: IntoIterator<Item = (Boo<'a, El<PlaintextRing<Self>>>, Boo<'b, Ciphertext<Self>>)>,
        Self: 'a + 'b,
    {
        ...
        let merged = summands
            ...
            .map(|(m, ct)| {
                ...
                let m_merged = P
                    .inclusion()
                    .mul_ref_fst_map(&m, P.base_ring().invert(&ct.implicit_scale).unwrap());
            ...
            .collect::<Vec<_>>();
        Self::hom_inner_product_plain_encoded(
            P,
            C,
            merged.into_iter().map(|(m, ct)| (Boo::Owned(m), Boo::Owned(ct))),
    ...
    /// [`ImplicitScalePolicy`].
    #[instrument(skip_all)]
    fn hom_inner_product_plain_encoded<'a, 'b, I>(
        P: &PlaintextRing<Self>,
        C: &CiphertextRing<Self>,
        summands: I,
        policy: ImplicitScalePolicy,
    ) -> Ciphertext<Self>
    where
        I: IntoIterator<Item = (Boo<'a, El<CiphertextRing<Self>>>, Boo<'b, Ciphertext<Self>>)>,
        Self: 'a + 'b,
    {
        ...
        let c0 = <_ as ComputeInnerProduct>::inner_product_ref_fst(
            C.get_ring(),
            ms.iter().map(|x| &**x).zip(c0s.into_iter()),
        );
        let c1 = <_ as ComputeInnerProduct>::inner_product_ref_fst(
            C.get_ring(),
            ms.iter().map(|x| &**x).zip(c1s.into_iter()),
        );
    ...
    fn hom_add(
        ...
        let (lhs, rhs, implicit_scale) = match policy {
            ImplicitScalePolicy::AssertEqual => {
                assert!(
                    P.base_ring().eq_el(&lhs.implicit_scale, &rhs.implicit_scale),
                    "ImplicitScalePolicy::AssertEqual requires all summands to have the same implicit scale"
                );
            ...
            ImplicitScalePolicy::Merge => (
                Self::merge_implicit_scale(P, C, lhs),
                Self::merge_implicit_scale(P, C, rhs),
                P.base_ring().one(),
            ),
        };
        let result = Ciphertext {
    ...
    fn hom_sub(
        ...
        Self::hom_add(
            ...
            Ciphertext {
                c0: rhs.c0,
                c1: rhs.c1,
                implicit_scale: P.base_ring().negate(rhs.implicit_scale),
            },
            policy,
        )
    }
    ...
    fn key_switch(
        P: &PlaintextRing<Self>,
        C: &CiphertextRing<Self>,
        C_special: &CiphertextRing<Self>,
        ct: Ciphertext<Self>,
        switch_key: &KeySwitchKey<Self>,
    ) -> Ciphertext<Self> {
    ...
    fn hom_mul(
        ...
        let mut result = Self::key_switch(
            P,
            C,
            C_special,
            Ciphertext {
    ...
    /// Computes an encryption of `sigma(x)`, where `x` is the message encrypted by the given
    /// ciphertext and `sigma` is the given Galois automorphism.
    ///  
    ...
    where
        V: VectorFn<&'b KeySwitchKey<Self>>,
        KeySwitchKey<Self>: 'b,
    {
        assert!(P.base_ring().is_unit(&ct.implicit_scale));
        ...
        let special_modulus_rns_factor_indices =
            RNSFactorIndexList::missing_from_subset(C.base_ring(), C_special.base_ring()).unwrap();

        let digits = gks.at(0).k0.gadget_vector_digits();
        let has_same_digits = |gk: &RNSGadgetProductRhsOperand<_>| {
            gk.gadget_vector_digits().len() == digits.len()
                && gk.gadget_vector_digits().iter().zip(digits.iter()).all(|(l, r)| l == r)
        };
        assert!(
            gks.iter().all(|gk| has_same_digits(&gk.k0) && has_same_digits(&gk.k1)),
            "hom_galois_many() requires all Galois keys to use the same parameters"
        );

        let c1_op = if special_modulus_rns_factor_indices.len() == 0 {
            RNSGadgetProductLhsOperand::from_element_with(C.get_ring(), &ct.c1, &digits)
        } else {
            let special_modulus = ZZbig.prod(
                special_modulus_rns_factor_indices
                    .iter()
                    .map(|i| int_cast(*C_special.base_ring().at(*i).modulus(), ZZbig, ZZi64)),
            );
            let special_modulus = C_special.base_ring().coerce(&ZZbig, special_modulus);
            let mut ct1_modswitched =
                C_special
                    .get_ring()
                    .add_rns_factor_element(C.get_ring(), &special_modulus_rns_factor_indices, &ct.c1);
            C_special
                .inclusion()
                .mul_assign_map(&mut ct1_modswitched, special_modulus);
            RNSGadgetProductLhsOperand::from_element_with(C_special.get_ring(), &ct1_modswitched, &digits)
        };
    ...
    fn mod_switch_compute_implicit_scale_factor(
        P: &PlaintextRing<Self>,
        q_new: &El<BigIntRing>,
        q_old: &El<BigIntRing>,
    ) -> <Self::PlaintextZnRing as RingBase>::Element {
        let ZZbig_to_Zt = P.base_ring().can_hom(&ZZbig).unwrap();
        let result = P
            .base_ring()
            .checked_div(&ZZbig_to_Zt.map_ref(q_new), &ZZbig_to_Zt.map_ref(q_old))
            .unwrap();
    ...
    fn rescale_ring_element_fn<'a>(
        ...
        let a = Cold.base_ring().coerce(
            &ZZbig,
            ZZbig.prod(
                added_rns_factors
                    .iter()
                    .map(|i| int_cast(*Cnew.base_ring().at(*i).modulus(), ZZbig, ZZi64)),
            ),
        );
                ...
                .collect_rns_factors((0..Cnew.base_ring().len()).map(|idx| {
                    if added_rns_factors.contains(idx) {
                        ...
                        let old_idx = Cold
                            ...
                            .as_iter()
                            .enumerate()
                            .filter(|(_, old_ring)| old_ring.get_ring() == Cnew.base_ring().at(idx).get_ring())
                            .next()
                            .unwrap()
                            .0;
        ...
        let b_inv = Cnew
            .base_ring()
            .invert(
                &Cnew.base_ring().coerce(
                    &ZZbig,
                    ZZbig.prod(
                        dropped_rns_factors
                            .iter()
                            .map(|i| int_cast(*Cold.base_ring().at(*i).modulus(), ZZbig, ZZi64)),
                    ),
                ),
            )
            .unwrap();
    ...
    fn mod_switch_ct(
        P: &PlaintextRing<Self>,
        Cnew: &CiphertextRing<Self>,
        Cold: &CiphertextRing<Self>,
        ct: Ciphertext<Self>,
    ) -> Ciphertext<Self> {
        assert!(P.base_ring().is_unit(&ct.implicit_scale));
        ...
        let result = Ciphertext {
            c0: rescale(ct.c0),
            c1: rescale(ct.c1),
            implicit_scale: P.base_ring().mul(
                ct.implicit_scale,
                Self::mod_switch_compute_implicit_scale_factor(
                    P,
                    Cnew.base_ring().modulus(),
                    Cold.base_ring().modulus(),
                ),
            ),
        };
        assert!(P.base_ring().is_unit(&result.implicit_scale));
    ...
    fn fake_mod_switch_down_ct(
        ...
        if drop_moduli.len() == 0 {
            ...
            Ciphertext {
                ...
                c0: Cnew
                    .get_ring()
                    .drop_rns_factor_element(Cold.get_ring(), &drop_moduli, &ct.c0),
                c1: Cnew
                    .get_ring()
                    .drop_rns_factor_element(Cold.get_ring(), &drop_moduli, &ct.c1),
            }
        }
    }
    ...
    /// much cheaper than creating a new ciphertext ring (e.g. using
    /// [`BGVInstantiation::create_ciphertext_ring()`]).
    #[instrument(skip_all)]
    ...
    fn mod_switch_sk(
        ...
        if let Ok(dropped_factors) = RNSFactorIndexList::missing_from_subset(Cnew.base_ring(), Cold.base_ring()) {
            return Cnew
                .get_ring()
                .drop_rns_factor_element(Cold.get_ring(), &dropped_factors, sk);
        } else {
            ...
            assert!(
                Cold.base_ring()
                    .as_iter()
                    .zip(mod_switch.input_rings())
                    .all(|(l, r)| l.get_ring() == r.get_ring())
            );
            assert!(
                Cnew.base_ring()
                    .as_iter()
                    .zip(mod_switch.output_rings())
                    .all(|(l, r)| l.get_ring() == r.get_ring())
            );
    ...
    /// Modulus-switches a relinearization key in a way compatible with modulus-switching
    /// ciphertexts.
    ///
    /// This is equivalent to creating a new relinearization key (using
    /// [`BGVInstantiation::gen_rk()`]) over `Cnew` for the secret key `mod_switch_sk(Cnew,
    /// Cold, drop_moduli, sk)`, but does not require access to `sk`. Currently, this function
    /// only supports decreasing the ciphertext modulus, by dropping RNS factors.
    #[instrument(skip_all)]
    fn mod_switch_down_rk(
        Cnew: &CiphertextRing<Self>,
        Cold: &CiphertextRing<Self>,
        rk: &RelinKey<Self>,
    ) -> RelinKey<Self> {
    ...
    /// over `Cnew` for the secret key `mod_switch_down_sk(Cnew, Cold, drop_moduli, sk)`, but does
    /// not require access to `sk`. Currently, this function only supports decreasing the
    /// ciphertext modulus, by dropping RNS factors.
    #[instrument(skip_all)]
    fn mod_switch_down_gk(
        Cnew: &CiphertextRing<Self>,
        Cold: &CiphertextRing<Self>,
        gk: &KeySwitchKey<Self>,
    ) -> KeySwitchKey<Self> {
    ...
    /// Converts an encrypted value `m` w.r.t. a plaintext modulus `t` to an encryption of `t' m /
    /// t` w.r.t. a plaintext modulus `t'`. This requires that `t' m / t` is an integral ring
    /// element (i.e. `t` divides `t' m`), otherwise this function will cause immediate noise
    /// overflow.
    #[instrument(skip_all)]
    fn change_plaintext_modulus(
        Pnew: &PlaintextRing<Self>,
        Pold: &PlaintextRing<Self>,
        C: &CiphertextRing<Self>,
        ct: Ciphertext<Self>,
    ) -> Ciphertext<Self> {
        ...
        let x = C
            ...
            .unwrap();
        let new_implicit_scale = Pnew.base_ring().coerce(
            Pnew.base_ring().integer_ring(),
            Pold.base_ring().smallest_positive_lift(ct.implicit_scale),
        );
        let result = Ciphertext {
    ...
    /// Modulus-switches from `R/qR` to `R/t'R`, where the latter one is given as a plaintext ring
    /// `target`. In particular, this is necessary during bootstrapping.
    ///
    ...
    fn mod_switch_to_plaintext(
        P: &PlaintextRing<Self>,
        target: &PlaintextRing<Self>,
        C: &CiphertextRing<Self>,
        ct: Ciphertext<Self>,
    ) -> (El<PlaintextRing<Self>>, El<PlaintextRing<Self>>) {
        let ZZ = P.base_ring().integer_ring();
        assert!(ZZ.get_ring() == target.base_ring().integer_ring().get_ring());
        assert!(
            ZZ.is_one(&signed_gcd(
                ZZ.clone_el(P.base_ring().modulus()),
                ZZ.clone_el(target.base_ring().modulus()),
                ZZ
            )),
            "can only mod-switch to ciphertext moduli that are coprime to t"
        );
        assert!(P.base_ring().is_unit(&ct.implicit_scale));

        // this is not very performance-critical, so implement it using big integers
        let t_target = int_cast(
            target.base_ring().integer_ring().clone_el(target.base_ring().modulus()),
            ZZbig,
            target.base_ring().integer_ring(),
        );
        let to_Pbase = P.base_ring().can_hom(&ZZbig).unwrap();
        let to_target_base = target.base_ring().can_hom(&ZZbig).unwrap();
        let factor = P
            .base_ring()
            .checked_div(&to_Pbase.map_ref(&t_target), &to_Pbase.map_ref(C.base_ring().modulus()))
            .unwrap();
        let rescale = |x| {
            target.from_canonical_basis(C.wrt_canonical_basis(x).iter().map(|a| {
                ...
                to_target_base.map(
                    ZZbig.add_ref_fst(
                        &scaled_a,
                        int_cast(
                            P.base_ring().smallest_lift(
                                P.base_ring()
                                    .sub(to_Pbase.mul_ref_fst_map(&factor, lift_a), to_Pbase.map_ref(&scaled_a)),
                            ),
                            ZZbig,
                            ZZ,
                        ),
        ...
        let factor = P.base_ring().smallest_lift(
            P.base_ring()
                .invert(&P.base_ring().mul(
                    ct.implicit_scale,
                    Self::mod_switch_compute_implicit_scale_factor(
                        P,
                        &int_cast(ZZ.clone_el(target.base_ring().modulus()), ZZbig, ZZ),
                        C.base_ring().modulus(),
                    ),
                ))
                .unwrap(),
...
#[derive(Debug)]
pub struct Pow2BGV<A: FheanorAllocator = DefaultCiphertextAllocator, C: FheanorNegacyclicNTT<Zn> = DefaultNegacyclicNTT>
{
...
impl Pow2BGV {
    pub fn new(m: usize) -> Self { Self::new_with_ntt(m, DefaultCiphertextAllocator::default()) }
}

impl<A: FheanorAllocator, C: FheanorNegacyclicNTT<Zn>> Pow2BGV<A, C> {
    #[instrument(skip_all)]
...
impl<A: FheanorAllocator, C: FheanorNegacyclicNTT<Zn>> Clone for Pow2BGV<A, C> {
    fn clone(&self) -> Self {
        Self {
...
impl<A: FheanorAllocator, C: FheanorNegacyclicNTT<Zn>> Display for Pow2BGV<A, C> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result { write!(f, "BGV({:?})", self.number_ring) }
}

impl<A: FheanorAllocator, C: FheanorNegacyclicNTT<Zn>> BGVInstantiation for Pow2BGV<A, C> {
    ...
    fn number_ring(&self) -> &Pow2CyclotomicNumberRing<C> { &self.number_ring }

    #[instrument(skip_all)]
    fn create_plaintext_ring(&self, modulus: El<BigIntRing>) -> PlaintextRing<Self> {
        if ZZbig.is_gt(&modulus, &ZZbig.power_of_two(SAMPLE_PRIMES_SIZE - 2)) {
            unimplemented!(
                "Plaintext modulus greater than 2^{} are not yet supported for BGV",
                SAMPLE_PRIMES_SIZE - 2
            )
        }
        NumberRingQuotientByIntBase::create(
            self.number_ring().clone(),
            Zn::new(int_cast(modulus, ZZi64, ZZbig) as u64),
    ...
    fn enc_sym_zero<R: Rng + CryptoRng>(
        ...
        assert!(ZZbig.is_one(&signed_gcd(
    ...
    fn create_rns_base(&self, log2_q: Range<usize>) -> zn_rns::Zn<Zn, BigIntRing> {
        ...
        let mut rns_base = sample_primes(log2_q.start, log2_q.end, SAMPLE_PRIMES_SIZE, |bound| {
            largest_prime_leq_congruent_to_one(bound, required_root_of_unity)
        })
        .unwrap();
    ...
    fn encode_plain(
        ...
        let ZZi64_to_Zq = C.base_ring().can_hom(P.base_ring().integer_ring()).unwrap();
        let result = C.from_canonical_basis(
            P.wrt_canonical_basis(m)
                .iter()
                .map(|c| ZZi64_to_Zq.map(P.base_ring().smallest_lift(c))),
        );
    ...
    fn cost_model(&self) -> CircuitEvaluatorCosts { DOUBLE_RNS_BGV_COST }
}

#[derive(Clone, Debug)]
pub struct CompositeBGV<A: FheanorAllocator = DefaultCiphertextAllocator> {
    number_ring: TensorProductNumberRing,
    ciphertext_allocator: A,
}

impl<A: FheanorAllocator> Display for CompositeBGV<A> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result { write!(f, "BGV({:?})", self.number_ring) }
}

impl CompositeBGV {
    pub fn new(m1: usize, m2: usize) -> Self { Self::new_with_alloc(m1, m2, DefaultCiphertextAllocator::default()) }
}

impl<A: FheanorAllocator> CompositeBGV<A> {
    #[instrument(skip_all)]
...
impl<A: FheanorAllocator> BGVInstantiation for CompositeBGV<A> {
    ...
    fn number_ring(&self) -> &TensorProductNumberRing { &self.number_ring }
    ...
    fn encode_plain(
        ...
        return C
            .get_ring()
            .to_doublerns(&result)
            .map(|x| C.get_ring().from_doublerns(C.get_ring().unmanaged_ring().clone_el(x)))
            .unwrap_or(C.zero());
    }
...
/// multiplications.
#[derive(Clone, Debug)]
pub struct CompositeSingleRNSBGV<
    A: FheanorAllocator = DefaultCiphertextAllocator,
    C: FheanorConvolution<Zn> = DefaultConvolution,
> {
    number_ring: TensorProductNumberRing,
    ciphertext_allocator: A,
...
impl<A: FheanorAllocator, C: FheanorConvolution<Zn>> CompositeSingleRNSBGV<A, C> {
    ...
    <C as ConvolutionAlgorithm<ZnBase>>::PreparedConvolutionOperand: Send + Sync,
{
    ...
    fn create_ciphertext_ring_with_rns_base(&self, rns_base: zn_rns::Zn<Zn, BigIntRing>) -> CiphertextRing<Self> {
        ...
        let convolutions = rns_base
            .as_iter()
            .map(|Zp| C::new(*Zp, max_log2_n))
            .map(Arc::new)
            .collect::<Vec<_>>();
    ...
    fn cost_model(&self) -> CircuitEvaluatorCosts { SINGLE_RNS_BGV_COST }
}
...
fn test_pow2_bgv_gen_sk() {
    ...
    assert!(
        C.wrt_canonical_basis(&sk)
            .iter()
            .filter(|c| C.base_ring().is_one(&c) || C.base_ring().is_neg_one(&c))
            .count()
            == 1
    );
...
fn test_pow2_bgv_mul() {
    ...
    let C = params.create_ciphertext_ring(500..520);
    let sk = Pow2BGV::gen_sk(&C, &mut rng, SecretKeyDistribution::UniformTernary);
    let rk = Pow2BGV::gen_rk(
        &P,
        &C,
        &mut rng,
        &sk,
        &RNSGadgetVectorDigitIndices::select_digits(3, C.base_ring().len()),
        3.2,
    );
...
fn test_pow2_bgv_hybrid_key_switch() {
    ...
    let switch_key = Pow2BGV::gen_switch_key(
        &P,
        &C_special,
        &mut rng,
        &sk,
        &sk_new,
        &RNSGadgetVectorDigitIndices::select_digits(6, C_special.base_ring().len()),
        3.2,
    );

    let input = P.int_hom().map(2);
    let ctxt = Pow2BGV::enc_sym(
        &P,
        &C,
        &mut rng,
        &input,
        &Pow2BGV::mod_switch_sk(&C, &C_special, &sk),
        3.2,
    );
    let result_ctxt = Pow2BGV::key_switch(&P, &C, &C_special, Pow2BGV::clone_ct(&P, &C, &ctxt), &switch_key);
    let result = Pow2BGV::dec(
        &P,
        &C,
        Pow2BGV::clone_ct(&P, &C, &result_ctxt),
        &Pow2BGV::mod_switch_sk(&C, &C_special, &sk_new),
    );
    assert_el_eq!(&P, P.int_hom().map(2), result);

    let critical_quantity_size = |ct: &Ciphertext<Pow2BGV>, sk: &SecretKey<Pow2BGV>| {
        C.wrt_canonical_basis(&C.add_ref_fst(
            &ct.c0,
            C.mul_ref_fst(&ct.c1, Pow2BGV::mod_switch_sk(&C, &C_special, sk)),
        ))
        .iter()
        .map(|c| ZZbig.abs_log2_ceil(&C.base_ring().smallest_lift(c)).unwrap_or(0))
        .max()
        .unwrap()
    };
    assert!(
        critical_quantity_size(&result_ctxt, &sk_new) <= critical_quantity_size(&ctxt, &sk) + 4,
        "critical quantity size increased too much; {} increased to {}",
        critical_quantity_size(&ctxt, &sk),
        critical_quantity_size(&result_ctxt, &sk_new)
    );
    ...
    assert_el_eq!(&P, P.int_hom().map(4), result);
    assert!(
        critical_quantity_size(&result_ctxt, &sk) <= original_critical_quantity_size + 20,
        "critical quantity size increased too much; {} increased to {}",
        original_critical_quantity_size,
        critical_quantity_size(&result_ctxt, &sk)
    );
    ...
    let rk = Pow2BGV::gen_rk(
        ...
        &mut rng,
        &sk,
        &RNSGadgetVectorDigitIndices::select_digits(3, C_special.base_ring().len()),
        3.2,
    );
    let input = P.int_hom().map(2);
...
fn test_pow2_bgv_modulus_switch() {
    ...
    for i in [0, 1, 8] {
        ...
        let result = Pow2BGV::dec(
            &P,
            &C1,
            Pow2BGV::clone_ct(&P, &C1, &ctxt1),
            &Pow2BGV::mod_switch_sk(&C1, &C0, &sk),
        );
        assert_el_eq!(&P, P.int_hom().map(2), result);
...
fn test_pow2_bgv_modulus_switch_rk() {
    ...
    let rk = Pow2BGV::gen_rk(
        ...
        &mut rng,
        &sk,
        &RNSGadgetVectorDigitIndices::select_digits(3, C0.base_ring().len()),
        3.2,
    );
...
fn test_pow2_mod_switch_repeated() {
    ...
    assert_el_eq!(
        &P,
        &P.int_hom().map(2),
        Pow2BGV::dec(&P, &C1, Pow2BGV::clone_ct(&P, &C1, &ctxt1), &sk1)
    );
...
fn measure_time_pow2_bgv_basic_ops() {
    ...
    let C = log_time("CreateCtxtRing", |[]| params.create_ciphertext_ring(790..800));
    ...
    let ct = log_time("EncSym", |[]| Pow2BGV::enc_sym(&P, &C, &mut rng, &m, &sk, 3.2));
    ...
    let res = log_time("HomMul", |[]| Pow2BGV::hom_mul(&P, &C, &C, ct, ct2, &rk));
    assert_el_eq!(
        &P,
        &P.int_hom().map(4),
        &Pow2BGV::dec(&P, &C, Pow2BGV::clone_ct(&P, &C, &res), &sk)
    );
    ...
    let res_new = log_time("ModSwitch", |[]| Pow2BGV::mod_switch_ct(&P, &C_new, &C, res));
...
fn measure_time_double_rns_composite_bgv_basic_ops() {
    ...
    let C = log_time("CreateCtxtRing", |[]| params.create_ciphertext_ring(1090..1100));
    ...
    let ct = log_time("EncSym", |[]| CompositeBGV::enc_sym(&P, &C, &mut rng, &m, &sk, 3.2));
    assert_el_eq!(
        &P,
        &P.int_hom().map(3),
        &CompositeBGV::dec(&P, &C, CompositeBGV::clone_ct(&P, &C, &ct), &sk)
    );
    ...
    let res = log_time("HomMul", |[]| CompositeBGV::hom_mul(&P, &C, &C, ct, ct2, &rk));
    assert_el_eq!(
        &P,
        &P.int_hom().map(1),
        &CompositeBGV::dec(&P, &C, CompositeBGV::clone_ct(&P, &C, &res), &sk)
    );

    let to_drop = RNSFactorIndexList::from(vec![0], C.base_ring().len());
    let C_new = CompositeBGV::mod_switch_down_C(&C, &to_drop);
    let sk_new = CompositeBGV::mod_switch_sk(&C_new, &C, &sk);
    let res_new = log_time("ModSwitch", |[]| CompositeBGV::mod_switch_ct(&P, &C_new, &C, res));
    assert_el_eq!(
        &P,
        &P.int_hom().map(1),
        &CompositeBGV::dec(&P, &C_new, res_new, &sk_new)
    );
}
...
fn measure_time_single_rns_composite_bgv_basic_ops() {
    ...
    assert_el_eq!(
        &P,
        &P.int_hom().map(3),
        &CompositeSingleRNSBGV::dec(&P, &C, CompositeSingleRNSBGV::clone_ct(&P, &C, &ct), &sk)
    );
    ...
    let res = log_time("HomMul", |[]| CompositeSingleRNSBGV::hom_mul(&P, &C, &C, ct, ct2, &rk));
    assert_el_eq!(
        &P,
        &P.int_hom().map(1),
        &CompositeSingleRNSBGV::dec(&P, &C, CompositeSingleRNSBGV::clone_ct(&P, &C, &res), &sk)
    );
    ...
    assert_el_eq!(
        &P,
        &P.int_hom().map(1),
        &CompositeSingleRNSBGV::dec(&P, &C_new, res_new, &sk_new)
    );
}

#[test]
fn test_transaction_experiment_bgv() {
    feanor_tracing::DelayedLogger::init_test();
    ...
    let inst = Pow2BGV::new(1 << 16);
    let P = inst.create_plaintext_ring(int_cast(65537, ZZbig, ZZi64));
    let C = inst.create_ciphertext_ring(835..855);
    ...
    type Strat = DefaultModswitchStrategy<Pow2BGV, NaiveBGVNoiseEstimator, false>;
    let strat: Strat = DefaultModswitchStrategy::new(NaiveBGVNoiseEstimator);
    ...
    let sk = Pow2BGV::gen_sk(&C, rng(), SparseWithHwt(128));
    let rk = Pow2BGV::gen_rk(&P, &C, rng(), &sk, &rk_digits, 3.2);
    let gk = Pow2BGV::gen_gk(&P, &C, rng(), &sk, &H.hypercube().map(&[1, 0]), &gk_digits, 3.2);
    let x = ModulusAwareCiphertext {
        data: Pow2BGV::enc_sym(&P, &C, rng(), &m_x, &sk, 3.2),
        dropped_rns_factor_indices: RNSFactorIndexList::empty(),
        info: strat.info_for_fresh_encryption(&P, &C, SparseWithHwt(128)),
    };
    let y = ModulusAwareCiphertext {
        data: Pow2BGV::enc_sym(&P, &C, rng(), &m_y, &sk, 3.2),
        dropped_rns_factor_indices: RNSFactorIndexList::empty(),
        info: strat.info_for_fresh_encryption(&P, &C, SparseWithHwt(128)),
    };
    ...
    let C_res = Pow2BGV::mod_switch_down_C(&C, &res.dropped_rns_factor_indices);
    let sk_res = Pow2BGV::mod_switch_sk(&C_res, &C, &sk);
    let values = H
        .get_slot_values(&Pow2BGV::dec(
            &P,
            &C_res,
            Pow2BGV::clone_ct(&P, &C_res, &res.data),
            &sk_res,
        ))
        .map(|x| {
            H.slot_ring()
    ...
    println!(
        "addition used {} of {} RNS factors",
        res.dropped_rns_factor_indices.len(),
        C.base_ring().len()
    );

    let sparse_sk_hwt = 32;
    let bootstrapper = ThinBootstrapper::build_pow2(
        &inst,
        &P,
        &C,
        1,
        // heuristic bound on the modulus-switching rounding error `e_0 + sum_i e_i s_i`, whose
        // standard deviation is about `sqrt(hwt / 12)`; note that the degree of the digit
        // extraction polynomial (and thus its depth) grows linearly in this bound
        Some(16),
        4,
        &gk_digits,
        strat,
    ...
    let P_boot = bootstrapper.intermediate_plaintext_ring();
    let sparse_key_encaps = SparseKeyEncapsulationKey::new(P_boot, &C, &sk, 3, sparse_sk_hwt, rng(), 3.2);
    let rk_boot = Pow2BGV::gen_rk(P_boot, &C, rng(), &sk, &rk_digits, 3.2);
    let gks = bootstrapper
        .required_galois_keys(&P)
        .into_iter()
        .map(|g| {
            let gk = Pow2BGV::gen_gk(P_boot, &C, rng(), &sk, &g, &gk_digits, 3.2);
            (g, gk)
        })
        .collect::<Vec<_>>();
    let start = Instant::now();
    let res = bootstrapper.bootstrap_thin(
        &res.dropped_rns_factor_indices,
        res.data,
        &rk_boot,
        &gks,
        SparseWithHwt(128),
        Some(&sparse_key_encaps),
        None,
    );
    let end = Instant::now();
    ...
    println!(
        "noise budget: {}/{}",
        Pow2BGV::noise_budget(&P, &C_res, &res.data, &sk_res),
        ZZbig.abs_log2_ceil(C_res.base_ring().modulus()).unwrap()
    );
    let res = Pow2BGV::dec(&P, &C_res, Pow2BGV::clone_ct(&P, &C_res, &res.data), &sk_res);
    let values = H
        .get_slot_values(&res)
        .map(|x| {
            H.slot_ring()
