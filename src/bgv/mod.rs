...
/// 
/// A variant of hybrid key-switching is used where the key-switching
...
pub trait BGVInstantiation {
    ...
    /// 
    /// On a technical level, hybrid key-switching is equivalent to modulus-switching the ciphertext 
    /// up to `C_special`, and modulus-switch it down after the key-switch. This decreases the noise
    ...
    /// 
    /// Since automatic modulus management is not supported (at least not implicitly),
    ...
    /// in `drop_moduli`.
    /// 
    /// Note that for the implementations in this crate at least, the underlying rings will share
