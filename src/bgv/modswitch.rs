    ...
    (0..=*a.iter().max().unwrap())
        .filter_map(|max_result| {
            ...
            while current_sum_c < k {
                let entry_to_decrease = (0..len)
                    .filter(|i| b[*i] - c[*i] != 0)
                    .min_by_key(|i| b[*i] - c[*i])
                    .unwrap();
        ...
        .filter_map(|(max_result, c)| {
            ...
            while current_sum_d < max_result {
                let i = (0..len)
                    .max_by_key(|i| min(a[*i] + c[*i] - b[*i] - d[*i], max_result + c[*i] - b[*i] - d[*i]))
                    .unwrap();
...
pub struct ModulusAwareCiphertext<Params: BGVInstantiation, Strategy: ?Sized + BGVModswitchStrategy<Params>> {
    ...
    /// that are not used for this ciphertext; in other words, the ciphertext modulus of this
    /// ciphertext is the product of all RNS factors of the master RNS base that are not
...
pub trait BGVModswitchStrategy<Params: BGVInstantiation> {
    ...
    ///  - `C_master` is the ciphertext ring with the largest relevant RNS base, i.e. its RNS base
    ///    should contain all RNS factors that are referenced by any ciphertext, and may have
    ///    additional unused RNS factors
    ///  - `inputs` contains all inputs to the circuit, i.e. must be of the same length as the
    ///    circuit has input wires. Each entry should be of the form `(drop_rns_factors, info,
    ///    ctxt)` where `ctxt` is the ciphertext w.r.t. the RNS base that contains all RNS factors
    ///    of `C_master` except those mentioned in `drop_rns_fctors`, and `info` should store the
    ///    additional information associated to the ciphertext that is required to determine
    ///    modulus-switching times.
    ...
    fn fresh_encryption(
        ...
        ModulusAwareCiphertext {
            ...
            info: self.info_for_fresh_encryption(P, C_master, sk),
        }
    }
...
/// When we do modulus-switching (e.g. during BGV), we remove RNS factors from the ciphertext
/// modulus. For the ciphertexts itself, it is (almost) irrelevant which of these RNS factors are
/// removed, but it makes a huge difference when mod-switching key-switching keys (e.g.
/// relinearization keys). This is because the used gadget vector relies is based on a decomposition
/// of RNS factors into groups, and removing a single RNS factor from every group will give a very
/// different behavior from removing a single, whole group and leaving the other groups unchanged.
///
/// This function will choose the RNS factors to drop with the goal of minimizing noise growth. In
/// particular, as long as the RNS factor groups (the digits) are larger than the special modulus,
/// this function will remove RNS factors from each group in a balanced manner.
///
/// This is probably the desired behavior in most cases, but other behaviors might as well be
/// reasonable in certain scenarios.
///
/// # Example
/// ```rust
/// # use feanor_math::seq::*;
...
/// ```
pub fn drop_rns_factors_balanced(
    key_digits: &RNSGadgetVectorDigitIndices,
    drop_prime_count: usize,
    ...
    for _ in 0..drop_prime_count {
        let largest_digit_idx = (0..key_digits.len())
            .max_by_key(|i| effective_len(key_digits.at(*i)) - drop_from_digit[*i])
            .unwrap();
        drop_from_digit[largest_digit_idx] += 1;
    }

    let result = RNSFactorIndexList::from(
        (0..key_digits.len()).flat_map(|i| key_digits.at(i).start..(key_digits.at(i).start + drop_from_digit[i])),
        key_digits.rns_base_len(),
    );
    return result;
}
...
///  - use the provided [`BGVNoiseEstimator`] to determine when and by how much we should reduce the
///    ciphertext modulus
///
/// These points lead to a relatively simple and generally well-performing modulus switching
/// strategy. However, there may be situations where deviating from 1. could lead to a lower number
/// of mod-switches (and thus better performance), and deviating from 2. could be used for a
/// finer-tuned mod-switching, and thus less noise growth.
...
pub fn compute_optimal_special_modulus<C: NumberRingRNSQuotient>(
    ...
    let a = key_switch_key_digits
        .iter()
        .map(|digit| digit.end - digit.start)
        .collect::<Vec<_>>();
    let b = key_switch_key_digits
        .iter()
        .map(|digit| digit.end - digit.start - dropped_factors_input.num_within(&digit))
        .collect::<Vec<_>>();
    if let Some((c, d)) = level_digits(&a, &b, drop_additional_count) {
        let B_additional = key_switch_key_digits
            .iter()
            .enumerate()
            .flat_map(|(digit_idx, digit)| digit.filter(|i| !dropped_factors_input.contains(*i)).take(c[digit_idx]));
        let B_final = RNSFactorIndexList::from(
            dropped_factors_input
                .iter()
                .copied()
                .chain(B_additional)
                .collect::<Vec<_>>(),
        ...
        let B_special = RNSFactorIndexList::from(
            key_switch_key_digits
                .iter()
                .enumerate()
                .flat_map(|(digit_idx, digit)| digit.filter(|i| B_final.contains(*i)).take(d[digit_idx]))
                .collect::<Vec<_>>(),
            C_master.base_ring().len(),
        );
        ...
        let additional_drop = drop_rns_factors_balanced(
            &key_switch_key_digits.remove_indices(dropped_factors_input),
            drop_additional_count,
        );
...
impl<Params: BGVInstantiation, N: BGVNoiseEstimator<Params>, const LOG: bool> DefaultModswitchStrategy<Params, N, LOG> {
    ...
    fn mod_switch_down(
        ...
        if LOG && drop_x.len() > 0 {
            println!(
                "{}: Dropping RNS factors {} of operand, estimated noise budget {}/{} -> {}/{}",
                context,
                drop_x,
                -self
                    .noise_estimator
                    .estimate_log2_relative_noise_level(P, &Cx, &x.info)
                    .round(),
                ZZbig.abs_log2_ceil(Cx.base_ring().modulus()).unwrap(),
                -self
                    .noise_estimator
                    .estimate_log2_relative_noise_level(P, C_target, &result.info)
                    .round(),
            ...
            if let Some(sk) = debug_sk {
                ...
                println!(
                    "  actual noise budget: {} -> {}",
                    x_noise_budget.unwrap(),
                    Params::noise_budget(P, C_target, &result.data, &sk_target)
                );
            }
        }
        return result;
    ...
    fn mod_switch_down_ref<'a>(
        ...
        if LOG && drop_x.len() > 0 {
            ...
            if let Some(sk) = debug_sk {
                ...
                println!(
                    "  actual noise budget: {} -> {}",
                    Params::noise_budget(P, &Cx, &x.data, &sk_x),
                    Params::noise_budget(P, C_target, &result.data, &sk_target)
                );
            }
        }
    ...
    fn bring_to_modulus<'a>(
        ...
        let t_log2 = P
            .base_ring()
            .integer_ring()
            .abs_log2_ceil(P.base_ring().modulus())
            .unwrap() as f64;
        ...
        let dropped_after_fake = |k: usize| {
            RNSFactorIndexList::from(
                x.dropped_rns_factor_indices
                    .iter()
                    .copied()
                    .chain(sorted_factors[..k].iter().copied())
                    .collect::<Vec<_>>(),
                C_master.base_ring().len(),
            )
        };
        ...
        for k in 1..=drop_x.len() {
            ...
            let reduction_noise = self
                .noise_estimator
                .estimate_log2_relative_noise_level(P, &C_inter, &fake_info);
        ...
        return Boo::Owned(self.mod_switch_down(
            P,
            C_target,
            C_master,
            dropped_factors_target,
            fake_ct,
            context,
            debug_sk,
        ));
    ...
    fn compute_optimal_mul_modswitch(
        ...
        let compute_result_noise = |num_to_drop: usize| {
            let (total_drop, special_modulus) =
                compute_optimal_special_modulus(C_master.get_ring(), &base_drop, num_to_drop, rk_digits);
        ...
        return (0..(C_master.base_ring().len() - base_drop.len()))
            .map(compute_result_noise)
            .min_by(|(_, l), (_, r)| f64::total_cmp(l, r))
            .unwrap()
            .0;
    }
    ...
    ///  - We decide not to perform intermediate modulus-switches, but only modulus-switch at the
    ///    very beginning. Note however that it might be possible to group summands depending on
    ///    their RNS base, and reduce the number of modulus-switches
    ///  - We have to decide on the `implicit_scale` of the result, its choice may affect noise
    ///    growth
    ///  - using inner product functionality of the underlying ring can give us better performance
    ///    than many isolated additions/multiplications
    #[instrument(skip_all)]
    fn inner_prod<'a, R>(
        &self,
        P: &PlaintextRing<Params>,
        ...
        R::Type: AsBGVPlaintext<Params>,
    {
        ...
        if int_products.len() == 0 && main_products.len() == 0 {
            ...
            return ModulusAwareCiphertext {
                data: Params::transparent_zero(P, C_master),
                dropped_rns_factor_indices: RNSFactorIndexList::empty(),
        ...
        let max_input_noise = int_products
            .iter()
            .map(|(_, ct)| ct)
            .chain(main_products.iter().map(|(_, ct)| ct))
            .map(|ct| {
                self.noise_estimator.estimate_log2_relative_noise_level(
                    P,
                    &Params::mod_switch_down_C(C_master, &ct.dropped_rns_factor_indices),
                    &ct.info,
                )
            })
        ...
        let int_products: Vec<(El<BigIntRing>, Boo<ModulusAwareCiphertext<Params, Self>>)> = int_products
            .iter()
            .map(|(lhs, rhs)| {
                (
                    ZZbig.clone_el(lhs),
                    self.bring_to_modulus(
                        P,
                        &C_target,
                        C_master,
                        &total_drop,
        ...
        let main_products: Vec<(&El<R>, Boo<ModulusAwareCiphertext<Params, Self>>)> = main_products
            .iter()
            .map(|(lhs, rhs)| {
                (
                    *lhs,
        ...
        let output_implicit_scale = int_products
            .iter()
            .filter_map(|(c, ct)| Zt.invert(&Zt.coerce(&ZZbig, ZZbig.clone_el(c))).map(|c| (c, ct)))
            .map(|(c, ct)| {
                (
                    self.noise_estimator
                        .estimate_log2_relative_noise_level(P, &C_target, &ct.info),
                    Zt.mul_ref_fst(&ct.data.implicit_scale, c),
                )
            })
            .max_by(|(l, _), (r, _)| f64::total_cmp(l, r))
            .map(|(_, scale)| scale)
            .unwrap_or(P.base_ring().one());

        let int_products: Vec<(El<BigIntRing>, ModulusAwareCiphertext<Params, Self>)> = int_products
            .into_iter()
            .map(|(lhs, rhs)| {
                let lhs = int_cast(
                    Zt.smallest_lift(
                        Zt.mul(
                            Zt.coerce(&ZZbig, lhs),
                            Zt.checked_div(&output_implicit_scale, &rhs.data.implicit_scale)
                                .unwrap(),
                        ),
                    ),
                    ZZbig,
        ...
        let main_products: Vec<(Boo<El<R>>, ModulusAwareCiphertext<Params, Self>)> = main_products
            .into_iter()
            .map(|(lhs, rhs)| {
                let factor = Zt
                    .checked_div(&output_implicit_scale, &rhs.data.implicit_scale)
                    .unwrap();
                let mut rhs = rhs.to_owned(|ct| self.clone_ct(P, C_master, ct));
                rhs.data.implicit_scale = Zt.one();
                rhs.info.implicit_scale = Zt.one();
                if !Zt.is_one(&factor) {
                    ...
                    rhs.info = self
                        .noise_estimator
                        .hom_mul_plain_scalar(&P, &C_target, &factor, &rhs.info);
        ...
        let int_product_noise = ZZbig.get_ring().hom_inner_product_noise(
            &self.noise_estimator,
            P,
            &C_target,
            int_products.iter().map(|(lhs, rhs)| (lhs, &rhs.info)),
        );
        let int_product_part = ZZbig.get_ring().hom_inner_product(
            P,
            &C_target,
            int_products
                .into_iter()
                .map(|(lhs, rhs)| (Boo::Owned(lhs), Boo::Owned(rhs.data))),
        );

        let main_product_noise = ring.get_ring().hom_inner_product_noise(
            &self.noise_estimator,
            P,
            &C_target,
            main_products.iter().map(|(lhs, rhs)| (&**lhs, &rhs.info)),
        );
        let main_product_part = ring.get_ring().hom_inner_product(
            P,
            &C_target,
            main_products.into_iter().map(|(lhs, rhs)| (lhs, Boo::Owned(rhs.data))),
        );

        // ignore the last plaintext addition for noise analysis, it's gonna be fine
        let mut product_info = self.noise_estimator.hom_add(
            P,
            &C_target,
            &int_product_noise,
            &main_product_noise,
        ...
        let mut product_data = Params::hom_add(
            P,
            &C_target,
            int_product_part,
            main_product_part,
        ...
        assert!(
            P.base_ring()
                .eq_el(&output_implicit_scale, &product_data.implicit_scale)
        );
        return ModulusAwareCiphertext {
            data: product_data,
            info: product_info,
        ...
        R::Type: AsBGVPlaintext<Params>,
    {
        ...
        let rk = rk.unwrap();
        let (total_drop, special_modulus) = self.compute_optimal_mul_modswitch(
            P,
            C_master,
            &x.info,
            &x.dropped_rns_factor_indices,
            &y.info,
            &y.dropped_rns_factor_indices,
            rk.gadget_vector_digits(),
            used_sk,
        );
        let total_drop_without_special = total_drop.subtract(&special_modulus);
        let C_special = Params::mod_switch_down_C(&C_master, &total_drop_without_special);
        ...
        if LOG {
            println!(
                "HomMul: Result has estimated noise budget {}/{}",
                -self
                    .noise_estimator
                    .estimate_log2_relative_noise_level(P, &C_target, &res_info)
                    .round(),
                ZZbig.abs_log2_ceil(C_target.base_ring().modulus()).unwrap()
            );
            if let Some(sk) = debug_sk {
                ...
                println!(
                    "  actual noise budget: {}",
                    Params::noise_budget(P, &C_target, &res_data, &sk_target)
                );
        ...
        R::Type: AsBGVPlaintext<Params>,
    {
        ...
        if LOG {
            println!(
                "HomSquare: Result has estimated noise budget {}/{}",
                -self
                    .noise_estimator
                    .estimate_log2_relative_noise_level(P, &C_target, &res_info)
                    .round(),
        ...
        R::Type: AsBGVPlaintext<Params>,
    {
        ...
        let get_gk = |g| {
            if let Some(res) = gks
                .iter()
                .filter(|(provided_g, _)| C_master.acting_galois_group().eq_el(g, provided_g))
                .next()
            {
                res
            } else {
                panic!(
                    "Galois key for {} not found",
                    C_master.acting_galois_group().representative(g)
                )
            }
        };
        let gk_digits = get_gk(&gs[0]).1.gadget_vector_digits();
        assert!(
            gs.iter().all(|g| get_gk(g).1.gadget_vector_digits() == gk_digits),
            "when using `gal_many()`, all Galois keys must have the same digits"
        );
        let (total_drop, special_modulus) =
            compute_optimal_special_modulus(C_master.get_ring(), &x.dropped_rns_factor_indices, 0, gk_digits);
        assert!(total_drop.len() < C_master.base_ring().len());
        let C_target = Params::mod_switch_down_C(C_master, &total_drop);
        let total_drop_without_special = total_drop.subtract(&special_modulus);
        let C_special = Params::mod_switch_down_C(&C_master, &total_drop_without_special);
        let gks_mod_switched = gs
            .iter()
            .map(|g| Params::mod_switch_down_gk(&C_special, C_master, &get_gk(g).1))
            .collect::<Vec<_>>();
        ...
        if LOG {
            println!(
                ...
                gk_digits
                    .remove_indices(&total_drop_without_special)
                    .iter()
                    .map(|digit| digit.end - digit.start)
                    .max()
                    .unwrap()
        ...
        return result
            .into_iter()
            .zip(gs.into_iter())
            .zip(gks_mod_switched.iter())
            .map(|((res, g), gk)| ModulusAwareCiphertext {
...
where
    R: ?Sized + AsBGVPlaintext<Inst>,
    Inst: BGVInstantiation,
    N: BGVNoiseEstimator<Inst>,
{
    ring: &'a R,
...
impl<'a, 'b, R, Inst, N, const LOG: bool>
    CircuitEvaluator<'b, ModulusAwareCiphertext<Inst, DefaultModswitchStrategy<Inst, N, LOG>>, R>
    for BGVEvaluator<'a, R, Inst, N, LOG>
where
    R: ?Sized + AsBGVPlaintext<Inst>,
    Inst: BGVInstantiation,
    N: BGVNoiseEstimator<Inst>,
{
    ...
    fn supports_mul(&self) -> bool { self.rk.is_some() && self.rk.is_some() }

    #[instrument(skip_all)]
    fn add_constant(
        &mut self,
        val: ModulusAwareCiphertext<Inst, DefaultModswitchStrategy<Inst, N, LOG>>,
        constant: &'b Coefficient<R>,
    ) -> ModulusAwareCiphertext<Inst, DefaultModswitchStrategy<Inst, N, LOG>> {
        let current_C = Inst::mod_switch_down_C(self.C_master, &val.dropped_rns_factor_indices);
        if let Some(int) = constant.as_integer() {
            ModulusAwareCiphertext {
                info: ZZbig.get_ring().hom_add_to_noise(
                    &self.strategy.noise_estimator,
                    self.P,
                    &current_C,
                    &int,
                    &val.info,
            ...
            ModulusAwareCiphertext {
                info: self.ring.hom_add_to_noise(
                    &self.strategy.noise_estimator,
                    self.P,
                    &current_C,
                    &constant,
    ...
    fn gal(
        ...
        val: ModulusAwareCiphertext<Inst, DefaultModswitchStrategy<Inst, N, LOG>>,
        gs: &'b [GaloisGroupEl],
    ) -> Vec<ModulusAwareCiphertext<Inst, DefaultModswitchStrategy<Inst, N, LOG>>> {
        self.strategy.gal_many(
            self.P,
            self.C_master,
            val,
    ...
    where
        I: Iterator<
            Item = (
                &'b Coefficient<R>,
                &'c ModulusAwareCiphertext<Inst, DefaultModswitchStrategy<Inst, N, LOG>>,
            ),
        >,
        R: 'b,
    ...
    fn mul(
        &mut self,
        lhs: ModulusAwareCiphertext<Inst, DefaultModswitchStrategy<Inst, N, LOG>>,
        rhs: ModulusAwareCiphertext<Inst, DefaultModswitchStrategy<Inst, N, LOG>>,
    ) -> ModulusAwareCiphertext<Inst, DefaultModswitchStrategy<Inst, N, LOG>> {
        self.strategy.mul(
            self.P,
            self.C_master,
...
impl<Params: BGVInstantiation, N: BGVNoiseEstimator<Params>, const LOG: bool> BGVModswitchStrategy<Params>
    for DefaultModswitchStrategy<Params, N, LOG>
{
    ...
    fn info_for_fresh_encryption(
        ...
        C: &CiphertextRing<Params>,
        sk: SecretKeyDistribution,
    ) -> <Self as BGVModswitchStrategy<Params>>::CiphertextInfo {
    ...
    fn print_info(
        ...
        println!(
            "estimated noise: {}",
            self.noise_estimator
                .estimate_log2_relative_noise_level(P, &Clocal, &ct.info)
        );
    }
}
...
fn test_modswitch_strategy_inner_prod() {
    ...
    let modswitch_strategy: DefaultModswitchStrategy<Pow2BGV, _, true> =
        DefaultModswitchStrategy::new(NaiveBGVNoiseEstimator);
    let inputs = [P.int_hom().map(2), P.int_hom().map(100), P.int_hom().map(-1)];
    let mut cts = inputs
        .iter()
        .map(|x| ModulusAwareCiphertext {
    ...
    let res = modswitch_strategy.inner_prod(
        &P,
        &C,
        &[
            &Coefficient::NegOne,
    ...
    assert_el_eq!(
        &P,
        &P.int_hom().map(-2 + 100 - 17),
        Pow2BGV::dec(&P, &res_C, res.data, &res_sk)
    );

    let to_drop = RNSFactorIndexList::from([0], C.base_ring().len());
    let C_new = Pow2BGV::mod_switch_down_C(&C, &to_drop);
    cts[0] = modswitch_strategy.mod_switch_down(
        &P,
        &C_new,
        &C,
        &to_drop,
        modswitch_strategy.clone_ct(&P, &C, &cts[0]),
        "",
        None,
    );
    ...
    let C_new = Pow2BGV::mod_switch_down_C(&C, &to_drop);
    cts[2] = modswitch_strategy.mod_switch_down(
        &P,
        &C_new,
        &C,
        &to_drop,
        modswitch_strategy.clone_ct(&P, &C, &cts[2]),
        "",
        None,
    );
...
/// chosen parameters, this triggers a nontrivial split (some factors fake-dropped, some
/// real-dropped), which can be inspected in the `LOG` output ("... via fake modulus-switch and ...
/// via real modulus-switch").
#[test]
fn test_modswitch_strategy_inner_prod_partial_fake_modswitch() {
    ...
    assert!(
        rns_len >= 5,
        "test requires enough RNS factors to drop a nontrivial subset"
    );
    let sk = Pow2BGV::gen_sk(&C, &mut rng, SecretKeyDistribution::UniformTernary);
...
fn test_modswitch_strategy_evaluate_circuit() {
    ...
    assert_el_eq!(
        &P,
        &circuit.evaluate(&[input], P.inclusion())[0],
        Pow2BGV::dec(&P, &res_C, res.data, &res_sk)
    );
}
