...
///
/// This is the analogue, on the noise-estimation side, of [`Ciphertext`]: every operation
/// of [`BGVNoiseEstimator`] takes and returns a `CiphertextDescriptor`, exactly mirroring
/// how the corresponding [`BGVInstantiation`] operation takes and returns a [`Ciphertext`].
/// Note that the implicit scale and secret key are tracked here in the same way for a
/// [`Ciphertext`] and a [`CiphertextNoRelin`], so a single `CiphertextDescriptor` describes
/// the noise of either.
///
...
///
/// Returns the implicit scale of a product, see [`BGVInstantiation::hom_mul_norelin()`].
///
...
///
/// Returns the implicit scale after a modulus-switch, see [`BGVInstantiation::mod_switch_ct()`].
///
...
///
/// Returns the implicit scale after a plaintext-modulus change, see
/// [`BGVInstantiation::change_plaintext_modulus()`].
///
...
///
/// Computes the implicit scale of the result of an addition, following the given
/// [`ImplicitScalePolicy`]; see [`BGVInstantiation::hom_add()`].
///
...
///  - [`Ciphertext`] (and [`CiphertextNoRelin`]) parameters/return values are replaced by
...
pub trait BGVNoiseEstimator<Params: BGVInstantiation>: Sized {
    ...
    ///
    fn hom_inner_product_plain_scalar<L, R, I>(&self, P: &PlaintextRing<Params>, C: &CiphertextRing<Params>, summands: I) -> CiphertextDescriptor<Params, Self>
        where L: Borrow<ImplicitScale<Params>>,
