...
use crate::bgv::eval::{AsBGVPlaintext, EncodedBGVPlaintextRing, EncodedBGVPlaintextRingBase};
use crate::bgv::modswitch::*;
use crate::bgv::noise_estimator::CiphertextDescriptor;
use crate::bgv::*;
use crate::circuit::*;
use crate::filename_keys;
use crate::lin_transform::{composite, pow2};
    ...
    Strategy: BGVModswitchStrategy<Inst>,
{
    ...
    original_plaintext_ring: EncodedBGVPlaintextRing<Inst>,
    ...
    CiphertextRing<Inst>: Clone,
{
    ...
    ///  - **Slots-to-Coeffs**: Move the values stored in the slots of the input ciphertext into its
    ///    coefficients
    ///  - **Mod-switch**: Modulus-switches the ciphertext to an intermediate plaintext modulus
    ///    `p^e`. This means the ciphertext now can be used as a plaintext.
    ///  - **Noisy expansion**: Converts the modulus-switched ciphertext into a low-noise
    ///    ciphertext, which encrypts the same coefficients as the the modulus-switched ciphertext,
    ///    plus some noise. This requires an encryption of the secret key.
    ///  - **Coeffs-to-Slots**: Moves the coefficients (with noise) into the slots of the
    ///    ciphertext.
    ///  - **Digit Extraction**: Removes the noise from the encoded values and scales them down.
    ///
    /// The parameters are as follows:
    ///  - `instantiation` describes the scheme whose ciphertexts are to be bootstrapped
    ///  - `C` is the ciphertext ring over which a to-be-bootstrapped input ciphertext should be
    ///    defined
    ///  - `slots_to_coeffs_thin` is the circuit which is used to compute the Slots-to-Coeffs
    ///    transform. The coefficients of this circuit should be taken from the plaintext ring of
    ///    the scheme with modulus `t`.
    ///  - `coeffs_to_slots_thin` is the circuit which is used to compute the Coeffs-to-Slots
    ///    transform. The coefficients of this circuit should be taken from the plaintext ring of
    ///    the scheme with modulus `p^e`.
    ///  - `digit_extract` is the function used for digit extraction.
    ///  - `slots_to_coeffs_rns_factors` is the number of RNS factors to use when computing the
    ///    Slots-to-Coeffs transform. More concretely, since the result of the Slots-to-Coeffs
    ///    transform does not have to have any noise budget left, the input ciphertext can be
    ///    mod-switched to a lower modulus ciphertext ring before the Slots-to-Coeffs transform,
    ///    which will improve performance of the Slots-to-Coeffs transform. Since BGV uses hybrid
    ///    key-switching, this can be quite low, and just has to be large enough to accomodate the
    ///    noise growth caused by the Slots-to-Coeffs transform.
    ///
    ...
    pub fn create(
        ...
        let original_plaintext_ring = EncodedBGVPlaintextRingBase::new(original_plaintext_ring, C_master.clone());
        let intermediate_plaintext_ring = EncodedBGVPlaintextRingBase::new(intermediate_plaintext_ring, C_master);
        let plaintext_ring_hierarchy = ((r + 1)..e)
            .map(|k| instantiation.create_plaintext_ring(ZZbig.pow(ZZbig.clone_el(&p), k)))
            .collect();
        let coeffs_to_slots_thin = coeffs_to_slots_thin
            .change_ring_uniform(|x| x.change_ring(|x| intermediate_plaintext_ring.get_ring().rev_delegate(x)));
        let slots_to_coeffs_thin = slots_to_coeffs_thin
            .change_ring_uniform(|x| x.change_ring(|x| original_plaintext_ring.get_ring().rev_delegate(x)));
        let tmp_coprime_modulus_plaintext =
            instantiation.create_plaintext_ring(ZZbig.add(ZZbig.pow(ZZbig.clone_el(&p), e), ZZbig.one()));
        Self {
            digit_extract,
    ...
    /// Parameters:
    ///  - `instantiation` describes the scheme whose ciphertexts are to be bootstrapped.
    ///  - `P` is the plaintext ring which the input ciphertext encrypts an element from. Its
    ///    modulus `t` should be a power of a prime, i.e. `t = p^r`.
    ///  - `C` is the ciphertext ring over which a to-be-bootstrapped input ciphertext should be
    ///    defined.
    ///  - `v` is the number of digits to remove. In other words, during bootstrapping the noise is
    ///    removed from an intermediate "noisy decryption" using a rounded division by `p^v`. Hence,
    ///    `p^v/2` should be larger than the expected magnitude of the noise, after
    ///    modulus-switching to `p^e` with `e = v + r`.
    ...
    ///  - `strategy` is the modulus-switching strategy to use when evaluating the digit extraction
    ///    circuits during bootstrapping.
    ///  - `cache_dir` specifies a directory to load and store precomputed data. If it is `None`, no
    ///    data will be read or written, but always computed from scratch.
    #[instrument(skip_all)]
    pub fn build_pow2(
        instantiation: &Inst,
    ...
    where
        Inst::PlaintextRing: SerializableElementRing,
        Inst::CiphertextRing: Clone,
    {
        let log2_m = ZZi64
            .abs_log2_ceil(&(instantiation.number_ring().galois_group().m() as i64))
            .unwrap();
        ...
        let plaintext_ring_hierarchy = ((r + 1)..e)
            .map(|k| instantiation.create_plaintext_ring(ZZbig.pow(ZZbig.clone_el(&p), k)))
            .collect::<Vec<_>>();
        let all_plaintext_rings = [&base_plaintext_ring]
            .into_iter()
            .chain(plaintext_ring_hierarchy.iter())
            .chain([&intermediate_plaintext_ring])
            .collect::<Vec<_>>();

        let hypercube = HypercubeStructure::default_pow2_hypercube(
            intermediate_plaintext_ring.acting_galois_group(),
            ZZbig.clone_el(&p),
        );
        ...
        let min_rns_factor_log2 = C_master
            .base_ring()
            .as_iter()
            .map(|rns_factor| *rns_factor.modulus() as i64)
        ...
        Inst::CiphertextRing: Clone,
    {
        ...
        let hypercube = HypercubeStructure::halevi_shoup_hypercube(
            intermediate_plaintext_ring.acting_galois_group(),
            ZZbig.clone_el(&p),
        );
        let H = LazyCell::new(|| HypercubeIsomorphism::new(&intermediate_plaintext_ring, &hypercube, cache_dir));
    ...
    Strategy: BGVModswitchStrategy<Inst>,
{
    ...
    pub fn r(&self) -> usize { self.digit_extract.e() - self.digit_extract.v() }

    pub fn e(&self) -> usize { self.digit_extract.e() }

    pub fn v(&self) -> usize { self.digit_extract.v() }

    pub fn p(&self) -> &El<BigIntRing> { self.digit_extract.p() }
    ...
    pub fn base_plaintext_ring(&self) -> &PlaintextRing<Inst> {
        self.original_plaintext_ring.get_ring().plaintext_ring()
    }
    ...
    pub fn complete_plaintext_ring_sequence<'a>(&'a self) -> Vec<&'a PlaintextRing<Inst>> {
        [self.base_plaintext_ring()]
            .into_iter()
            .chain(self.plaintext_ring_hierarchy.iter())
            .chain([self.intermediate_plaintext_ring()])
            .collect::<Vec<_>>()
    }
    ...
    pub fn required_galois_keys(&self, P: &PlaintextRing<Inst>) -> Vec<GaloisGroupEl> {
        ...
        result.extend(
            self.slots_to_coeffs_thin
                .required_galois_keys(&P.acting_galois_group())
                .into_iter(),
        );
        result.extend(
    ...
    fn prepare_input_for_slots_to_coefficients(
        ...
        // First, we mod-switch the input ciphertext so that it only has `self.slots_to_coeffs_rns_factors`
        // many RNS factors
        ...
        let ct_input = Inst::mod_switch_ct(
            P_base,
            &C_input,
            &Inst::mod_switch_down_C(C_master, ct_dropped_moduli),
            ct,
        );
    ...
    fn perform_slots_to_coefficients(
        ...
        let never_modswitch = DefaultModswitchStrategy::never_modswitch();
        let result = never_modswitch.evaluate_circuit(
            &self.slots_to_coeffs_thin,
            &self.original_plaintext_ring,
            P_base,
            C_master,
            &[ModulusAwareCiphertext {
                info: CiphertextDescriptor {
                    implicit_scale: P_base.base_ring().clone_el(&ct.implicit_scale),
                    noise: (),
    ...
    fn perform_noisy_expansion(
        ...
        let C_master = self.master_ciphertext_ring();
        // this is slightly more complicated than in BFV, since we cannot mod-switch to a ciphertext modulus
        // that is not coprime to `t = p^r`. Instead, we first multiply by `p^v`, then mod-switch to
        // `p^e + 1`, and then reduce the shortest lift of the result modulo `p^e`.
        // This will introduce the overflow modulo `p^e + 1` as error in the lower bits, which we will later
        // remove during digit extraction
        let perform_noisy_expansion = |C: &CiphertextRing<Inst>, ct: Ciphertext<Inst>, enc_sk: Ciphertext<Inst>| {
            ...
            // change to `p^e + 1`
            let (c0, c1) =
                Inst::mod_switch_to_plaintext(P_main, &self.tmp_coprime_modulus_plaintext, &C, values_scaled);
            // reduce modulo `p^e`, which will introduce additional error in the lower digits
            let mod_pe = P_main
                .base_ring()
                .can_hom(self.tmp_coprime_modulus_plaintext.base_ring().integer_ring())
                .unwrap();
            let (c0, c1) = (
                P_main.from_canonical_basis(
                    self.tmp_coprime_modulus_plaintext
                        .wrt_canonical_basis(&c0)
                        .iter()
                        .map(|x| mod_pe.map(self.tmp_coprime_modulus_plaintext.base_ring().smallest_lift(x))),
                ),
                P_main.from_canonical_basis(
                    self.tmp_coprime_modulus_plaintext
                        .wrt_canonical_basis(&c1)
                        .iter()
                        .map(|x| mod_pe.map(self.tmp_coprime_modulus_plaintext.base_ring().smallest_lift(x))),
                ),
            );
            return ModulusAwareCiphertext {
                data: Inst::hom_add_plain(
                    P_main,
                    C_master,
                    &c0,
                    Inst::hom_mul_plain(P_main, C_master, &c1, enc_sk),
                ),
                info: self
                    .modswitch_strategy
                    .info_for_fresh_encryption(P_main, C_master, used_sk),
        ...
        let result = if let Some(sparse_sk_encaps) = sparse_sk_encaps {
            ...
            perform_noisy_expansion(
                &sparse_sk_encaps.C_sparse_sk,
                ct_keyswitched,
                Inst::clone_ct(P_main, C_master, &sparse_sk_encaps.encapsulated_key),
            )
        } else {
    ...
    fn perform_coefficients_to_slots(
        ...
        if let Some(sk) = debug_sk {
            ...
            Inst::dec_println_slots(
                P_main,
                &C_current,
                &result.data,
                &Inst::mod_switch_sk(&C_current, C_master, sk),
                Some("./cache"),
            );
        }
        return result;
    ...
    ///  - `ct_dropped_moduli` contains all RNS factor indices of `C_master` that aren't used by
    ///    `ct` (anymore); More concrete, `ct` lives over the ciphertext ring one obtains by
    ///    dropping the RNS factors with these indices from the RNS base of `C_master`
    ///  - `ct` is the ciphertext to bootstrap; It must be thinly packed (i.e. each slot may only
    ///    contain an element of `Z/(t)`), otherwise this function will cause immediate noise
    ///    overflow.
    ...
    ///    must contain a Galois key for each Galois automorphism listed in
    ///    [`ThinBootstrapper::required_galois_keys()`], but may contain additional Galois keys
    ///  - `debug_sk` can be a reference to a secret key, which is used to print out decryptions of
    ///    intermediate results for debugging purposes. May only be set if `LOG == true`.
    #[instrument(skip_all)]
    ...
    where
        Inst: 'a,
    {
        let (C_slots_to_coeffs, ct) = self.prepare_input_for_slots_to_coefficients(
            ct_dropped_moduli,
            ct,
            gks[0].1.gadget_vector_digits(),
            debug_sk,
        );

        let values_in_coefficients = self.perform_slots_to_coefficients(&C_slots_to_coeffs, ct, gks, debug_sk);

        let noisy_decryption = self.perform_noisy_expansion(
            &C_slots_to_coeffs,
            values_in_coefficients,
            used_sk,
            sk_encaps_data,
            debug_sk,
        );
    ...
    Params: BGVInstantiation,
{
    pub fn create<R: CryptoRng + Rng>(
        ...
        C_sparse_sk: CiphertextRing<Params>,
        sparse_sk: SecretKey<Params>,
        standard_sk: &SecretKey<Params>,
        mut rng: R,
        noise_sigma: f64,
    ) -> Self {
        ...
        let ZZ_to_Pbase = P
            .base_ring()
            .can_hom(P.base_ring().integer_ring())
            .unwrap()
            .compose(P.base_ring().integer_ring().can_hom(&ZZbig).unwrap());
        let sparse_sk_as_plain = P.from_canonical_basis(
            C_sparse_sk
                .wrt_canonical_basis(&sparse_sk)
                .iter()
                .map(|x| ZZ_to_Pbase.map(C_sparse_sk.base_ring().smallest_lift(x))),
        );
    ...
    pub fn new<R: CryptoRng + Rng>(
        ...
        C: &CiphertextRing<Params>,
        standard_sk: &SecretKey<Params>,
        C_sparse_rns_factor_count: usize,
        hwt: usize,
        mut rng: R,
        noise_sigma: f64,
    ) -> Self {
        let C_sparse_sk = RingValue::from(C.get_ring().drop_rns_factor(&RNSFactorIndexList::from(
            C_sparse_rns_factor_count..C.base_ring().len(),
            C.base_ring().len(),
        )));
...
impl<R: ?Sized + RingBase> DigitExtract<R> {
        ...
        R: AsBGVPlaintext<Inst>,
    {
        ...
        for i in 0..=self.v() {
            assert_el_eq!(
                ZZbig,
                ZZbig.pow(ZZbig.clone_el(self.p()), actual_r + i),
                int_cast(ZZ.clone_el(P[i].base_ring().modulus()), ZZbig, ZZ)
            );
        }
        ...
        return self.evaluate_generic(
            input,
            |exp, inputs, circuit| {
                let digit_extracted = modswitch_strategy.evaluate_circuit(
                    circuit,
                    &rings[exp - self.r()],
                    P[exp - self.r()],
                    C_master,
            ...
            |exp_old, exp_new, input| {
                ...
                let result = ModulusAwareCiphertext {
                    data: Inst::change_plaintext_modulus(
                        P[exp_new - self.r()],
                        P[exp_old - self.r()],
                        &C_current,
                        input.data,
...
fn test_digit_extract_homomorphic() {
    ...
    let rk = Pow2BGV::gen_rk(
        ...
        &mut rng,
        &sk,
        &RNSGadgetVectorDigitIndices::select_digits(7, C_master.base_ring().len()),
        3.2,
    );
    ...
    let (ct_high, ct_low) = digitextract.evaluate_bgv::<_, Pow2BGV, _>(
        &[P1.base_ring(), P2.base_ring()],
        &strategy,
        &[&P1, &P2],
        &C_master,
        ModulusAwareCiphertext {
    ...
    let m_high = Pow2BGV::dec(
        &P1,
        &C_result,
        Pow2BGV::clone_ct(&P1, &C_result, &ct_high.data),
        &sk_result,
    );
    assert!(
        P1.wrt_canonical_basis(&m_high)
            .iter()
            .skip(1)
            .all(|x| P1.base_ring().is_zero(&x))
    );
    let m_high = P1.base_ring().smallest_lift(P1.wrt_canonical_basis(&m_high).at(0));
    assert_eq!(17 + 2, m_high);

    let m_low = Pow2BGV::dec(
        &P2,
        &C_result,
        Pow2BGV::clone_ct(&P2, &C_result, &ct_low.data),
        &sk_result,
    );
    assert!(
        P2.wrt_canonical_basis(&m_low)
            .iter()
            .skip(1)
            .all(|x| P2.base_ring().is_zero(&x))
    );
...
fn test_pow2_bgv_thin_bootstrapping_17() {
    ...
    let bootstrapper = ThinBootstrapper::build_pow2(
        ...
        4,
        &key_switch_params,
        DefaultModswitchStrategy::<_, _, true>::new(NaiveBGVNoiseEstimator),
        Some("./cache"),
    );

    let sk = Pow2BGV::gen_sk(&C_master, &mut rng, SecretKeyDistribution::UniformTernary);
    let gk = Pow2BGV::gen_gks(
        bootstrapper.intermediate_plaintext_ring(),
        &C_master,
        &mut rng,
        &sk,
        bootstrapper.required_galois_keys(&P),
        &key_switch_params,
        3.2,
    );
    let rk = Pow2BGV::gen_rk(
    ...
    assert_el_eq!(
        P,
        P.int_hom().map(2),
        Pow2BGV::dec(&P, &C_result, ct_result.data, &sk_result)
    );
}
...
fn test_composite_bgv_thin_bootstrapping_2_sparse_key_encapsulation() {
    ...
    let C_master = params.create_ciphertext_ring(790..800);
    let key_switch_params = RNSGadgetVectorDigitIndices::select_digits(5, C_master.base_ring().len());
    let bootstrapper = ThinBootstrapper::build_odd(
        &params,
        &P,
        &C_master,
        4,
    ...
    let encaps = SparseKeyEncapsulationKey::new(
        bootstrapper.intermediate_plaintext_ring(),
        &C_master,
        &sk,
        2,
    ...
    assert_el_eq!(
        P,
        P.int_hom().map(2),
        CompositeBGV::dec(&P, &C_result, ct_result.data, &sk_result)
    );
}
...
fn measure_time_single_rns_composite_bgv_thin_bootstrapping() {
    ...
    let bootstrapper = ThinBootstrapper::build_odd(
        ...
        4,
        &key_switch_params,
        DefaultModswitchStrategy::<_, _, false>::new(NaiveBGVNoiseEstimator),
        Some("./cache"),
    );

    let sk = CompositeSingleRNSBGV::gen_sk(&C_master, &mut rng, sk_distr);
    let gk = CompositeSingleRNSBGV::gen_gks(
        bootstrapper.intermediate_plaintext_ring(),
        &C_master,
        &mut rng,
        &sk,
    ...
    let rk = CompositeSingleRNSBGV::gen_rk(
        bootstrapper.intermediate_plaintext_ring(),
        &C_master,
        &mut rng,
        &sk,
    ...
    let ct_result = bootstrapper.bootstrap_thin(&RNSFactorIndexList::empty(), ct, &rk, &gk, sk_distr, None, None);
    ...
    println!(
        "final noise budget: {}",
        CompositeSingleRNSBGV::noise_budget(&P, &C_result, &ct_result.data, &sk_result)
    );
...
fn measure_time_double_rns_composite_bgv_thin_bootstrapping() {
    ...
    println!(
        "final noise budget: {}",
        CompositeBGV::noise_budget(&P, &C_result, &ct_result.data, &sk_result)
    );
...
fn measure_time_double_rns_pow2_bgv_thin_bootstrapping() {
    ...
    println!(
        "final noise budget: {}",
        Pow2BGV::noise_budget(&P, &C_result, &ct_result.data, &sk_result)
    );
