...
#[instrument(skip_all)]
fn dwt1d_matrix<R>(
    H: &HypercubeStructure,
    slot_ring: &SlotRingOver<R>,
    dim_index: usize,
    zeta_powertable: &PowerTable<&SlotRingOver<R>>,
) -> OwnedMatrix<El<SlotRingOver<R>>>
where
    R: RingStore,
    R::Type: NiceZn,
{
...
fn dwt1d<'a, R>(
    ...
    dim_index: usize,
    zeta_powertable: &PowerTable<&SlotRingOf<R>>,
    ...
    BaseRing<R>: NiceZn,
{
    ...
    if H.hypercube().dim_length(dim_index) == 1 {
        ...
        // multiplication with the matrix `A(i, j) = 𝝵^(j * shift_element(-i))` if we consider an element as
        // multiple vectors along the `dim_index`-th dimension
        let A = dwt1d_matrix(H.hypercube(), H.slot_ring(), dim_index, zeta_powertable);

        vec![MatmulTransform::matmul1d(H, dim_index, |i, j, _idxs| {
            H.slot_ring().clone_el(A.at(i, j))
        })]
    }
}
...
///
/// In this step, for each hypercolumn along the first hypercube dimension, the coefficients of each
/// slot of (w.r.t. the power-of-`𝝵` basis of each slot) are moved to the coefficients of a `d l_1 =
/// phi(m_1)` degree polynomial, stored "along" the hypercolumn.
...
/// ```
///
/// This requires that the underlying hypercube structure is a Halevi-Shoup hypercube structure, and
/// that `d l_1 = phi(m_1)`.
#[instrument(skip_all)]
fn slots_to_powcoeffs_fat_fst_step<R>(
    H: &HypercubeIsomorphism<R>,
    dim_index: usize,
    zeta_powertable: &PowerTable<&SlotRingOf<R>>,
) -> OwnedMatrix<El<<R::Type as RingExtension>::BaseRing>>
where
    R: RingStore,
    R::Type: Sized + NumberRingQuotient,
    BaseRing<R>: NiceZn,
{
    ...
    OwnedMatrix::from_fn(
        H.hypercube().dim_length(dim_index) * H.slot_ring().rank(),
        H.hypercube().dim_length(dim_index) * H.slot_ring().rank(),
        |row_idx, col_idx| {
            ...
            return H
                .slot_ring()
                .wrt_canonical_basis(
                    &*zeta_powertable.get_power(Gal.underlying_ring().smallest_positive_lift(exponent)),
                )
                .at(k);
        },
    )
...
///
/// If for the linear transform input, the slot `(i_1, ..., i_r)` contains `sum_j a_(j, i_1, ...,
/// i_r) 𝝵^j`, this this transform "puts" `a_(j, i_1, ..., i_r)` into the powerful-basis coefficient
/// of `X1^(j * l_1 + i_1) X2^i_2 ... Xr^i_r`. Formally, this is the `Z/p^eZ`-linear map that maps
/// ```text
...
/// that `d l_1 = phi(m_1)`.
#[instrument(skip_all)]
pub fn slots_to_powcoeffs_fat<R>(
    H: &HypercubeIsomorphism<R>,
    max_levels: usize,
    ...
    BaseRing<R>: NiceZn,
{
    ...
    let zeta_powertable = PowerTable::new(
        H.slot_ring(),
        H.slot_ring().canonical_gen(),
        H.galois_group().m() as usize,
    );

    let fst_step_matrix = slots_to_powcoeffs_fat_fst_step(H, 0, &zeta_powertable);
    result.push(MatmulTransform::blockmatmul1d(H, 0, |(i, k), (j, l), _idxs| {
        H.slot_ring()
            .base_ring()
            .clone_el(fst_step_matrix.at(i * H.slot_ring().rank() + k, j * H.slot_ring().rank() + l))
    }));
...
///
/// In other words, this moves the powerful-basis coefficient of `X1^(j * l_1 + i1) X2^i_2 ...
/// Xr^i_r` to the coefficient of `𝝵^j` within the slot `(i_1, ..., i_r)`.
...
/// that `d l_1 = phi(m_1)`.
#[instrument(skip_all)]
pub fn powcoeffs_to_slots_fat<R>(
    H: &HypercubeIsomorphism<R>,
    max_levels: usize,
    cost_model: &CircuitEvaluatorCosts,
) -> PlaintextCircuit<R::Type>
    ...
    BaseRing<R>: NiceZn,
{
    ...
    let mut rhs = OwnedMatrix::identity(
        H.hypercube().dim_length(0) * H.slot_ring().rank(),
        H.hypercube().dim_length(0) * H.slot_ring().rank(),
        H.slot_ring().base_ring(),
    );
    let mut sol = OwnedMatrix::zero(
        H.hypercube().dim_length(0) * H.slot_ring().rank(),
        H.hypercube().dim_length(0) * H.slot_ring().rank(),
        H.slot_ring().base_ring(),
    );
    <_ as LinSolveRingStore>::solve_right(H.slot_ring().base_ring(), A.data_mut(), rhs.data_mut(), sol.data_mut())
        .assert_solved();

    result.push(MatmulTransform::blockmatmul1d(H, 0, |(i, k), (j, l), _idxs| {
        H.slot_ring()
            .base_ring()
            .clone_el(sol.at(i * H.slot_ring().rank() + k, j * H.slot_ring().rank() + l))
    }));
...
///
/// If for the linear transform input, the slot `(i_1, ..., i_r)` contains a scalar `a_(i_1, ...,
/// i_r)`, this transform "puts" `a_(i_1, ..., i_r)` into the powerful-basis coefficient of `X1^i_1
/// ... Xr^i_r`. If the slot doesn't contain a scalar, the behavior is unspecified.
#[instrument(skip_all)]
pub fn slots_to_powcoeffs_thin<R>(
    H: &HypercubeIsomorphism<R>,
    max_levels: usize,
    cost_model: &CircuitEvaluatorCosts,
) -> PlaintextCircuit<R::Type>
    ...
    BaseRing<R>: NiceZn,
{
    MatmulTransform::to_circuit_many(
        H.ring(),
        H.hypercube(),
        slots_to_powcoeffs_thin_impl(H),
        max_levels,
        cost_model,
    )
}
...
#[instrument(skip_all)]
fn powcoeffs_to_slots_thin_last_step<R>(
    H: &HypercubeIsomorphism<R>,
    dim_index: usize,
    zeta_powertable: &PowerTable<&SlotRingOf<R>>,
) -> OwnedMatrix<El<SlotRingOf<R>>>
where
    R: RingStore,
    R::Type: Sized + NumberRingQuotient,
    BaseRing<R>: NiceZn,
{
    ...
    let scale = slot_ring
        ...
        .invert(
            &slot_ring
                .base_ring()
                .coerce(&StaticRing::<i64>::RING, H.hypercube().factor_of_m(dim_index).unwrap()),
        )
        .unwrap();

    OwnedMatrix::from_fn(
        H.hypercube().dim_length(dim_index),
        H.hypercube().dim_length(dim_index),
        |k, j| {
            ...
            return slot_ring.inclusion().mul_ref_map(
                &slot_ring.sub_ref(
...
///
/// It does move the value from the powerful-basis coefficients `X1^i_1 ... Xr^i_r` for `i_1 <
/// phi(m_1)/d` and `i_2 < phi(m_2), ..., i_r < phi(m_r)` to the slot `(i_1, ..., i_r)`; However,
/// values corresponding to other powerful-basis coefficients are discarded, i.e. mapped to zero. In
/// particular this transform does not have full rank, and cannot be the mathematical inverse of
/// [`slots_to_powcoeffs_thin()`].
#[instrument(skip_all)]
pub fn powcoeffs_to_slots_thin<R>(
    H: &HypercubeIsomorphism<R>,
    max_levels: usize,
    cost_model: &CircuitEvaluatorCosts,
) -> PlaintextCircuit<R::Type>
    ...
    BaseRing<R>: NiceZn,
{
    ...
    trace_circuit.compose(
        MatmulTransform::to_circuit_many(
            H.ring(),
            H.hypercube(),
            powcoeffs_to_slots_thin_base(H),
            max_levels,
            cost_model,
        ),
        H.ring(),
    ...
    BaseRing<R>: NiceZn,
{
    ...
    result.push(MatmulTransform::matmul1d(H, 0, |i, j, _| {
        H.slot_ring().clone_el(last_step_matrix.at(i, j))
    }));
...
fn test_slots_to_powcoeffs_thin() {
    ...
    for transform in slots_to_powcoeffs_thin_impl(&H) {
        current = ring
            .get_ring()
            .compute_linear_transform(H.hypercube(), &current, &transform);
    }
    let expected = ring.sum(
        [0, 5, 7, 12, 14, 19, 21, 26]
            .into_iter()
            .map(|k| ring.pow(ring.canonical_gen(), k)),
    );
    ...
    let ring_ref = &ring;
    let expected = ring.sum((0..2).flat_map(|i| {
        (0..4).map(move |j| {
            ring_ref.mul(
                ring_ref.pow(ring_ref.canonical_gen(), i * 5 + j * 7),
                ring_ref.int_hom().map((1 + j + i * 4) as i32),
            )
        })
    }));
    assert_el_eq!(ring, expected, current);
    ...
    let ring_ref = &ring;
    let expected = ring.sum((0..4).flat_map(|i| {
        (0..6).map(move |j| {
            ring_ref.mul(
                ring_ref.pow(ring_ref.canonical_gen(), i * 7 + j * 5),
                ring_ref.int_hom().map((1 + j + i * 6) as i32),
            )
        })
    }));
    assert_el_eq!(ring, expected, current);
    ...
    let expected = ring.sum((0..30).map(|j| {
        ring.mul(
            ring.pow(ring.canonical_gen(), j * 11),
            ring.int_hom().map((j + 1) as i32),
        )
    }));
    assert_el_eq!(ring, expected, current);
}
...
fn test_powcoeffs_to_slots_thin() {
    ...
    let hypercube = HypercubeStructure::halevi_shoup_hypercube(ring.acting_galois_group(), int_cast(11, ZZbig, ZZi64));
    let H = HypercubeIsomorphism::new(&ring, &hypercube, None);
    let trace = MatmulTransform::linear_combine_shifts_with_frobenius(
        &H,
        (0..H.slot_ring().rank()).map(|i| (vec![i as i64, 0, 0], H.ring().one())),
    );
    assert_eq!(7, H.hypercube().factor_of_m(0).unwrap());
    assert_eq!(2, H.hypercube().dim_length(0));
    assert_eq!(5, H.hypercube().factor_of_m(1).unwrap());
    ...
    current = ring
        .get_ring()
        .compute_linear_transform(H.hypercube(), &current, &trace);
    let expected = H.from_slot_values(
        [H.slot_ring().one()]
            .into_iter()
            .chain((2..9).map(|_| H.slot_ring().zero())),
    );
    assert_el_eq!(ring, expected, current);

    let ring_ref = &ring;
    let mut current = ring.sum((0..6).flat_map(|i| {
        (0..4).map(move |j| {
            ring_ref.mul(
                ring_ref.pow(ring_ref.canonical_gen(), i * 5 + j * 7),
                ring_ref.int_hom().map((1 + j + i * 4) as i32),
    ...
    let expected = H.from_slot_values(
        [1, 2, 3, 4, 5, 6, 7, 8]
            .into_iter()
            .map(|i| H.slot_ring().int_hom().map(i)),
    );
    assert_el_eq!(ring, expected, current);
    ...
    let ring_ref = &ring;
    let mut current = ring.sum((0..4).flat_map(|i| {
        (0..6).map(move |j| {
            ring_ref.mul(
                ring_ref.pow(ring_ref.canonical_gen(), i * 7 + j * 5),
                ring_ref.int_hom().map((1 + j + i * 6) as i32),
...
fn test_slots_to_powcoeffs_fat() {
    ...
    let mut current = H.from_slot_values((1..9).map(|i| {
        H.slot_ring()
            .from_canonical_basis([hom.map(i), hom.map(i + 100), hom.map(i + 200)])
    }));
    for transform in slots_to_powcoeffs_fat_impl(&H) {
        current = ring
            .get_ring()
    ...
    let ring_ref = &ring;
    let expected = ring.sum((0..3).flat_map(|k| {
        (0..2).flat_map(move |i| {
            (0..4).map(move |j| {
                ring_ref.mul(
                    ring_ref.pow(ring_ref.canonical_gen(), (i + k * 2) * 5 + j * 7),
                    ring_ref.int_hom().map((1 + j + i * 4 + k * 100) as i32),
                )
            })
        })
    }));
...
fn test_powcoeffs_to_slots_fat() {
    ...
    let expected = H.from_slot_values([1, 2, 3, 4, 5, 6, 7, 8].into_iter().map(|i| {
        H.slot_ring().from_canonical_basis(
            [i, i + 8, i + 16]
                .into_iter()
                .map(|i| H.slot_ring().base_ring().int_hom().map(i)),
        )
    }));
    assert_el_eq!(ring, expected, current);
}
...
fn test_powcoeffs_to_slots_thin_large() {
    feanor_tracing::DelayedLogger::init_test();
    let ring = RingValue::from(
        NumberRingQuotientByIntBase::new(TensorProductNumberRing::new(337, 127), Zn::new(65536)).into(),
    );
    ...
    let expected = H.from_slot_values(H.hypercube().hypercube_iter(|idxs| {
        if idxs[0] == 7 && idxs[1] == 2 {
            H.slot_ring().one()
        } else {
            H.slot_ring().zero()
        }
