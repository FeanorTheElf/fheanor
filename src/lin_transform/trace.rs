...
fn cyclic_trace_norm_circuit<R>(
    ring: R,
    galois_group: &Subgroup<CyclotomicGaloisGroup>,
    generator: &GaloisGroupEl,
    l: usize,
    combiner: PlaintextCircuit<R::Type>,
    ...
    R: RingStore + Copy,
{
    ...
    let extend_circuit = RefCell::new(|l_idx: usize, r_idx: usize, l_num: i64| {
        take_mut::take(&mut circuit, |circuit| {
            PlaintextCircuit::identity(circuit.output_count(), ring)
                .tensor(
                    combiner.clone(ring).compose(
                        PlaintextCircuit::identity(1, ring).tensor(
                            PlaintextCircuit::gal(
                                galois_group.pow(generator, &int_cast(l_num, ZZbig, ZZi64)),
                                galois_group,
                                ring,
                            ),
                            ring,
...
fn cyclic_trace_circuit_window<R>(
    ring: R,
    galois_group: &Subgroup<CyclotomicGaloisGroup>,
    generator: &GaloisGroupEl,
    l: usize,
    ...
    R: RingStore + Copy,
{
    ...
    if start == 1 {
        ...
        circuit = PlaintextCircuit::add(ring)
            .compose(
                PlaintextCircuit::linear_transform(
                    &(0..window_size)
                        .map(|i| if i < next { Coefficient::One } else { Coefficient::Zero })
                        .collect::<Vec<_>>(),
                    ring,
                )
    ...
    for digit in l_decomp_it {
        let gs = (0..window_size)
            .map(|i| galois_group.pow(generator, &int_cast((i * current_step + digit) as i64, ZZbig, ZZi64)))
            .collect::<Vec<_>>();
        let multiplied =
            PlaintextCircuit::linear_transform(&(0..window_size).map(|_| Coefficient::One).collect::<Vec<_>>(), ring)
                .compose(PlaintextCircuit::gal_many(&gs, galois_group, ring), ring);
        ...
        circuit = PlaintextCircuit::add(ring)
            .compose(multiplied.tensor(additional, ring), ring)
            .tensor(
...
pub fn trace_circuit<R>(
    ring: R,
    relative_galois_group: &Subgroup<CyclotomicGaloisGroup>,
    ...
    R: RingStore + Copy,
{
    let cyclic_trace_circuit = |(g, l)| {
        ...
        let (window_size, cost) = (2..=l)
            .map(|w| {
                (
                    w,
                    (cost_model.cost_setup_hoisted_gal + cost_model.cost_hoisted_gal * w as f64) / (w as f64).log2(),
                )
            })
            .min_by(|(_, lc), (_, rc)| f64::total_cmp(lc, rc))
            .unwrap();
    ...
    relative_galois_group
        .get_group()
        .rectangular_form()
        .into_iter()
        .map(cyclic_trace_circuit)
        .fold(PlaintextCircuit::identity(1, ring), |current, next| {
            current.compose(next, ring)
        })
}
...
where
    G: FnMut(El<SlotRingOver<R>>) -> El<R>,
    R: RingStore,
    R::Type: NiceZn,
{
    ...
    slot_ring
        .base_ring()
        .solve_right(lhs.data_mut(), rhs.data_mut(), sol.data_mut())
        .assert_solved();
...
fn test_extract_coefficient_map() {
    ...
    let convolution = DynConvolutionAlgorithmConvolution::<ZnBase, Arc<dyn DynConvolutionAlgorithm<ZnBase>>>::new(
        Arc::new(STANDARD_CONVOLUTION),
    );
...
fn test_trace_circuit() {
    ...
    let relative_galois_group = full_galois_group
        .get_group()
        .clone()
        .subgroup([full_galois_group.from_representative(3)]);
    ...
    let relative_galois_group = full_galois_group
        .get_group()
        .clone()
        .subgroup([full_galois_group.from_representative(2)]);
    ...
    let actual = relative_trace
        .evaluate(std::slice::from_ref(&input), ring.identity())
        .pop()
        .unwrap();
    let expected = ring.sum([
        ring.canonical_gen(),
        ring.pow(ring.canonical_gen(), 2),
        ring.pow(ring.canonical_gen(), 4),
    ]);
    assert_el_eq!(&ring, expected, actual);

    let ring = NumberRingQuotientByIntBase::new(TensorProductNumberRing::new(5, 7), Zn::new(65537));
    let full_galois_group = ring.number_ring().galois_group();
    let trace = trace_circuit(
        &ring,
        &full_galois_group.get_group().clone().full_subgroup(),
        &DEFAULT_EVALUATOR_COSTS,
    );
    for i in 0..24 {
        let actual = trace
            .evaluate(&[ring.pow(ring.canonical_gen(), i)], ring.identity())
            .pop()
            .unwrap();
        assert_el_eq!(
            &ring,
            ring.inclusion().map(ring.trace(ring.pow(ring.canonical_gen(), i))),
            actual
        );
    }
}
...
fn test_cyclic_trace_circuit_window() {
    ...
    let expected: Program<i64> = Program::parse_check(
        r#"
        func(%in) {
    ...
    .unwrap();
    assert!(circuit.eq(
        &PlaintextCircuit::from_ir(ZZi64, Some(&galois_group), &expected),
        ZZi64,
        Some(&galois_group)
    ));
