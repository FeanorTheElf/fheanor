...
/// straightforward way (e.g. using [`MatmulTransform::matmul1d()`] and
/// [`MatmulTransform::blockmatmul1d()`]), and then compiled into circuit.
///
...
///
/// When a [`MatmulTransform`] is converted to [`PlaintextCircuit`], we use a baby-step-giant-step
/// method to create a circuit with a low number of galois gates. This requires a decomposition `T
/// <= T_baby * T_giant` of the set of Galois automorphisms `T` that are used by linear transform.
/// The size of `T_baby` and `T_giant` determine the performance, hence we want to choose both as
/// close to `sqrt(#T)` as possible (a limited amount of asymmetry is desired, since baby-step
/// automorphisms can be evaluated by a constant factor faster than giant-step automorphisms).
///
/// Unfortunately, such a decomposition is not easy to find for general sets of automorphisms `T`.
/// For this reason, a [`MatmulTransform`] stores not just the Galois automorphism, but also a
/// preimage of the Galois group element under the used [`HypercubeStructure`].
/// Approximating the resulting set of points of `Z^n` by a cubic grid
/// `X { b_i, b_i + s_i, b_i + 2 s_i, ..., b_i + l_i s_i }` then allows us to efficiently find a
/// suitable decomposition.
///
/// However, not all sets can be approximated well by such a cubic grid. Generally speaking, the
/// cubic grid performs well on [`MatmulTransform`]s generated by [`MatmulTransform::matmul1d()`] or
/// [`MatmulTransform::blockmatmul1d()`], which is the main setting we optimize for. If you use
/// [`MatmulTransform::linear_combine_shifts()`], note that the function takes shift vectors to
/// represent Galois automorphisms, and try to use integer vectors that can be approximated well by
/// a cube. Also note that when there are multiple shift vectors for the same Galois automorphism,
/// all shift vectors are replaced by a default choice, based on
/// [`HypercubeStructure::std_preimage_signed()`]. In most cases, this is a sensible choice, but
/// might discard carefully chosen representations.
///
    ...
    <<R as RingExtension>::BaseRing as RingStore>::Type: NiceZn,
{
        ...
        S: Copy + RingStore<Type = R>,
    {
        ...
        for (self_d, other_d) in self.data.iter().zip(other.data.iter()) {
            if !H
                .galois_group()
                .eq_el(&H.map_incl_frobenius(&self_d.0), &H.map_incl_frobenius(&other_d.0))
            {
                return false;
            }
    ...
    ///
    /// More concretely, it creates the linear transform that operates on each "slice" (i.e.
    /// hypercolumn) of the hypercube along the given dimension.
    ///
    /// # Details
    ///
    ...
    where
        G: Fn(usize, usize, &[usize]) -> El<SlotRingOf<S>>,
        S: RingStore<Type = R>,
    {
        ...
        let mut result = MatmulTransform {
            data: ((1 - m)..m)
                ...
                .filter_map(|s| {
                    let coeffs = H
                        .hypercube()
                        .hypercube_iter(|idxs: &[usize]| {
                            if idxs[dim_index] as i64 >= s && idxs[dim_index] as i64 - s < m {
                    ...
                    if coeffs
                        .iter()
                        .all(|x| x.is_none() || H.slot_ring().is_zero(x.as_ref().unwrap()))
                    {
                        None
                    } else {
                        Some((
                            [0].into_iter()
                                .chain((0..H.hypercube().dim_count()).map(|j| if j == dim_index { s } else { 0 }))
                                .collect::<Vec<_>>()
                                .into_boxed_slice(),
    ...
    fn slot_ring_frobenius<'a, S>(
        H: &'a HypercubeIsomorphism<S>,
    ) -> Arc<dyn 'a + Fn(&El<SlotRingOf<S>>, usize) -> El<SlotRingOf<S>>>
    where
        S: 'a + RingStore<Type = R>,
        R: 'a,
    {
        let d = H.hypercube().d();
        ...
        for _ in 0..d {
            generator_frobenius_conjugates.push(
                (0..d)
                    .map(|j| H.slot_ring().pow(H.slot_ring().clone_el(&current), j))
                    .collect::<Vec<_>>(),
            );
            current = H
                .slot_ring()
                .pow(current, H.galois_group().representative(H.hypercube().p()) as usize);
        }

        #[instrument(skip_all)]
        fn apply_frobenius<S>(
            generator_frobenius_conjugates: &Vec<Vec<El<SlotRingOf<S>>>>,
            slot_ring: &SlotRingOf<S>,
            d: usize,
            x: &El<SlotRingOf<S>>,
            count: usize,
        ) -> El<SlotRingOf<S>>
            ...
            <<S::Type as RingExtension>::BaseRing as RingStore>::Type: NiceZn,
        {
            ...
            for i in 0..d {
                result = slot_ring.inclusion().fma_map(
                    &generator_frobenius_conjugates[count][i],
                    &x_wrt_basis.at(i),
                    result,
                );
            }
        ...
        // this is the map `X -> X^p`, which is the frobenius in our case, since we choose the canonical
        // generator of the slot ring as root of unity
        return Arc::new(move |x: &El<SlotRingOf<S>>, count: usize| {
            apply_frobenius::<S>(&generator_frobenius_conjugates, H.slot_ring(), d, x, count)
        });
    }
        ...
        S: RingStore<Type = R>,
    {
        ...
        let extract_coeff_factors = (0..d)
            .map(|j| extract_linear_map(H.slot_ring(), |x| H.slot_ring().wrt_canonical_basis(&x).at(j)))
            .collect::<Vec<_>>();

        let frobenius = Self::slot_ring_frobenius(H);

        let extract_coeff_factor_conjugates = extract_coeff_factors
            .iter()
            .map(|c| (0..d).map(|i| frobenius(c, i)).collect::<Vec<_>>())
            .collect::<Vec<_>>();
            ...
            data: (0..d)
                .filter_map(|frobenius_index| {
                    ...
                    if coeffs.iter().all(|x| H.slot_ring().is_zero(x)) {
                        ...
                        Some((
                            [frobenius_index as i64]
                                .into_iter()
                                .chain((0..H.hypercube().dim_count()).map(|_| 0))
                                .collect::<Vec<_>>()
                                .into_boxed_slice(),
        ...
        S: RingStore<Type = R>,
    {
        Self {
            data: self
                .data
                .iter()
                .map(|(idx, coeff)| (idx.clone(), ring.clone_el(coeff)))
                .collect(),
        }
    }
    ...
    where
        G: Fn(usize, usize, &[usize]) -> El<R::BaseRing>,
        S: RingStore<Type = R>,
    {
        ...
        S: RingStore<Type = R>,
    {
        ...
        Self::blockmatmul0d_internal(H, |idxs| {
            ...
            S.base_ring()
                .solve_right(
                    matrix.data_mut(),
                    OwnedMatrix::identity(d, d, S.base_ring()).data_mut(),
                    inv.data_mut(),
                )
                .assert_solved();
    ...
    ///
    /// More concretely, it creates the linear transform that operates on each "slice" of the
    /// hypercube along the given dimension.
    ///
    /// # Details
    ///
    /// For each hypercolumn along the `dim_index`-th dimension containing the slots of index
    /// `U(i) = (u1, ..., u(dim_index - 1), i, u(dim_index + 1), ..., ur)` for all `i`,
    /// we can consider the `Fp`-basis given by `𝝵^k e_U(i)`. An `Fp`-linear transform that operates
    /// on each set `{ 𝝵^k e_U(i) | k, i }` separately, for all `U`, is said to be of
    /// `blockmatmul1d`-type, and can be created using this function.
    ///
    ...
    where
        G: Fn((usize, usize), (usize, usize), &[usize]) -> El<R::BaseRing>,
        S: RingStore<Type = R>,
    {
        let m = H.hypercube().dim_length(dim_index) as i64;
        let d = H.slot_ring().rank();
        ...
        // We consider the matrix by block-diagonals as in [`matmul1d()`], which correspond to shifting
        // slots within a hypercolumn. Additionally however, we need to take care of the
        // transformation within a slot. Unfortunately, the matrix structure does not nicely
        // correspond to structure of the Frobenius anymore (more concretely, the basis `1, 𝝵, ..., 𝝵^(d -
        // 1)` w.r.t. which we represent the matrix is not normal). Thus, we have to solve a linear
        // system, which is done by `extract_linear_map()`. In other words, we compute the
        // Frobenius-coefficients for the maps `sum a_k 𝝵^k -> a_l` for all `l`. Then we we take the
        // desired map as the linear combination of these extract-coefficient-maps.
        let mut result = MatmulTransform {
            data: ((1 - m)..m)
                .flat_map(|s| (0..d).map(move |frobenius_index| (s, frobenius_index)))
                .filter_map(|(s, frobenius_index)| {
                    let coeffs = H
                        .hypercube()
                        .hypercube_iter(|idxs| {
                            if idxs[dim_index] as i64 >= s && idxs[dim_index] as i64 - s < m {
                                ...
                                <_ as ComputeInnerProduct>::inner_product_ref_fst(
                                    ...
                                    (0..d).map(|l| {
                                        (
                                            &extract_coeff_factor_conjugates[l][frobenius_index],
                                            H.slot_ring()
                                                .from_canonical_basis((0..d).map(|k| matrix((i, k), (j, l), idxs))),
                                        )
                                    }),
                                )
                            } else {
        ...
        H: Homomorphism<R, S>,
    {
        MatmulTransform::<S> {
            data: self
                .data
                .iter()
                .map(|(g, coeff)| (g.clone(), hom.map_ref(coeff)))
                .collect::<Vec<(_, S::Element)>>(),
        }
    }
        ...
        S: RingStore<Type = R>,
    {
        ...
        let inverse_automorphisms = original_automorphisms
            .clone()
            .map(|g| g.iter().map(|i| -i).collect::<Vec<_>>().into_boxed_slice())
            .collect::<Vec<_>>();

        let mut composed_automorphisms = original_automorphisms
            .clone()
            .flat_map(|g| {
                inverse_automorphisms.iter().map(move |s| {
                    g.iter()
                        .zip(s.iter())
                        .map(|(i1, i2)| i1 + i2)
                        .collect::<Vec<_>>()
        ...
        composed_automorphisms.dedup_by(|a, b| {
            Gal.eq_el(
                &H.hypercube().map_incl_frobenius(a),
                &H.hypercube().map_incl_frobenius(b),
            )
        });

        let mut matrix: OwnedMatrix<_> =
            OwnedMatrix::zero(composed_automorphisms.len(), inverse_automorphisms.len(), H.ring());
        for (i, g) in original_automorphisms.enumerate() {
            for (j, s) in inverse_automorphisms.iter().enumerate() {
                let row_index =
                    composed_automorphisms
                        .binary_search_by_key(
                            &Gal.representative(&H.hypercube().map_incl_frobenius(
                                &g.iter().zip(s.iter()).map(|(i1, i2)| i1 + i2).collect::<Vec<_>>(),
                            )),
                ...
                let entry = H
                    .ring()
                    .get_ring()
                    .apply_galois_action(&self.data[i].1, &H.hypercube().map_incl_frobenius(s));
                *matrix.at_mut(row_index, j) = entry;
            }
        }

        let mut matrix_by_slots = (0..matrix.row_count())
            .map(|i| {
                (0..matrix.col_count())
                    .map(|j| H.get_slot_values(matrix.at(i, j)))
                    .collect::<Vec<_>>()
            })
            .collect::<Vec<_>>();
        ...
        for _ in H.hypercube().element_iter() {
            ...
            *rhs.at_mut(0, 0) = H.slot_ring().one();
            ...
            H.slot_ring()
                .get_ring()
                .solve_right(lhs.data_mut(), rhs.data_mut(), sol.data_mut(), Global)
                .assert_solved();
            for j in 0..matrix.col_count() {
        ...
        let result = result_by_slots
            .into_iter()
            .map(|coeff_by_slots| H.from_slot_values(coeff_by_slots.into_iter()))
            .collect::<Vec<_>>();
        ...
        let mut result = Self {
            data: self
                ...
                .map(|((g, _), coeff)| (g.iter().map(|i| -i).collect::<Vec<_>>().into_boxed_slice(), coeff))
                .collect(),
        };
        result.canonicalize(H.ring(), H.hypercube());
        ...
        S: RingStore<Type = R>,
    {
        ...
        assert!(
            self.data
                .is_sorted_by_key(|(g, _)| Gal.representative(&H.map_incl_frobenius(g)))
        );
        ...
        S: Copy + RingStore<Type = R>,
    {
        ...
        let mut result = Self {
            data: self
                .data
                .iter()
                .flat_map(|(self_g, self_coeff)| {
                    run_first.data.iter().map(|(first_g, first_coeff)| {
                        (
                            self_g
                                .iter()
                                .zip(first_g.iter())
                            ...
                            ring.mul_ref_snd(
                                ring.get_ring()
                                    .apply_galois_action(first_coeff, &H.map_incl_frobenius(self_g)),
                                self_coeff,
                            ),
                        )
                    })
        ...
        debug_assert_eq!(
            self.composed_automorphism_count(H, run_first),
    ...
    fn composed_automorphism_count(&self, H: &HypercubeStructure, run_first: &MatmulTransform<R>) -> usize {
        ...
        let mut result_automorphisms = self
            .data
            .iter()
            .flat_map(|(self_g, _)| {
                run_first.data.iter().map(|(first_g, _)| {
    ...
    pub fn automorphism_count(&self) -> usize { self.data.len() }
    ...
    where
        S: RingStore<Type = R>,
    {
        Self::mult_ring_element(
            H.ring(),
            H.hypercube(),
            &H.from_slot_values((0..H.slot_count()).map(|_| H.slot_ring().clone_el(scalar))),
        )
    }
        ...
        S: RingStore<Type = R>,
    {
        let mut result = Self {
            data: summands
                ...
                .map(|(positions, factor)| (positions.collect::<Vec<_>>().into_boxed_slice(), factor))
                .collect(),
        };
        result.canonicalize(H.ring(), H.hypercube());
        return result;
    ...
    {
        self.data
            .sort_unstable_by_key(|(g, _)| H.galois_group().representative(&H.map_incl_frobenius(g)));
        let mut normalize_all_shifts = false;
        self.data.dedup_by(|second, first| {
            if H.galois_group()
                .eq_el(&H.map_incl_frobenius(&second.0), &H.map_incl_frobenius(&first.0))
            {
        ...
        if normalize_all_shifts {
            ...
            for (g, _) in self.data.iter_mut() {
                *g = H
                    .std_preimage_signed(&H.map_incl_frobenius(g))
                    .iter()
                    .map(|i| *i as i64)
                    .collect::<Vec<_>>()
                    .into_boxed_slice();
        ...
        // This takes significant time; make the parent call responsible for not introducing too
        // much zeros self.data.retain(|(_, coeff)| !ring.is_zero(coeff));
    }

    /// Computes `max_step, min_step, gcd_step, sizes` such that all automorphisms are in the
    /// hypercube `X { min_step[i], min_step[i] + gcd_step[i], ..., max_step[i] }`.
    ///
    /// The last list `sizes` contains the number of elements along each dimension. This is correct
    /// even if `gcd_step[i] == usize::MAX`, we is set instead of `0` if there is only one entry
    /// in dimension `i` (i.e. `min_step[i] = max_step[i]`), since this makes using it via `step_by`
    /// easier.
    ///
    ...
    fn compute_automorphisms_per_dimension<S>(
        &self,
        ring: S,
        ...
        S: Copy + RingStore<Type = R>,
    {
        ...
        for i in 0..=H.dim_count() {
            ...
            let gcd = self
                ...
                .iter()
                .map(|(g, _)| g[i])
                .fold(0, |a, b| signed_gcd(a, b, StaticRing::<i64>::RING))
                .try_into()
                .unwrap();
            if gcd == 0 {
            ...
            if gcd_step[i] != usize::MAX {
                sizes.push(
                    StaticRing::<i64>::RING
                        .checked_div(
                            &(max_step[i] - min_step[i] + gcd_step[i] as i64),
                            &gcd_step[i].try_into().unwrap(),
                        )
                        .unwrap()
                        .try_into()
                        .unwrap(),
        ...
        return (max_step, min_step, gcd_step, sizes);
    }

    #[instrument(skip_all)]
    pub fn baby_step_giant_step_params<V>(
        automorphisms_per_dim: V,
        preferred_baby_steps: usize,
    ) -> BabyStepGiantStepParams
    where
        V: VectorFn<usize>,
    {
        ...
        let giant_steps = (giant_step_start_dim..automorphisms_per_dim.len())
            .map(|i| automorphisms_per_dim.at(i))
            .product::<usize>()
            * ((automorphisms_per_dim.at(mixed_dim_i) - 1) / mixed_dim_baby_steps + 1);
        ...
        S: Copy + RingStore<Type = R>,
    {
        ...
        let preferred_baby_steps = (1..=(sizes.iter().copied().product::<usize>()))
            .map(|preferred_baby_steps| {
    ...
    #[instrument(skip_all)]
    pub fn to_circuit_many<S>(
        ring: S,
        H: &HypercubeStructure,
        mut transforms: Vec<Self>,
        max_levels: usize,
        cost_model: &CircuitEvaluatorCosts,
    ) -> PlaintextCircuit<R>
    where
        S: Copy + RingStore<Type = R>,
    {
        ...
        while transforms.len() > max_levels {
            let delta_complexity = |[t1, t2]: &[MatmulTransform<_>; 2]| {
                (t2.composed_automorphism_count(H, t1) as f64).sqrt()
                    - (t1.automorphism_count() as f64).sqrt()
                    - (t2.automorphism_count() as f64).sqrt()
            };
            let merge_idx = transforms
                .array_windows::<2>()
                .enumerate()
                .min_by(|(_, ts1), (_, ts2)| f64::total_cmp(&delta_complexity(ts1), &delta_complexity(ts2)))
                .unwrap()
                .0;
    ...
    /// Requires that `self` is defined w.r.t. the given ring and [`HypercubeStructure`].
    #[instrument(skip_all)]
    pub fn to_circuit_with_baby_steps<S>(
        self,
        ring: S,
        H: &HypercubeStructure,
        preferred_baby_steps: usize,
        ...
        S: Copy + RingStore<Type = R>,
    {
        ...
        let mixed_dim_steps_on_top = StaticRing::<i64>::RING
            .checked_div(&max_step[mixed_dim_i], &gcd_step[mixed_dim_i].try_into().unwrap_or(0))
            .unwrap();
        let mixed_dim_steps_on_bottom = StaticRing::<i64>::RING
            .checked_div(&min_step[mixed_dim_i], &gcd_step[mixed_dim_i].try_into().unwrap_or(0))
            .unwrap();
        ...
        let mixed_dim_giant_steps_on_top = StaticRing::<i64>::RING
            .checked_div(
                &(mixed_dim_steps_on_top - mixed_dim_baby_steps_on_top),
                &(mixed_dim_baby_steps as i64),
            )
            .unwrap();
        let mixed_dim_giant_steps_on_bottom =
            (mixed_dim_steps_on_bottom - mixed_dim_baby_steps_on_bottom).div_floor(mixed_dim_baby_steps as i64);
        let mixed_dim_giant_step_range =
            ((mixed_dim_giant_steps_on_bottom * mixed_dim_baby_steps as i64 * gcd_step[mixed_dim_i] as i64)
                ..=(mixed_dim_giant_steps_on_top * mixed_dim_baby_steps as i64 * gcd_step[mixed_dim_i] as i64))
                .step_by(mixed_dim_baby_steps * gcd_step[mixed_dim_i]);
        let mixed_dim_baby_step_range = ((mixed_dim_baby_steps_on_bottom * gcd_step[mixed_dim_i] as i64)
            ..=(mixed_dim_baby_steps_on_top * gcd_step[mixed_dim_i] as i64))
            .step_by(gcd_step[mixed_dim_i]);
        debug_assert!(
            mixed_dim_baby_steps_on_top + mixed_dim_giant_steps_on_top * mixed_dim_baby_steps as i64
                == mixed_dim_steps_on_top
        );
        debug_assert!(
            mixed_dim_baby_steps_on_bottom + mixed_dim_giant_steps_on_bottom * mixed_dim_baby_steps as i64
                <= mixed_dim_steps_on_bottom
        );

        let giant_step_range_iters = [mixed_dim_giant_step_range].into_iter().chain(
            params
                .pure_giant_step_dimensions
                .clone()
                .map(|i| (min_step[i]..=max_step[i]).step_by(gcd_step[i])),
        );

        let baby_step_range_iters = params
            .pure_baby_step_dimensions
            .clone()
            .map(|i| (min_step[i]..=max_step[i]).step_by(gcd_step[i]))
            .chain([mixed_dim_baby_step_range]);

        let shift_or_frobenius = |dim_or_frobenius: usize, steps: i64| {
            if dim_or_frobenius == 0 {
                H.frobenius(steps)
            } else {
        ...
        let giant_steps_galois_els = multi_cartesian_product(
            giant_step_range_iters,
            |indices| {
                indices[1..]
                    .iter()
                    .enumerate()
                    .map(|(i, s)| shift_or_frobenius(i + params.pure_giant_step_dimensions.start, *s))
                    .fold(shift_or_frobenius(mixed_dim_i, indices[0]), |a, b| {
                        H.galois_group().op(a, b)
                    })
            },
            |_, x| *x,
        )
        .map(|g_el| {
            if H.galois_group().is_identity(&g_el) {
                None
            } else {
                Some(g_el)
            }
        ...
        let baby_steps_galois_els = multi_cartesian_product(
            baby_step_range_iters,
            move |indices| {
                indices
                    .iter()
                    .enumerate()
        ...
        debug_assert!(
            params.unhoisted_automorphism_count == giant_steps_galois_els.len() - 1
                || params.unhoisted_automorphism_count == giant_steps_galois_els.len()
        );

        let mut lin_transform_data = self.data;
        let compiled_coeffs: Vec<Vec<Coefficient<_>>> = giant_steps_galois_els
            .iter()
            .map(|gs_el| {
                baby_steps_galois_els
                    .iter()
                    .map(|bs_el| {
                        let gs_el = gs_el.clone().unwrap_or(H.galois_group().identity());
                        let total_el = H.galois_group().op_ref(&gs_el, bs_el);
                        let mut coeff = None;
                        lin_transform_data.retain(|(g, c)| {
                            if H.galois_group().eq_el(&H.map_incl_frobenius(g), &total_el) {
                        ...
                        if coeff.is_none() {
                            ...
                            return Coefficient::Other(
                                ring.get_ring()
                                    .apply_galois_action(coeff.as_ref().unwrap(), &H.galois_group().inv(&gs_el)),
                            );
                        }
                    })
                    .collect::<Vec<_>>()
        ...
        for (g, coeffs) in giant_steps_galois_els.iter().cloned().zip(compiled_coeffs) {
            ...
            let summand = if let Some(g) = g {
                let galois_of_lin_transform = PlaintextCircuit::gal(g, &H.galois_group(), ring)
                    .compose(PlaintextCircuit::linear_transform(&coeffs, ring), ring);
            ...
            current = summand
                .tensor(PlaintextCircuit::drop(1), ring)
                .tensor(PlaintextCircuit::identity(baby_steps_galois_els.len(), ring), ring)
                .compose(current.output_twice(ring), ring);
            debug_assert_eq!(baby_steps_galois_els.len() + 1, current.output_count());
        }

        return PlaintextCircuit::identity(1, ring)
            .tensor(PlaintextCircuit::drop(baby_steps_galois_els.len()), ring)
...
{
    #[instrument(skip_all)]
    pub fn compute_linear_transform(
        &self,
        H: &HypercubeStructure,
        el: &<Self as RingBase>::Element,
        transform: &MatmulTransform<Self>,
        ...
        <_ as RingBase>::sum(
            ...
            transform
                .data
                .iter()
                .map(|(s, c)| self.mul_ref_fst(c, self.apply_galois_action(el, &H.map_incl_frobenius(s)))),
        )
    }
}
...
fn test_to_circuit_single() {
    ...
    let actual = compiled_transform
        .evaluate(from_ref(&input), ring.identity())
        .pop()
        .unwrap();
    assert_el_eq!(&ring, &input, &actual);

    let transform = MatmulTransform::blockmatmul1d(&H, 0, |(i, k), (j, l), _| {
        if j == i + 1 && k == 0 && l == 0 {
    ...
    let transform = MatmulTransform::linear_combine_shifts(
        &H,
        [
            (
                [-1],
                H.from_slot_values([1, 1, 1, 0].into_iter().map(|x| H.slot_ring().int_hom().map(x))),
            ),
            (
                [0],
                H.from_slot_values([2, 2, 2, 2].into_iter().map(|x| H.slot_ring().int_hom().map(x))),
            ),
            (
                [1],
                H.from_slot_values([0, 3, 3, 3].into_iter().map(|x| H.slot_ring().int_hom().map(x))),
            ),
        ]
        .into_iter(),
    );
...
fn test_to_circuit_many() {
    ...
    impl<'a, R: ?Sized + RingBase> CircuitEvaluator<'a, usize, R> for MulDepthEvaluator {
        ...
        where
            I: Iterator<Item = (&'a Coefficient<R>, &'b usize)>,
            R: 'a,
            usize: 'b,
        {
    ...
    let H = HypercubeIsomorphism::new(&ring, &hypercube, None);
    let perm = [0, 2, 1, 3];
    let permutation = MatmulTransform::matmul1d(&H, 0, |i, j, _| {
        if i == perm[j] {
            H.slot_ring().one()
        } else {
            H.slot_ring().zero()
    ...
    let transform = MatmulTransform::to_circuit_many(
        &ring,
        H.hypercube(),
        vec![
    ...
    let transform = MatmulTransform::to_circuit_many(
        ...
        H.hypercube(),
        vec![
            MatmulTransform::blockmatmul0d(&H, |i, j, _| {
                if i == 0 && j == 0 {
                    ring.base_ring().one()
                } else {
                    ring.base_ring().zero()
    ...
    let transform = MatmulTransform::to_circuit_many(
        ...
        H.hypercube(),
        vec![
            MatmulTransform::linear_combine_shifts(
                &H,
                [([0, 0], ring.int_hom().map(1)), ([1, 0], ring.int_hom().map(2))],
            ),
            MatmulTransform::linear_combine_shifts(
                &H,
                [([0, 0], ring.int_hom().map(3)), ([2, 0], ring.int_hom().map(4))],
            ),
            MatmulTransform::linear_combine_shifts(
                &H,
                [([0, 0], ring.int_hom().map(5)), ([4, 0], ring.int_hom().map(6))],
            ),
            MatmulTransform::linear_combine_shifts(
                &H,
                [([0, 0], ring.int_hom().map(7)), ([8, 0], ring.int_hom().map(8))],
            ),
        ],
        2,
        &DEFAULT_EVALUATOR_COSTS,
...
fn test_compose() {
    ...
    let transform1 = MatmulTransform::matmul1d(&H, 0, |i, j, _| {
        if i == j + 1 {
            H.slot_ring().one()
        } else {
            H.slot_ring().zero()
    ...
    let transform2 = MatmulTransform::matmul1d(&H, 0, |i, j, _| {
        if i + 1 == j {
            H.slot_ring().one()
        } else {
            H.slot_ring().zero()
    ...
    let actual = ring
        .get_ring()
        .compute_linear_transform(H.hypercube(), &input, &composed_transform);
    assert_el_eq!(&ring, &expected, &actual);
    ...
    let transform =
        MatmulTransform::linear_combine_shifts(&H, [([-1, 0], ring.one()), ([0, 1], ring.int_hom().map(2))]);
    ...
    let expected = ring.get_ring().compute_linear_transform(
        H.hypercube(),
        &ring
            .get_ring()
            .compute_linear_transform(H.hypercube(), &input, &transform),
        &transform,
    );
    let actual = ring
        .get_ring()
...
fn test_invert() {
    ...
    let transform = MatmulTransform::matmul1d(&H, 0, |i, j, _| {
        H.slot_ring()
            .int_hom()
            .map(StaticRing::<i32>::RING.pow(i as i32 + 1, j))
    });
    ...
    let actual = ring
        .get_ring()
        .compute_linear_transform(H.hypercube(), &input, &inverse_transform);
...
fn test_blockmatmul1d() {
    ...
    let H = HypercubeIsomorphism::new(&ring, &hypercube, None);
    let matrix = [[1, 0, 1, 0], [0, 0, 0, 2], [0, 0, 0, 0], [5, 0, 8, 8]];
    ...
    for i in 0..4 {
        ...
        let expected = H
            .ring()
            .from_canonical_basis((0..4).map(|j| H.ring().base_ring().int_hom().map(matrix[j][i])));
        let actual = ring
            .get_ring()
            .compute_linear_transform(H.hypercube(), &input, &lin_transform);
    ...
    for i in 0..3 {
        let input = H.from_slot_values([
            H.slot_ring().zero(),
            H.slot_ring().pow(H.slot_ring().canonical_gen(), i),
        ]);
        let expected = H.from_slot_values([
            H.slot_ring()
                .from_canonical_basis((0..3).map(|j| H.slot_ring().base_ring().int_hom().map(matrix[j][i]))),
            H.slot_ring().zero(),
        ]);
        let actual = ring
            .get_ring()
