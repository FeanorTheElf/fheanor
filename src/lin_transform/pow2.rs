...
/// Works separately on each block of size `l = blocksize` along the given given hypercube
/// dimension. This function computes the length-`l` DWT
/// ```text
...
///
/// # Notes
///  - `row_autos` can be given to use different `𝝵`s for each block; in particular, for the block
///    with hypercube indices `idxs`, the DWT with root of unity `𝝵 = root_of_unity^row_autos(idxs)`
///    is used. Note that the index passed to `row_autos` is the hypercube index of some element in
///    the block. It does not make sense for `row_autos` to behave differently on different indices
///    in the same block, this will lead to `pow2_bitreversed_dwt_butterfly` give nonsensical
///    results. If you pass `row_autos = |_| H.galois_group().one()` then this uses the same roots
///    of unity everywhere, i.e. results in the behavior as outlined above.
fn pow2_bitreversed_dwt_butterfly<G, R>(
    H: &HypercubeIsomorphism<R>,
    dim_index: usize,
    l: usize,
    root_of_unity: El<SlotRingOf<R>>,
    row_autos: G,
) -> MatmulTransform<R::Type>
where
    G: Fn(&[usize]) -> GaloisGroupEl,
    R: RingStore,
    ...
    BaseRing<R>: NiceZn,
{
    ...
    assert!(H.slot_ring().eq_el(
        &root_of_unity,
        &H.slot_ring().negate(H.slot_ring().pow(
            H.slot_ring().clone_el(&root_of_unity),
            Zm.smallest_positive_lift(Zm.pow(*Gal.as_ring_el(&g), l / 2)) as usize
        ))
    ));
    ...
    let pow_of_zeta = |factor: TwiddleFactor| match factor {
        TwiddleFactor::PosPowerZeta(pow) => H.slot_ring().pow(
            H.slot_ring().clone_el(&root_of_unity),
            Zm.smallest_positive_lift(pow) as usize,
        ),
        TwiddleFactor::NegPowerZeta(pow) => H.slot_ring().negate(H.slot_ring().pow(
            H.slot_ring().clone_el(&root_of_unity),
            Zm.smallest_positive_lift(pow) as usize,
        )),
    ...
    let forward_mask = H.from_slot_values(
        H.hypercube()
            .hypercube_iter(|idxs| {
    ...
    let diagonal_mask = H.from_slot_values(
        H.hypercube()
            .hypercube_iter(|idxs| {
                let idx_in_block = idxs[dim_index] % l;
                if idx_in_block >= l / 2 {
                    TwiddleFactor::NegPowerZeta(Zm.mul(
                        Zm.pow(*Gal.as_ring_el(&g), idx_in_block - l / 2),
                        *Gal.as_ring_el(&row_autos(&idxs)),
                    ))
                } else {
                    TwiddleFactor::PosPowerZeta(Gal.underlying_ring().zero())
                }
    ...
    let backward_mask = H.from_slot_values(
        H.hypercube()
            .hypercube_iter(|idxs| {
                let idx_in_block = idxs[dim_index] % l;
                if idx_in_block < l / 2 {
                    TwiddleFactor::PosPowerZeta(Zm.mul(
                        Zm.pow(*Gal.as_ring_el(&g), idx_in_block),
                        *Gal.as_ring_el(&row_autos(&idxs)),
                    ))
                } else {
                    TwiddleFactor::Zero
    ...
    let result = MatmulTransform::linear_combine_shifts(
        H,
        [
            (
            ...
            (
                (0..H.hypercube().dim_count())
                    .map(|i| if i == dim_index { l as i64 / 2 } else { 0 })
                    .collect::<Vec<_>>(),
                forward_mask,
            ),
            (
...
fn pow2_bitreversed_inv_dwt_butterfly<G, R>(
    H: &HypercubeIsomorphism<R>,
    dim_index: usize,
    l: usize,
    root_of_unity: El<SlotRingOf<R>>,
    ...
    BaseRing<R>: NiceZn,
{
    ...
    let inv_2 = H
        .ring()
        .base_ring()
        .invert(&H.ring().base_ring().int_hom().map(2))
        .unwrap();

    let mut forward_mask = H.from_slot_values(
        H.hypercube()
            .hypercube_iter(|idxs| {
                let idx_in_block = idxs[dim_index] % l;
                if idx_in_block >= l / 2 {
                    TwiddleFactor::PosPowerZeta(Zm.mul(
                        Zm.negate(Zm.pow(*Gal.as_ring_el(&g), idx_in_block - l / 2)),
                        *Gal.as_ring_el(&row_autos(&idxs)),
                    ))
                } else {
                    TwiddleFactor::Zero
    ...
    let mut diagonal_mask = H.from_slot_values(
        H.hypercube()
            .hypercube_iter(|idxs| {
                let idx_in_block = idxs[dim_index] % l;
                if idx_in_block >= l / 2 {
    ...
    let mut backward_mask = H.from_slot_values(
        H.hypercube()
            .hypercube_iter(|idxs| {
                let idx_in_block = idxs[dim_index] % l;
                if idx_in_block < l / 2 {
...
#[instrument(skip_all)]
fn pow2_bitreversed_dwt<G, R>(
    H: &HypercubeIsomorphism<R>,
    dim_index: usize,
    row_autos: G,
) -> Vec<MatmulTransform<R::Type>>
where
    G: Fn(&[usize]) -> GaloisGroupEl,
    R: RingStore,
    R::Type: Sized + NumberRingQuotient,
    BaseRing<R>: NiceZn,
{
    ...
    let zeta = H.slot_ring().pow(
        H.slot_ring().canonical_gen(),
        H.hypercube().ord_generator(dim_index) / dim_len,
    );
    ...
    for log2_l in 1..=log2_len {
        result.push(pow2_bitreversed_dwt_butterfly(
            ...
            1 << log2_l,
            H.slot_ring()
                .pow(H.slot_ring().clone_el(&zeta), dim_len / (1 << log2_l)),
            &row_autos,
        ));
    }
...
#[instrument(skip_all)]
fn pow2_bitreversed_inv_dwt<G, R>(
    H: &HypercubeIsomorphism<R>,
    dim_index: usize,
    row_autos: G,
) -> Vec<MatmulTransform<R::Type>>
...
///
/// In the case `p = 1 mod 4`, the slots are enumerated by `i, j` with `0 <= i < l` and `j in {0,
/// 1}`. Then the returned linear transform will then put the value of slot `(i, 0)` into the
/// coefficient of `X^(bitrev(i, l) * m/(4l))` and the value of slot `(i, 1)` into the coefficient
/// of `X^(bitrev(i, l) * m/(4l) + m/4)`.
///
/// If `p = 3 mod 4`, the slots are enumerated by `i` with `0 <= i < l` and the transform will put
/// the value of slot `i` into the coefficient of `X^(bitrev(i, l) * m/(4l))`
#[instrument(skip_all)]
pub fn slots_to_coeffs_thin<R>(
    H: &HypercubeIsomorphism<R>,
    max_levels: usize,
...
where
    ...
    BaseRing<R>: NiceZn,
{
    MatmulTransform::to_circuit_many(H.ring(), H.hypercube(), slots_to_coeffs_base(H), max_levels, cost_model)
}
    ...
    BaseRing<R>: NiceZn,
{
    ...
    if H.hypercube().dim_count() == 2 {
        ...
        let root_of_unity = H
            .slot_ring()
            .pow(H.slot_ring().canonical_gen(), H.hypercube().ord_generator(0));
        ...
        result.push(MatmulTransform::linear_combine_shifts(
            H,
            [
                (
                    (0..H.hypercube().dim_count()).map(|_| 0).collect::<Vec<_>>(),
                    H.from_slot_values(H.hypercube().hypercube_iter(|idxs| {
                        if idxs[1] == 0 {
                            H.slot_ring().one()
                        } else {
        ...
        result.extend(pow2_bitreversed_dwt(H, 0, |idxs| {
            if idxs[1] == 0 {
...
///
/// In the case `p = 1 mod 4`, the slots are enumerated by `i, j` with `0 <= i < l` and `j in {0,
/// 1}`. Then the returned linear transform will then put the coefficient of `𝝵^k` in slot `(i, 0)`
/// into the coefficient of `X^(bitrev(i, l) * m/(4l) + k)` and the coefficient of `𝝵^k` in slot
/// `(i, 1)` into the coefficient of `X^(bitrev(i, l) * m/(4l) + m/4 + k)`.
///
/// If `p = 3 mod 4`, the slots are enumerated by `i` with `0 <= i < l` and the transform will put
/// the coefficient of `𝝵^k` in slot `i` into the coefficient of `X^(bitrev(i, l) * m/(4l) + k)` if
/// `k < d/2` and into the coefficient of `X^(bitrev(i, l) * m/(4l) + m/4 + k - d/2)` if `k >= d/2`.
#[instrument(skip_all)]
pub fn slots_to_coeffs_fat<R>(
    H: &HypercubeIsomorphism<R>,
    max_levels: usize,
    cost_model: &CircuitEvaluatorCosts,
) -> PlaintextCircuit<R::Type>
where
    ...
    BaseRing<R>: NiceZn,
{
    MatmulTransform::to_circuit_many(
        H.ring(),
        H.hypercube(),
        slots_to_coeffs_fat_impl(H),
        max_levels,
        cost_model,
    )
}
...
///
/// In the case `p = 1 mod 4`, the slots are enumerated by `i, j` with `0 <= i < l` and `j in {0,
/// 1}`. Then the returned linear transform will then put the slot `(i, 0)` of the `bitrev(k, d)`-th
/// input into the coefficient coefficient of `X^(bitrev(i, l) * m/(4l) + k)` and slot `(i, 1)` of
/// the `bitrev(k, d)`-th input into the coefficient of `X^(bitrev(i, l) * m/(4l) + m/4 + k)`.
///
/// If `p = 3 mod 4`, the slots are enumerated by `i` with `0 <= i < l` and the transform will put
/// slot `i` of the `bitrev(k, d)`-th input into the coefficient of `X^(bitrev(i, l) * m/(4l) + k)`
/// and slot `i` of the `bitrev(k + d/2, d)`-th into the coefficient of `X^(bitrev(i, l) * m/(4l) +
/// m/4 + k)`, where `0 <= k < d/2`.
#[instrument(skip_all)]
pub fn slots_to_coeffs_fat_pack<R>(
    H: &HypercubeIsomorphism<R>,
    max_levels: usize,
    cost_model: &CircuitEvaluatorCosts,
) -> PlaintextCircuit<R::Type>
    ...
    BaseRing<R>: NiceZn,
{
    ...
    if H.hypercube().dim_count() == 2 {
        // this is the `p = 1 mod 4` case
        assert_eq!(2, H.hypercube().dim_length(1));
        result = PlaintextCircuit::linear_transform(
            &[
                Coefficient::One,
                Coefficient::Other(ring.pow(ring.canonical_gen(), d / 2)),
            ],
            ring,
        )
        .tensor_pow(d / 2, ring)
        ...
        result = PlaintextCircuit::linear_transform(
            &[
                Coefficient::One,
                Coefficient::Other(ring.pow(ring.canonical_gen(), m / 4)),
            ],
            ring,
        )
        .tensor_pow(d / 2, ring)
    ...
    for i in 2..=log2_d {
        result = PlaintextCircuit::linear_transform(
            &[
                Coefficient::One,
                Coefficient::Other(ring.pow(ring.canonical_gen(), d >> i)),
            ],
            ring,
        )
        .tensor_pow(d >> i, ring)
    ...
    result = MatmulTransform::to_circuit_many(
        ring,
        H.hypercube(),
        slots_to_coeffs_fat_Xbasis(H),
        max_levels,
        cost_model,
    )
    .compose(result, ring);
...
///
/// In the case `p = 1 mod 4`, the slots are enumerated by `i, j` with `0 <= i < l` and `j in {0,
/// 1}`. Then the returned linear transform will then put the coefficient of `𝝵^(k g^i)` in slot
/// `(i, 0)` into the coefficient of `X^(bitrev(i, l) * m/(4l) + k)` and the coefficient of `𝝵^(-k
/// g^i)` in slot `(i, 1)` into the coefficient of `X^(bitrev(i, l) * m/(4l) + m/4 + k)`.
///
/// If `p = 3 mod 4`, the slots are enumerated by `i` with `0 <= i < l` and the transform will put
/// the coefficient of `𝝵^(k g^i)` in slot `i` into the coefficient of `X^(bitrev(i, l) * m/(4l) +
/// k)` and the coefficient of `𝝵^(k g^i + m/4)` into the coefficient of `X^(bitrev(i, l) * m/(4l) +
/// m/4 + k)` for `0 <= k < d/2`.
#[instrument(skip_all)]
    ...
    BaseRing<R>: NiceZn,
{
    ...
    if H.hypercube().dim_count() == 2 {
        ...
        let initial_transform = MatmulTransform::blockmatmul0d_inv(H, |i, j, idxs| {
            S.wrt_canonical_basis(&S.pow(
                S.canonical_gen(),
                j * Gal.representative(&Gal.inv(&H.hypercube().map_usize(idxs))) as usize,
            ))
            .at(i)
        });
        ...
        let initial_transform = MatmulTransform::blockmatmul0d_inv(H, |i, j, idxs| {
            if j < d / 2 {
                ...
                S.wrt_canonical_basis(&S.pow(
                    S.canonical_gen(),
                    m / 4 + (j - d / 2) * Gal.representative(&Gal.inv(&H.hypercube().map_usize(idxs))) as usize,
                ))
                .at(i)
            }
        });
        ...
        result.extend(slots_to_coeffs_fat_impl(H));
        take_mut::take(&mut result[0], |first| {
            first.compose(H.ring(), H.hypercube(), &initial_transform)
        });
    }
    return result;
}
    ...
    BaseRing<R>: NiceZn,
{
    ...
    if H.hypercube().dim_count() == 2 {
        ...
        result.push(MatmulTransform::blockmatmul0d(H, |row, col, idxs| {
            S.wrt_canonical_basis(&S.pow(
                S.canonical_gen(),
                col * Gal.representative(&Gal.inv(&H.hypercube().map_usize(idxs))) as usize,
            ))
            .at(row)
        }));
    } else {
        ...
        result.push(MatmulTransform::blockmatmul0d(H, |row, col, _idxs| {
            if col < d / 2 {
                if row == col {
                    S.base_ring().one()
                } else {
                ...
                S.wrt_canonical_basis(&S.pow(S.canonical_gen(), col - d / 2 + m as usize / 4))
                    .at(row)
            }
        }));
        ...
        result.push(MatmulTransform::blockmatmul0d(H, |row, col, idxs| {
            if col < d / 2 {
                ...
                S.wrt_canonical_basis(&S.pow(
                    S.canonical_gen(),
                    d / 2 + (col - d / 2) * Gal.representative(&Gal.inv(&H.hypercube().map_usize(idxs))) as usize,
                ))
                .at(row)
            }
        }));
    ...
    BaseRing<R>: NiceZn,
{
    ...
    if H.hypercube().dim_count() == 2 {
        ...
        let two_inv = H
            .ring()
            .base_ring()
            .invert(&H.slot_ring().base_ring().int_hom().map(2))
            .unwrap();
        let mut result = Vec::new();

        result.extend(pow2_bitreversed_inv_dwt(H, 0, |idxs| {
            if idxs[1] == 0 {
                H.galois_group().identity()
            } else {
                debug_assert!(idxs[1] == 1);
...
/// i.e. where all slots contain elements in `Z/pZ`.
///
/// In the case `p = 1 mod 4`, the slots are enumerated by `i, j` with `0 <= i < l/2` and `j in {0,
/// 1}`. If `p = 1 mod 4`. Then the returned linear transform will put the value of the coefficient
/// of `X^(i * m/(2l))` into slot `(bitrev(i, l/2), 0)` and the value the coefficient of `X^(i *
/// m/(2l) + m/4)` into slot `(bitrev(i, l/2), 1)`.
///
/// If `p = 3 mod 4`, the slots are enumerated by `i` with `0 <= i < l` and the transform will put
/// the value of the coefficient of `X^(i * m/(4l))` into slot `bitrev(i, l)`.
///
/// Note that the values of all other coefficients are discarded, hence this transform is not the
/// inverse of [`slots_to_coeffs_thin()`].
/// However, `coeff_to_slots_thin()(slots_to_coeffs_thin()(x))` does give `x` for all thinly-packed
/// plaintexts `x`, i.e. `x` where each slot only contains an element in `Z/pZ`.
#[instrument(skip_all)]
pub fn coeffs_to_slots_thin<R>(
    H: &HypercubeIsomorphism<R>,
    max_levels: usize,
    cost_model: &CircuitEvaluatorCosts,
) -> PlaintextCircuit<R::Type>
    ...
    BaseRing<R>: NiceZn,
{
    ...
    let last = MatmulTransform::mult_scalar_slots(
        H,
        &H.slot_ring().inclusion().map(
            H.slot_ring()
                .base_ring()
                .invert(&H.slot_ring().base_ring().int_hom().map(H.slot_ring().rank() as i32))
                .unwrap(),
        ),
    );
    *result.last_mut().unwrap() = result.last().unwrap().compose(H.ring(), H.hypercube(), &last);

    let frobenius_subgroup = H
        .galois_group()
        .parent()
        .get_group()
        .clone()
        .subgroup([H.hypercube().frobenius(1)]);
    ...
    let result_circuit = MatmulTransform::to_circuit_many(H.ring(), H.hypercube(), result, max_levels, cost_model);
...
///
/// In the case `p = 1 mod 4`, the slots are enumerated by `i, j` with `0 <= i < l` and `j in {0,
/// 1}`. Then the returned linear transform will then put the coefficient of `X^(bitrev(i, l) *
/// m/(4l) + k)` into the coefficient of `𝝵^k` in slot `(i, 0)` and the coefficient of `X^(bitrev(i,
/// l) * m/(4l) + m/4 + k)` into to coefficient of `𝝵^k` of slot `(i, 1)`.
///
/// If `p = 3 mod 4`, the slots are enumerated by `i` with `0 <= i < l` and the transform will put
/// the coefficient of `X^(bitrev(i, l) * m/(4l) + k + m/4 * l)` into the coefficient of `𝝵^(k + d/2
/// * l)` in slot `i`, where `0 <= k < d/2` and `l in {0, 1}`.
#[instrument(skip_all)]
pub fn coeffs_to_slots_fat<R>(
    H: &HypercubeIsomorphism<R>,
    max_levels: usize,
    cost_model: &CircuitEvaluatorCosts,
) -> PlaintextCircuit<R::Type>
...
/// In the case `p = 1 mod 4`, the slots are enumerated by `i, j` with `0 <= i < l` and `j in {0,
/// 1}`. Then the returned linear transform will then put the coefficient of `X^(bitrev(i, l) *
/// m/(4l) + k)` into the slot `(i, 0)` of the `bitrev(k, d)`-th output, and the coefficient of
/// `X^(bitrev(i, l) * m/(4l) + m/4 + k)` into slot `(i, 1)` of the `bitrev(k, d)`-th output.
///
/// If `p = 3 mod 4`, the slots are enumerated by `i` with `0 <= i < l` and the transform will put
/// the coefficient of `X^(bitrev(i, l) * m/(4l) + k)` into slot `i` of the `bitrev(k, d)`-th output
/// and the coefficient of `X^(bitrev(i, l) * m/(4l) + m/4 + k)` into slot `i` of the `bitrev(k +
/// d/2, d)`-th output, where `0 <= k < d/2`.
#[instrument(skip_all)]
pub fn coeffs_to_slots_fat_unpack<R>(
    H: &HypercubeIsomorphism<R>,
    max_levels: usize,
    cost_model: &CircuitEvaluatorCosts,
) -> PlaintextCircuit<R::Type>
    ...
    BaseRing<R>: NiceZn,
{
    ...
    take_mut::take(base_transform.last_mut().unwrap(), |transform| {
        transform.compose(
            ring,
            H.hypercube(),
            &MatmulTransform::mult_ring_element(
                ring,
                H.hypercube(),
                &ring.inclusion().map(
                    ring.base_ring()
                        .invert(&ring.base_ring().int_hom().map(d as i32))
                        .unwrap(),
                ),
            ),
        )
    });
    let mut result = MatmulTransform::to_circuit_many(H.ring(), H.hypercube(), base_transform, max_levels, cost_model);

    for i in (1..log2_d).rev() {
        let coeff = H.ring().pow(H.ring().canonical_gen(), m - (1 << (log2_d - i - 1)));
        result = PlaintextCircuit::add(ring)
            .tensor(
                PlaintextCircuit::linear_transform_ring(&[coeff], ring).compose(PlaintextCircuit::sub(ring), ring),
                ring,
            )
            .tensor_pow(1 << (log2_d - i - 1), ring)
            .compose(
                PlaintextCircuit::identity(1, ring)
                    .tensor(PlaintextCircuit::gal(H.hypercube().frobenius(1 << i), Gal, ring), ring)
    ...
    if H.hypercube().dim_count() == 2 {
        ...
        result = PlaintextCircuit::add(ring)
            ...
            .compose(
                PlaintextCircuit::identity(1, ring)
                    .tensor(PlaintextCircuit::gal(H.hypercube().frobenius(1), Gal, ring), ring)
                    .compose(PlaintextCircuit::identity(1, ring).output_twice(ring), ring)
                    .output_twice(ring)
                    .tensor_pow(1 << (log2_d - 1), ring),
                ring,
    ...
    BaseRing<R>: NiceZn,
{
    ...
    if H.hypercube().dim_count() == 2 {
        ...
        result.push(MatmulTransform::blockmatmul0d_inv(H, |row, col, _idxs| {
            if col < d / 2 {
                if row == col {
                    S.base_ring().one()
                } else {
    ...
    BaseRing<R>: NiceZn,
{
    ...
    if H.hypercube().dim_count() == 2 {
        ...
        take_mut::take(result.last_mut().unwrap(), |last| {
            final_transform.compose(H.ring(), H.hypercube(), &last)
        });
    }
    return result;
}
...
fn test_slots_to_coeffs_non_cyclotomic_ring() {
    ...
    let acting_galois_group = number_ring
        .galois_group()
        .get_group()
        .clone()
        .subgroup([number_ring.galois_group().from_representative(17)]);
    ...
    let actual = MatmulTransform::to_circuit_many(
        &ring,
        &h,
        slots_to_coeffs_base(&H),
        usize::MAX,
        &DEFAULT_EVALUATOR_COSTS,
    )
    .evaluate(&[input], ring.identity())
    .pop()
    .unwrap();
    ...
    let acting_galois_group = number_ring.galois_group().get_group().clone().subgroup([
        number_ring.galois_group().from_representative(33),
        number_ring.galois_group().from_representative(-1),
    ]);
...
fn test_slots_to_coeffs_fat() {
    ...
    let mut current = H.from_slot_values(H.hypercube().element_iter().enumerate().map(|(i, _)| {
        H.slot_ring().sum((0..2).map(|k| {
            S.int_hom()
                .mul_map(S.pow(S.canonical_gen(), k), (i + 1 + 16 * k) as i32)
        }))
    }));
    ...
    let mut current = H.from_slot_values(H.hypercube().element_iter().enumerate().map(|(i, _)| {
        H.slot_ring()
            .sum((0..8).map(|k| S.int_hom().mul_map(S.pow(S.canonical_gen(), k), (i + 1 + 4 * k) as i32)))
    }));
    for T in slots_to_coeffs_fat_impl(&H) {
        current = ring.get_ring().compute_linear_transform(H.hypercube(), &current, &T);
    }
...
fn test_slots_to_coeffs_fat_Xbasis() {
    ...
    let mut current = H.from_slot_values(H.hypercube().element_iter().enumerate().map(|(i, g)| {
        H.slot_ring().sum((0..2).map(|k| {
            S.int_hom().mul_map(
...
fn test_coeffs_to_slots_fat() {
    ...
    let expected = H.from_slot_values(H.hypercube().element_iter().enumerate().map(|(i, _)| {
        H.slot_ring().sum((0..2).map(|k| {
            S.int_hom()
                .mul_map(S.pow(S.canonical_gen(), k), (i + 1 + 16 * k) as i32)
        }))
...
fn test_coeffs_to_slots_fat_unpack() {
    ...
    let actual =
        coeffs_to_slots_fat_unpack(&H, usize::MAX, &DEFAULT_EVALUATOR_COSTS).evaluate(&[current], ring.identity());
    let expected = (0..2)
        .map(|k| {
            H.from_slot_values(
                H.hypercube()
                    .element_iter()
                    .enumerate()
                    .map(|(i, _)| H.slot_ring().int_hom().map((i + 1 + 16 * bitreverse(k, 1)) as i32)),
            )
        })
        .collect::<Vec<_>>();
    ...
    let actual = coeffs_to_slots_fat_unpack(&H, usize::MAX, &DEFAULT_EVALUATOR_COSTS)
        .evaluate(&[ring_literal(&ring, &current)], ring.identity());
    let expected = (0..4)
        .map(|k| {
            H.from_slot_values(
                H.hypercube()
                    .element_iter()
                    .enumerate()
                    .map(|(i, _)| H.slot_ring().int_hom().map((i + 1 + 16 * bitreverse(k, 2)) as i32)),
            )
        })
        .collect::<Vec<_>>();
...
fn test_slots_to_coeffs_pack() {
    ...
    let input = (0..2)
        ...
        .collect::<Vec<_>>();
    let actual = slots_to_coeffs_fat_pack(&H, usize::MAX, &DEFAULT_EVALUATOR_COSTS).evaluate(&input, ring.identity());
...
fn test_coeffs_to_slots_fat_Xbasis() {
    ...
    let expected = H.from_slot_values(H.hypercube().element_iter().enumerate().map(|(i, g)| {
        H.slot_ring().sum((0..2).map(|k| {
            S.int_hom().mul_map(
                S.pow(S.canonical_gen(), k * Gal.representative(&Gal.inv(&g)) as usize),
                (i + 1 + 16 * k) as i32,
...
fn test_slots_to_coeffs_thin_inv() {
    ...
    for (transform, actual) in slots_to_coeffs_base(&H)
        .into_iter()
        .rev()
        .zip(slots_to_coeffs_base_inv(&H).into_iter())
    {
...
fn test_coeffs_to_slots_thin() {
    ...
    let circuit = coeffs_to_slots_thin(&H, usize::MAX, &DEFAULT_EVALUATOR_COSTS);
    let actual = circuit
        .evaluate(std::slice::from_ref(&current), ring.identity())
        .pop()
        .unwrap();
