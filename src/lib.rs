...
pub type DefaultConvolution = feanor_math::algorithms::convolution::ntt::NTTConvolution<
    feanor_math::rings::zn::zn_64::ZnBase,
    feanor_math::rings::zn::zn_64::ZnFastmulBase,
    feanor_math::homomorphism::CanHom<feanor_math::rings::zn::zn_64::ZnFastmul, feanor_math::rings::zn::zn_64::Zn>,
>;
...
fn is_parallel() -> bool { cfg!(feature = "parallel") }
...
fn euler_phi(factorization: &[(i64, usize)]) -> i64 {
    StaticRing::<i64>::RING.prod(
        factorization
            .iter()
            .map(|(p, e)| (p - 1) * StaticRing::<i64>::RING.pow(*p, e - 1)),
    )
}
...
where
    F: FnOnce(&mut [usize; COUNTER_VAR_COUNT]) -> T,
{
