...
use feanor_math::homomorphism::{Homomorphism, SelfIso};
...
use crate::number_ring::galois::{
    CyclotomicGaloisGroup, CyclotomicGaloisGroupBase, CyclotomicGaloisGroupOps, GaloisGroupEl,
};
...
fn divisors(n: i64) -> Vec<i64> {
    ...
    for size in 0..=max_exponents.iter().copied().sum() {
        result.extend(multiset_combinations(&max_exponents, size, |exponents| {
            multiply_product(exponents, &primes)
        }));
    }
    return result;
}

/// Assuming `monic_poly + delta` is irreducible of degree `d` modulo `p`, and `d` divides the rank
/// of the ring `S`, then this constructs a circuit that evaluates `monic_poly` using the norm in a
/// Galois subring of `S`.
///
/// The resulting circuit has the final evaluation as last output. Earlier outputs are `1, x, x^2,
/// x^4, x^8, ...`.
#[instrument(skip_all)]
    ...
    BaseRing<R>: ZnRing,
{
    ...
    let (roots, _) = <_ as FactorPolyField>::factor_poly(
        &FqX,
        &FqX.lifted_hom(&ZpeX, Fq.inclusion().compose(&Zpe_to_Fp))
            .map_ref(&irred_poly),
    );
    ...
    let mut root_in_S = S.from_canonical_basis(
        Fq.wrt_canonical_basis(&root)
            .iter()
            .map(|x| Sbase_to_Fp.smallest_lift(x)),
    );
    let irred_poly_derivate = derive_poly(&ZpeX, &irred_poly);
    for _ in 0..ZZi64.abs_log2_ceil(&e.try_into().unwrap()).unwrap() {
        root_in_S = S.sub_ref_fst(
            &root_in_S,
            S.checked_div(
                &ZpeX.evaluate(&irred_poly, &root_in_S, S.inclusion().compose(&Zpe_to_Sbase)),
                &ZpeX.evaluate(&irred_poly_derivate, &root_in_S, S.inclusion().compose(&Zpe_to_Sbase)),
            )
            .unwrap(),
        );
    }
    ...
    let subring_frobenius =
        subring_galois_group.from_representative(parent_galois_group.representative(&frobenius).try_into().unwrap());
    let relative_galois_group = subring_galois_group.into().subgroup([subring_frobenius]);
    let base_circuit = norm_circuit(&S, &relative_galois_group)
        .compose(PlaintextCircuit::sub(&S), &S)
        .compose(
            PlaintextCircuit::constant(root_in_S, &S).tensor(PlaintextCircuit::identity(1, &S), &S),
            &S,
        );
    let k = ZZi64
        .abs_log2_ceil(&ZpeX.degree(&delta).unwrap().try_into().unwrap())
        .unwrap();
    assert_eq!(1 << k, ZpeX.degree(&delta).unwrap());
    let pows2_circuit = (0..k).fold(
        PlaintextCircuit::constant_int(ZZbig.one(), &S).tensor(PlaintextCircuit::identity(1, &S), &S),
        |current, _| {
            PlaintextCircuit::identity(current.output_count(), &S)
                .tensor(
                    PlaintextCircuit::square(&S).compose(
                        PlaintextCircuit::select(current.output_count(), &[current.output_count() - 1], &S),
                        &S,
                    ),
                    &S,
                )
    ...
    let correction_circuit = PlaintextCircuit::linear_transform_ring(
        &[ZpeX.coefficient_at(&delta, 0)]
            .into_iter()
            .chain((0..=k).map(|i| ZpeX.coefficient_at(&delta, 1 << i)))
            .map(|x| S.inclusion().map(Zpe_to_Sbase.map_ref(x)))
            .collect::<Vec<_>>(),
        &S,
    );
    let result = PlaintextCircuit::identity(pows2_circuit.output_count(), &S)
        .tensor(
            PlaintextCircuit::linear_transform(&[Coefficient::NegOne, Coefficient::One], &S)
                .compose(correction_circuit.tensor(base_circuit, &S), &S),
            &S,
        )
        .compose(
            pows2_circuit
                .output_twice(&S)
                .tensor(PlaintextCircuit::identity(1, &S), &S),
            &S,
        )
        .compose(PlaintextCircuit::identity(1, &S).output_twice(&S), &S);
    return result;
}

/// Assuming that `monic_poly` is a monic polynomial of degree `d` dividing the rank of `S`, then
/// this constructs a circuit evaluating `monic_poly` using the norm in a Galois subring of `S`.
///
/// This may fail in rare cases if no suitable irreducible polynomials are found that suitably
/// relate to `monic_poly`. The resulting circuit has the final evaluation as last output. Earlier
/// outputs are `1, x, x^2, x^4, x^8, ...`.
#[instrument(skip_all)]
    ...
    BaseRing<R>: ZnRing,
{
    ...
    let FpX = DensePolyRing::new(
        zn_64::Zn::new(int_cast(p, ZZi64, Zpe.integer_ring()) as u64)
            .as_field()
            .unwrap(),
        "X",
    );
    ...
    for _ in 0..(2 * d) {
        let delta = ZpeX.from_terms([
            (Zpe.random_element(|| rng.rand_u64()), 0),
            (Zpe.random_element(|| rng.rand_u64()), 1),
        ]);
        ...
        if <_ as FactorPolyField>::is_irred(&FpX, &FpX.add_ref_fst(&monic_poly_mod_p, delta_mod_p)) {
            return Ok(compute_circuit_from_irreducible_poly(
                &FpX,
                ZpeX,
                monic_poly,
                &delta,
    ...
    for k in 0..ZZi64.abs_log2_ceil(&d.try_into().unwrap()).unwrap() {
        for _ in 0..d {
            ...
            let delta = ZpeX.from_terms(
                (0..=k)
                    .map(|i| (Zpe.random_element(|| rng.rand_u64()), 1 << i))
                    .chain([(delta_constant, 0)]),
            );
            let delta_mod_p = FpX.lifted_hom(&ZpeX, &Zpe_to_Fp).map_ref(&delta);
            if <_ as FactorPolyField>::is_irred(&FpX, &FpX.add_ref_fst(&monic_poly_mod_p, delta_mod_p)) {
                return Ok(compute_circuit_from_irreducible_poly(
...
#[instrument(skip_all)]
pub fn poly_circuit_via_norm<P, R>(
    hypercube_iso: &HypercubeIsomorphism<R>,
    poly_ring: P,
    poly: &El<P>,
) -> Result<PlaintextCircuit<R::Type>, ()>
where
    P: RingStore,
    P::Type: PolyRing,
    BaseRing<P>: ZnRing + DivisibilityRing,
    ...
    BaseRing<R>: NiceZn,
{
    ...
    let Zpe_to_S = S
        .inclusion()
        .compose(ZnReductionMap::new(poly_ring.base_ring(), S.base_ring()).unwrap());
    let rank_divisors = divisors(S.rank().try_into().unwrap());

    let result = loop {
        subfield_rank = rank_divisors
            .iter()
            .copied()
            .map(|x| usize::try_from(x).unwrap())
            .filter(|new_d| *new_d >= subfield_rank)
            .min()
            .ok_or(())?;
        if poly_ring.degree(poly).unwrap() < subfield_rank {
            let new_poly = poly_ring.add_ref_fst(
                poly,
                poly_ring.from_terms([(poly_ring.base_ring().one(), subfield_rank)]),
            );
            let main_circuit = find_irreducible_modification(
                &poly_ring,
                &new_poly,
                hypercube_iso.slot_ring(),
                hypercube_iso.galois_group().parent(),
                hypercube_iso.hypercube().frobenius(1),
            )?;
            debug_assert!(main_circuit.output_count() >= 3);
            let k = main_circuit.output_count() - 3;
            let correction_circuit = compute_powers_circuit(
                S,
                &[0].into_iter().chain((0..=k).map(|i| 1 << i)).collect::<Vec<_>>(),
                &[subfield_rank],
            );
            ...
            break PlaintextCircuit::linear_transform_ring(&[Zpe_to_S.map_ref(poly_ring.lc(poly).unwrap())], S)
                .compose(
                    PlaintextCircuit::select(main_circuit.output_count(), &[main_circuit.output_count() - 1], S),
                    S,
                )
                .compose(main_circuit, S);
        } else {
    ...
    return Ok(result.change_ring_uniform(|x| match x {
        ...
        Coefficient::Other(x) => Coefficient::Other(
            hypercube_iso
                .from_slot_values((0..hypercube_iso.slot_count()).map(|_| hypercube_iso.slot_ring().clone_el(&x))),
        ),
    }));
}
...
fn test_poly_circuit_via_norm_pow2() {
    ...
    let hypercube = HypercubeIsomorphism::new(
        &ring,
        &HypercubeStructure::default_pow2_hypercube(ring.acting_galois_group(), int_cast(3, ZZbig, ZZi64)),
        None,
    );
    ...
    for x in ring.base_ring().elements() {
        assert_el_eq!(
            ring,
            ring.inclusion()
                .map(poly_ring.evaluate(&f, &x, ring.base_ring().identity())),
            &circuit.evaluate(&[ring.inclusion().map(x)], ring.identity())[0]
        );
    }
    ...
    let [f] =
        poly_ring.with_wrapped_indeterminate(|X| [4 * X.pow_ref(4) + 7 * X.pow_ref(3) + 2 * X.pow_ref(2) + 9 * X + 3]);
    let circuit = poly_circuit_via_norm(&hypercube, &poly_ring, &f).unwrap();
    assert_eq!(1, circuit.output_count());
    assert_eq!(2, circuit.multiplication_gate_count());
    assert_eq!(2, circuit.galois_gate_output_sum());
