        ...
        BaseRing<R>: ZnRing
{
    ...
    if ZZ.eq_el(&p, &ZZ.int_hom().map(2)) {
        ...
        //  - if A even, B even: solvable iff K = 0, 4, 12 mod 16 (with g = 2 if K = 4, 12 mod 16 and g = 4 otherwise)
        let K = Zpe.sum([
        ...
        let x = if Zpe.divides(&A, &Zpe.int_hom().map(2)) {
            let g = if Zpe.divides(&B, &Zpe.int_hom().map(2)) {
                match (
                    ...
                    (false, _, _) => return Err(Deg8PolyEvalError::Char2NotSolvable),
        ...
        let y = Zpe.checked_div(&Zpe.sum([
            Zpe.mul_ref_fst(&u, Zpe.mul_ref(&x, &x)),
            Zpe.negate(Zpe.mul_ref_fst(&u, Zpe.mul_ref(&A, &x))),
            Zpe.mul_ref(&B, &x),
            Zpe.mul_ref(&u, &C),
            Zpe.negate(D)
        ]), &Zpe.sub_ref_snd(Zpe.int_hom().mul_ref_map(&x, &2), &A)).unwrap();
        ...
        circuit = PlaintextCircuit::identity(2, Zpe).tensor(
            PlaintextCircuit::mul(Zpe).compose(
                PlaintextCircuit::linear_transform(
                    ...
                    PlaintextCircuit::select(2, &[1], Zpe), Zpe
                ), 
                Zpe
            ),
            Zpe
...
fn test_degree_8_poly_even_char() {
    ...
    for x in Zpe.elements() {
        assert_el_eq!(Zpe, ZpeX.evaluate(&f1, &x, Zpe.identity()), circuit.evaluate_no_galois(&[x], Zpe.identity()).pop().unwrap());
    }

    let circuit = degree_8_poly_circuit(&ZpeX, ZpeX.clone_el(&f2)).unwrap();
    for x in Zpe.elements() {
        assert_el_eq!(Zpe, ZpeX.evaluate(&f2, &x, Zpe.identity()), circuit.evaluate_no_galois(&[x], Zpe.identity()).pop().unwrap());
    }

    assert_eq!(Deg8PolyEvalError::Char2OddCoeff7, degree_8_poly_circuit(&ZpeX, ZpeX.clone_el(&f3)).err().unwrap());
    assert_eq!(Deg8PolyEvalError::Char2NotSolvable, degree_8_poly_circuit(&ZpeX, ZpeX.clone_el(&f4)).err().unwrap());
}