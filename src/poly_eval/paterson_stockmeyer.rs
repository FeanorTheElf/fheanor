    ...
    F: FnMut(usize) -> i64,
{
    ...
    let approx = (*range.start()..*range.end())
        .step_by(delta)
        .chain([*range.end()])
        .min_by_key(|x| f(*x))
        .unwrap();
    if delta > 1 {
        ...
        return minimum_somewhat_continuous_function(
            max(*range.start(), approx.saturating_sub(len as usize / 3))..=min(*range.end(), approx + len as usize / 3),
            f,
            steps,
        );
    } else {
...
///  - compute powers `x^i` for `i` in `precomputed_powers`; this always includes `1, 2, ..., k`,
///    and usually many powers of two
///  - for a polynomial `f` of degree `d`, recursively write it as `f = q X^l + r` and `r = c q +
///    s`, where `l in split_at_monomial[d].0`. Then `f = q (X^l + c) + s`, and we evaluate it by
///    combining `x^l` and `q(x), c(x), s(x)`, which are evaluated recursively; "monic
///    augmentation": as fallback, we also consider the splitting as `f + X^m = q X^l + r` and `r =
///    c q + s` with `(l, m) = split_at_monomial[d].1` and `m > deg(f)` - this might be necessary if
///    the leading coefficient of `f` is not a unit. In that case, we evaluate by combining `x^l,
///    x^m` and `q(x), c(x), s(x)` as `f(x) = q(x) (x^l + c(x)) + s(x) - x^m`;
///  - asymptotically, we would choose l = (d + k) / 2 so that deg(c) < k and we can derive `c(x)``
///    from the precomputed powers; this leads to a reduction `d -> 2 x (d - k)/2` on each recursive
///    step, with means we need `log2(d/k) - 1` recursive steps until we reach degree `k` and can
///    continue with precomputed powers. Thus, the total cost is `2^(log2(d/k) - 1) + k = d/(2k) +
///    k`, which is optimized at `k = sqrt(d/2)`, leading to `sqrt(2d)`. In practice, we will
///    optimize the concrete choice of `l` using dynamic programming.
...
/// evaluation of generic polynomials of the given degrees (as explained in
/// [`PatersonStockmeyerPlan`]), and picks the choice leading to the smallest number of
/// multiplications, under the restriction of having optimal multiplicative depth.
///
...
fn plan_paterson_stockmeyer_circuit(ds: &[usize]) -> PatersonStockmeyerPlan {
    ...
    /// cost of evaluating `x^i`, excluding costs for unconditionally precomputed values.
    /// `prev_costs[i]` for `i < d` should be the cost of recursively evaluating a polynomial of
    /// degree `i`.
    ...
    fn compute_costs(d: usize, k: usize, power_costs: &[usize]) -> usize {
        ...
        for i in (k + 1)..(d + 1) {
            result.push(
                (1..(i + 1))
                    .map(|l| compute_cost_for_degree(i, l, power_costs, &result))
                    .min()
                    .unwrap(),
            );
        }
    ...
    //  - get a more precise (still heuristic) value for the set of precomputed powers, currently
    //    consisting of `k` consecutive powers, followed by powers to powers of two
    //  - get values for the splitting points `l` for each degree; this step may overestimate the actual
    //    cost, since it assumes that only the powers `precomputed_powers` are available for free for
    //    every degree, even if additional powers have been computed for lower degrees (but of course we
    //    don't know if those lower degree computations will actually be used for the final polynomial)
    ...
    let rough_k = minimum_somewhat_continuous_function(
        min_k..=max_k,
        |k| {
            if let Some(res) = cache.get(&k) {
                *res
                ...
                let cost =
                    ds.iter().map(|d| compute_costs(*d, k, &power_costs)).sum::<usize>() + precomputed_powers.len() - 2;
    ...
    let (exact_k, precomputed_powers, precomputed_powers_pow2) = (max(rough_k.saturating_sub(4), 1)..(rough_k + 9))
        .rev()
        .flat_map(|k| (0..5).map(move |skipped_pow2s| (k, skipped_pow2s)))
        .map(|(k, skipped_pow2s)| {
            let precomputed_powers_pow2 = (0..max_log2_d.saturating_sub(skipped_pow2s))
                .map(|i| 1 << i)
                .collect::<Vec<_>>();
            let mut precomputed_powers = (0..=k)
                .chain((0..max_log2_d.saturating_sub(skipped_pow2s)).map(|i| 1 << i))
                .collect::<Vec<_>>();
            precomputed_powers.sort();
            precomputed_powers.dedup();
        ...
        .min_by_key(|(k, precomputed_powers, _)| {
            ...
            let cost = ds
                .iter()
                .map(|d| compute_costs(*d, (*k).try_into().unwrap(), &power_costs))
                .sum::<usize>()
                + precomputed_powers.len()
                - 2;
            return cost;
        })
        .unwrap();
    ...
    /// if we split `f = f(x) = q(x) (x^l + c(x)) + s(x)`, the multiplicative depth of the result
    /// will be `max(dpt(x^l), log2(q)) + 1`; thus we cannot split to asymmetrically. The idea
    /// is that we restrict to splitting points that satisfy `log2(l), log2(d - l) <=
    /// log2(deg(f) * (1 + 2 * DEPTH_RELAX))`. In that case, the multiplicative depth increases
    /// at most by `log2(1 + 2 * DEPTH_RELAX)` on every recursion step, and also behaves very
    /// controllably in practice.
    ...
    BaseRing<R>: DivisibilityRing,
{
    fn is_fallback_split(&self) -> bool {
        if let PatersonStockmeyerSplit::FallbackSplit {
            q: _,
            c: _,
            s: _,
            l: _,
        ...
        if d <= plan.k {
            ...
            for l in splitting_points.iter().skip(1).take(MAX_ATTEMPTS) {
                ...
                if !q.is_fallback_split() && !c.is_fallback_split() && !s.is_fallback_split() {
                    return PatersonStockmeyerSplit::StandardSplit {
                        q: Box::new(q),
                        c: Box::new(c),
                        s: Box::new(s),
                        factor: lc_f,
            ...
            let c = PatersonStockmeyerSplit::create_recursive(poly_ring, c, plan, rec_depth + 1);
            let s = PatersonStockmeyerSplit::create_recursive(poly_ring, s, plan, rec_depth + 1);
            return PatersonStockmeyerSplit::FallbackSplit {
                q: Box::new(q),
                c: Box::new(c),
                s: Box::new(s),
                l: *monic_augment_l,
    ...
    #[instrument(skip_all)]
    fn to_circuit_recursive(
        self,
        poly_ring: R,
        input_powers: &[usize],
        rec_depth: usize,
    ) -> (El<R>, PlaintextCircuit<BaseRing<R>>) {
        let base_ring = poly_ring.base_ring();
        let get_power_idx = |power: usize| {
            input_powers
                .iter()
                .enumerate()
                .filter(|(_, val)| **val == power)
                .next()
                .unwrap()
                .0
        };
        match self {
                ...
                let circuit = PlaintextCircuit::add(base_ring).compose(
                    PlaintextCircuit::linear_transform_ring(
                        &input_powers
                            .iter()
                            .copied()
                            .map(|k| base_ring.clone_el(poly_ring.coefficient_at(&f, k)))
                            .collect::<Vec<_>>(),
                        base_ring,
                    )
                    .tensor(
                        PlaintextCircuit::constant(base_ring.clone_el(poly_ring.coefficient_at(&f, 0)), base_ring),
                        base_ring,
                    ),
                    base_ring,
                );
            ...
            PatersonStockmeyerSplit::StandardSplit { q, c, s, l, factor } => {
                ...
                let f_circuit = PlaintextCircuit::linear_transform_ring(&[base_ring.clone_el(&factor)], base_ring)
                    .compose(
                        PlaintextCircuit::add(base_ring)
                            .compose(
                                PlaintextCircuit::mul(base_ring)
                                    .compose(
                                        PlaintextCircuit::add(base_ring)
                                            .compose(Xl.tensor(c_circuit, base_ring), base_ring)
                ...
                let f = poly_ring
                    .inclusion()
                    .mul_map(poly_ring.add(poly_ring.mul(poly_ring.add(c, Xl), q), s), factor);
                return (f, f_circuit);
            }
            PatersonStockmeyerSplit::FallbackSplit { q, c, s, l, m } => {
                let Xl = PlaintextCircuit::select(input_powers.len(), &[get_power_idx(l)], base_ring);
                ...
                let (c, c_circuit) = c.to_circuit_recursive(poly_ring, input_powers, rec_depth + 1);
                let (s, s_circuit) = s.to_circuit_recursive(poly_ring, input_powers, rec_depth + 1);
                let f_circuit = PlaintextCircuit::linear_transform(
                    &[Coefficient::One, Coefficient::One, Coefficient::NegOne],
                    base_ring,
                )
                .compose(
                    PlaintextCircuit::mul(base_ring)
    ...
    BaseRing<R>: DivisibilityRing + FiniteRing,
{
    let degrees = polynomials
        .iter()
        .map(|f| poly_ring.degree(f).expect("all polynomials must be nonzero"))
        .collect::<Vec<_>>();
    ...
    (0..10)
        .map(|_| {
            ...
            for poly in polynomials {
                let randomized_poly = poly_ring.evaluate(
                    poly,
                    &poly_ring.from_terms([
                        (poly_ring.base_ring().one(), 1),
                        (poly_ring.base_ring().clone_el(&random_value), 0),
                    ]),
                    poly_ring.inclusion(),
                );
                splits.push(PatersonStockmeyerSplit::create_recursive(
                    &poly_ring,
                    randomized_poly,
                    &plan,
                    0,
            ...
            let main_circuit =
                splits
                    .into_iter()
                    .fold(PlaintextCircuit::drop(precomputed_powers.len()), |current, next| {
                        current
                            .tensor(
                                next.to_circuit_recursive(&poly_ring, &precomputed_powers, 0).1,
                                poly_ring.base_ring(),
                            )
                            .compose(
                                PlaintextCircuit::identity(precomputed_powers.len(), poly_ring.base_ring())
                                    .output_twice(poly_ring.base_ring()),
                                poly_ring.base_ring(),
                            )
                    });
            ...
            return main_circuit
                .compose(precompute_powers_circuit, poly_ring.base_ring())
                .compose(de_randomize_circuit, poly_ring.base_ring());
        })
        .min_by_key(|circuit| circuit.multiplication_gate_count())
        .unwrap()
}
...
#[test]
fn test_plan_multiple() { assert_eq!(12, plan_paterson_stockmeyer_circuit(&[17, 34]).mul_count) }
...
fn test_evaluation_circuit() {
    ...
    for i in 0..10 {
        assert_el_eq!(
            Fp,
            FpX.evaluate(&f, &Fp.int_hom().map(i), Fp.identity()),
            circuit.evaluate_no_galois(&[Fp.int_hom().map(i)], Fp.identity())[0]
        );
    }
...
fn test_evaluation_circuit_multiple() {
    ...
    for i in 0..10 {
        assert_el_eq!(
            Fp,
            FpX.evaluate(&polys[0], &Fp.int_hom().map(i), Fp.identity()),
            circuit.evaluate_no_galois(&[Fp.int_hom().map(i)], Fp.identity())[0]
        );
        assert_el_eq!(
            Fp,
            FpX.evaluate(&polys[1], &Fp.int_hom().map(i), Fp.identity()),
            circuit.evaluate_no_galois(&[Fp.int_hom().map(i)], Fp.identity())[1]
        );
    }

    let polys = FpX.with_wrapped_indeterminate(|X| {
        [
            X.pow_ref(5) + X.pow_ref(3) + 2 * X.pow_ref(2) - 4 * X + 1,
            X.pow_ref(4) + 2 * X.pow_ref(2) - 4 * X + 1,
            X.pow_ref(2) - 1,
        ]
    });
    let circuit = paterson_stockmeyer_circuit(&FpX, &polys);
    ...
    for i in 0..10 {
        ...
        assert_el_eq!(
            Fp,
            FpX.evaluate(&polys[2], &Fp.int_hom().map(i), Fp.identity()),
            circuit.evaluate_no_galois(&[Fp.int_hom().map(i)], Fp.identity())[2]
        );
    }
}
