...
impl OddSquarefreeCyclotomicNumberRing {
    pub fn new(m: usize) -> Self {
        ...
        // of the number ring as lattice don't hold anymore (currently this refers to the
        // `norm1_to_norm2_expansion_factor` functions)
    ...
    pub fn m(&self) -> u64 { self.galois_group.m() }
    ...
    ///
    /// [`TensorProductNumberRing`]: crate::number_ring::tensor_ring::TensorProductNumberRing
    pub fn powinf_to_coeffinf_expansion(&self) -> f64 { self.powinf_to_coeffinf_expansion }
    ...
    ///
    /// [`TensorProductNumberRing`]: crate::number_ring::tensor_ring::TensorProductNumberRing
    pub fn coeffinf_to_powinf_expansion(&self) -> f64 { self.coeffinf_to_powinf_expansion }
...
pub fn compute_powinf_to_coeffinf_expansion(m: i64) -> f64 {
    ...
    let factorization_rings = factorization
        .iter()
        .map(|(p, e)| Zn::new(ZZi64.pow(*p, *e) as u64))
        .collect::<Vec<_>>();
    ...
    let is_powerful_basis_monomial = |i: usize| {
        factorization_rings
            .iter()
            .zip(factorization.iter())
            .all(|(ring, (p, e))| {
                ring.smallest_positive_lift(
                    ring.checked_div(
                        &ring.int_hom().map(i as i32),
                        &ring.int_hom().map((m / *ring.modulus()).try_into().unwrap()),
                    )
                    .unwrap(),
                ) < ZZi64.pow(*p, e - 1) * (p - 1)
            })
...
pub fn compute_coeffinf_to_powinf_expansion(m: i64) -> f64 {
    ...
    let ZZX = SparsePolyRing::new(ZZi64, "X");
    let cyclotomic_polys = factorization
        .iter()
        .map(|(p, e)| cyclotomic_polynomial(&ZZX, ZZi64.pow(*p, *e) as usize))
        .collect::<Vec<_>>();
    let mut row_accumulated: Vec<i64> = (0..phi_m).map(|_| 0).collect::<Vec<_>>();
    for i in 0..phi_m {
        let tensor_indices = factorization_rings.iter().map(|ring| {
            ring.smallest_positive_lift(
                ring.checked_div(
                    &ring.int_hom().map(i as i32),
                    &ring.int_hom().map((m / *ring.modulus()).try_into().unwrap()),
        ...
        let tensor_polys = tensor_indices
            .enumerate()
            .map(|(j, power)| {
                ZZX.div_rem_monic(ZZX.from_terms([(1, power as usize)]), &cyclotomic_polys[j])
                    .1
            })
            .collect::<Vec<_>>();
        let tensor_polys_terms = tensor_polys
            .iter()
            .map(|f| ZZX.terms(f).collect::<Vec<_>>())
            .collect::<Vec<_>>();
        for (k, coeff) in multi_cartesian_product(
            tensor_polys_terms.iter().map(|terms| terms.iter()),
            |terms| {
                (
                    terms
                        .iter()
                        .map(|(_, pow)| pow)
                        .zip(factorization.iter())
                        .fold(0, |current, (next, (p, e))| {
                            current * ZZi64.pow(*p, e - 1) as usize * (p - 1) as usize + next
                        }),
...
impl Debug for OddSquarefreeCyclotomicNumberRing {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result { write!(f, "Z[𝝵_{}]", self.m()) }
}
...
impl PartialEq for OddSquarefreeCyclotomicNumberRing {
    fn eq(&self, other: &Self) -> bool { self.m_factorization_squarefree == other.m_factorization_squarefree }
}

impl NumberRingDescriptor for OddSquarefreeCyclotomicNumberRing {
    type NumberRingQuotientBases = OddSquarefreeCyclotomicDecomposedNumberRing<
        BluesteinFFT<ZnBase, ZnFastmulBase, CanHom<ZnFastmul, Zn>, AllocArc<DynLayoutMempool<Global>>>,
        AllocArc<DynLayoutMempool<Global>>,
    >;
    ...
    fn coeff_basis_product_expansion_factor(&self) -> f64 { self.small_basis_product_expansion_factor() }
    ...
    fn bases_mod_p(&self, Fp: Zn) -> Self::NumberRingQuotientBases {
        ...
        let log2_help_len = StaticRing::<i64>::RING
            .abs_log2_ceil(&(2 * m).try_into().unwrap())
            .unwrap();
    ...
    fn mod_p_required_root_of_unity(&self) -> u64 {
        return self.m()
            << StaticRing::<i64>::RING
                .abs_log2_ceil(&(2 * self.m()).try_into().unwrap())
                .unwrap();
    }
    ...
    fn rank(&self) -> usize { euler_phi_squarefree(&self.m_factorization_squarefree) as usize }

    fn galois_group(&self) -> &CyclotomicGaloisGroup { &self.galois_group }
}

pub struct OddSquarefreeCyclotomicDecomposedNumberRing<F, A = Global>
where
    F: FFTAlgorithm<ZnBase> + PartialEq,
    A: FheanorAllocator,
{
    galois_group: CyclotomicGaloisGroup,
    ring: Zn,
    fft_table: F,
    /// contains `usize::MAX` whenenver the fft output index corresponds to a non-primitive root of
    /// unity, and an index otherwise
    ...
    A: FheanorAllocator,
{
    fn create_squarefree(
        ...
        for (a, i) in poly_ring.terms(&cyclotomic_poly) {
            if i != rank {
                zeta_pow_rank.push((
                    i,
                    Fp.negate(Fp_as_field.get_ring().unwrap_element(Fp_as_field.clone_el(a))),
                ));
            }
        }
        zeta_pow_rank.sort_unstable_by_key(|(i, _)| *i);

        let fft_output_indices_to_indices = (0..fft_table.len())
            .scan(0, |state, i| {
    ...
    fn fft_output_indices<'a>(&'a self) -> impl Iterator<Item = (usize, usize)> + 'a {
        self.fft_output_indices_to_indices
            .iter()
            .enumerate()
            .filter_map(|(i, j)| if *j == usize::MAX { None } else { Some((*j, i)) })
    }
...
where
    F: FFTAlgorithm<ZnBase> + PartialEq + Send + Sync,
    A: FheanorAllocator,
{
    fn galois_group(&self) -> &CyclotomicGaloisGroup { &self.galois_group }
    ...
    fn base_ring(&self) -> &Zn { &self.ring }
        ...
        V2: SwappableVectorViewMut<ZnEl>,
    {
        ...
        for (j, i) in self.fft_output_indices() {
            *dst.at_mut(j) = ring.clone_el(src.at(
                self.fft_output_indices_to_indices[self.fft_table.unordered_fft_permutation_inv(
                    index_ring.smallest_positive_lift(index_ring.mul(
                        *galois_group.as_ring_el(galois_element),
                        hom.map(self.fft_table.unordered_fft_permutation(i) as i64),
                    )) as usize,
                )],
            ));
        }
...
fn test_permute_galois_automorphism() {
    ...
    assert_el_eq!(
        R,
        ring_literal(&R, &[0, 0, 1, 0, 0, 0]),
        R.apply_galois_action(&ring_literal(&R, &[0, 1, 0, 0, 0, 0]), &gal_el(2))
    );
    assert_el_eq!(
        R,
        ring_literal(&R, &[0, 0, 0, 1, 0, 0]),
        R.apply_galois_action(&ring_literal(&R, &[0, 1, 0, 0, 0, 0]), &gal_el(3))
    );
    assert_el_eq!(
        R,
        ring_literal(&R, &[0, 0, 0, 0, 1, 0]),
        R.apply_galois_action(&ring_literal(&R, &[0, 0, 1, 0, 0, 0]), &gal_el(2))
    );
    assert_el_eq!(
        R,
        ring_literal(&R, &[-1, -1, -1, -1, -1, -1]),
        R.apply_galois_action(&ring_literal(&R, &[0, 0, 1, 0, 0, 0]), &gal_el(3))
    );
}
