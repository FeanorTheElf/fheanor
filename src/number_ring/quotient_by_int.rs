...
pub type NumberRingQuotientByInt<NumberRing, ZnTy, A = Global, C = KaratsubaAlgorithm> =
    RingValue<NumberRingQuotientByIntBase<NumberRing, ZnTy, A, C>>;
    ...
    C: ConvolutionAlgorithm<ZnTy::Type>,
{
    ...
    pub fn create(number_ring: NumberRing, base_ring: ZnTy, allocator: A, convolution: C) -> RingValue<Self> {
        ...
        assert!(ZZX.eq_el(
            &number_ring.generating_poly(&ZZX),
            &cyclotomic_polynomial(&ZZX, number_ring.galois_group().m() as usize)
        ));
        let acting_galois_group = number_ring.galois_group().clone().into().full_subgroup();
        let reducer = CyclotomicPolyReducer::new(
            poly_ring.into().into_base_ring(),
            number_ring.galois_group().m(),
            convolution,
        );
    ...
    C: ConvolutionAlgorithm<ZnTy::Type>,
{
    ...
    fn m(&self) -> usize { self.acting_galois_group().m() as usize }
}
    ...
    C: ConvolutionAlgorithm<ZnTy::Type>,
{
    ...
    fn apply_galois_action(&self, x: &Self::Element, g: &GaloisGroupEl) -> Self::Element {
        ...
        for i in 0..m {
            result[Zm.smallest_positive_lift(mod_m.mul_ref_map(g_Zm, &(i as i64))) as usize] =
                self.base_ring().clone_el(&x.data[i]);
        }
    ...
    C: ConvolutionAlgorithm<ZnTy::Type>,
{
    ...
    fn prepare_multiplicant(&self, x: &Self::Element) -> Self::PreparedMultiplicant {
        NumberRingQuotientByIntPreparedMultiplicant {
            ...
            data: self
                .convolution()
                .prepare_convolution_operand(&x.data, Some(2 * self.m()), self.base_ring()),
        }
    }
    ...
    C: ConvolutionAlgorithm<ZnTy::Type>,
{
    type VectorRepresentation<'a>
        = CloneElFn<Vec<El<ZnTy>, A>, El<ZnTy>, CloneRingEl<&'a ZnTy>>
    where
        Self: 'a;
    ...
    C: ConvolutionAlgorithm<ZnTy::Type>,
{
        ...
        S: Serializer,
    {
        ...
        SerializableNewtypeStruct::new(
            ...
            SerializableSeq::new_with_len(
                el_reduced.data[..self.rank()]
                    .iter()
                    .map(|x| SerializeWithRing::new(x, self.base_ring())),
                self.rank(),
            ),
        )
    ...
    where
        D: Deserializer<'de>,
    {
        let mut result = DeserializeSeedNewtypeStruct::new(
            "RingEl",
...
impl<NumberRing, ZnTy1, ZnTy2, A1, A2, C1, C2> CanHomFrom<NumberRingQuotientByIntBase<NumberRing, ZnTy2, A2, C2>>
    for NumberRingQuotientByIntBase<NumberRing, ZnTy1, A1, C1>
where
    NumberRing: NumberRingDescriptor + Clone,
    ...
    ZnTy1::Type: CanHomFrom<ZnTy2::Type>,
{
    ...
    fn has_canonical_hom(
        &self,
        from: &NumberRingQuotientByIntBase<NumberRing, ZnTy2, A2, C2>,
    ...
    fn map_in(
        &self,
        from: &NumberRingQuotientByIntBase<NumberRing, ZnTy2, A2, C2>,
        el: <NumberRingQuotientByIntBase<NumberRing, ZnTy2, A2, C2> as RingBase>::Element,
        hom: &Self::Homomorphism,
    ) -> Self::Element {
        assert_eq!(el.data.len(), self.m());
        let mut result = Vec::with_capacity_in(self.m(), self.allocator.clone());
        result.extend((0..self.m()).map(|i| {
            self.base_ring()
                .get_ring()
...
impl<NumberRing, ZnTy1, ZnTy2, A1, A2, C1, C2> CanIsoFromTo<NumberRingQuotientByIntBase<NumberRing, ZnTy2, A2, C2>>
    for NumberRingQuotientByIntBase<NumberRing, ZnTy1, A1, C1>
where
    NumberRing: NumberRingDescriptor + Clone,
    ZnTy1: RingStore + Clone,
    ...
    ZnTy1::Type: CanIsoFromTo<ZnTy2::Type>,
{
    ...
    ) -> <NumberRingQuotientByIntBase<NumberRing, ZnTy2, A2, C2> as RingBase>::Element {
        assert_eq!(el.data.len(), self.m());
...
#[cfg(test)]
pub fn test_with_number_ring<NumberRing: NumberRingDescriptor>(number_ring: NumberRing) {
