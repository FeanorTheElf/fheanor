...
use crate::cache::{
    DeserializeSeedDeserializableWithData, SerializeDeserializeWith, SerializeSerializableWithData, StoreAs,
    create_cached,
};
...
#[instrument(skip_all)]
pub(super) fn create_convolution<R>(
    d: usize,
    log2_input_size: usize,
) -> DynConvolutionAlgorithmConvolution<R, Arc<dyn DynConvolutionAlgorithm<R>>>
where
    R: ?Sized + ZnRing + CanHomFrom<BigIntRingBase> + CanHomFrom<StaticRingBase<i64>>,
{
    ...
    <<R2::Type as RingExtension>::BaseRing as RingStore>::Type: ZnRing,
{
    ...
    assert_el_eq!(
        Fp.base_ring().integer_ring(),
        Fp.base_ring().modulus(),
        int_cast(p, Fp.base_ring().integer_ring(), S.base_ring().integer_ring())
    );

    let red_map = ZnReductionMap::new(S.base_ring(), Fp.base_ring()).unwrap();
    let mut result = S.from_canonical_basis(
        Fp.wrt_canonical_basis(&root_of_unity)
            .into_iter()
            .map(|x| red_map.smallest_lift(x)),
    );
    ...
    for _ in 0..e {
        let delta = S
            .checked_div(
                ...
                &S.inclusion().mul_map(
                    S.pow(S.clone_el(&result), m - 1),
                    S.base_ring().coerce(&ZZi64, m as i64),
                ),
            )
            .unwrap();
...
type FpPolyRing<R> = DensePolyRing<
    ...
    Global,
    DynConvolutionAlgorithmConvolution<
        AsFieldBase<RingValue<BaseRing<R>>>,
        Arc<dyn DynConvolutionAlgorithm<AsFieldBase<RingValue<BaseRing<R>>>>>,
    >,
>;
type ZpePolyRing<R> = DensePolyRing<
    ...
    Global,
    DynConvolutionAlgorithmConvolution<
        AsLocalPIRBase<RingValue<BaseRing<R>>>,
        Arc<dyn DynConvolutionAlgorithm<AsLocalPIRBase<RingValue<BaseRing<R>>>>>,
    >,
>;
type TmpSlotRingOf<'a, R> = AsLocalPIR<
    FreeAlgebraImpl<
        AsLocalPIR<RingRef<'a, BaseRing<R>>>,
        SparseMapVector<AsLocalPIR<RingRef<'a, BaseRing<R>>>>,
        Global,
        DynConvolutionAlgorithmConvolution<
            AsLocalPIRBase<RingRef<'a, BaseRing<R>>>,
            Arc<dyn DynConvolutionAlgorithm<AsLocalPIRBase<RingRef<'a, BaseRing<R>>>>>,
        >,
    >,
>;
...
/// the slot ring.
pub type SlotRingOver<R> = AsLocalPIR<
    FreeAlgebraImpl<
        R,
        Vec<El<R>>,
        Global,
        DynConvolutionAlgorithmConvolution<
            <R as RingStore>::Type,
            Arc<dyn DynConvolutionAlgorithm<<R as RingStore>::Type>>,
        >,
    >,
>;
...
///  - you can serialize the isomorphism including the implementation of `Fp[X]/(Phi_m(X))` (if the
///    latter is serializable) using the implementation of [`serde::Serialize`] and
///    [`serde::Deserialize`]
///  - alternatively, you can serialize the isomorphism without the ring implementation using
///    [`SerializeDeserializeWith`]
    ...
    BaseRing<R>: NiceZn,
{
    ...
    ///  - `slot_ring_moduli` is a list of all factors of `Phi_m(X) mod p`, ordered such as to be
    ///    compatible with the ordering of the slots, as given by [`HypercubeStructure`].
    #[instrument(skip_all)]
    pub fn create(
        ring: R,
        hypercube_structure: HypercubeStructure,
        ZpeX: ZpePolyRing<R>,
        slot_ring_moduli: Vec<El<ZpePolyRing<R>>>,
    ) -> Self {
        ...
        assert!(
            hypercube_structure.galois_group().eq_el(
                &frobenius,
                &hypercube_structure.galois_group().from_ring_el(
                    hypercube_structure
                        .galois_group()
                        .underlying_ring()
                        .coerce(&ZZbig, ZZbig.clone_el(&p))
                )
            )
        );

        let ring_ref = &ring;
        let convolution = create_convolution(
            d,
            ring_ref
                .base_ring()
                .integer_ring()
                .abs_log2_ceil(ring_ref.base_ring().modulus())
                .unwrap(),
        );
        let slot_rings: Vec<SlotRingOf<R>> = span!(Level::INFO, "compute_slot_rings").in_scope(|| {
            slot_ring_moduli
                ...
                .map(|f| {
                    ...
                    let modulus = (0..d)
                        .map(|i| ring_ref.base_ring().negate(unwrap.map_ref(ZpeX.coefficient_at(f, i))))
                        .collect::<Vec<_>>();
                    ...
                    let max_ideal_gen = slot_ring
                        .inclusion()
                        .map(slot_ring.base_ring().coerce(&ZZbig, ZZbig.clone_el(&p)));
                    return SlotRingOf::<R>::from(AsLocalPIRBase::promise_is_local_pir(
                        slot_ring,
                        max_ideal_gen,
                        Some(e),
                    ));
        ...
        let interpolation = span!(Level::INFO, "compute_interpolation_data")
            .in_scope(|| FastPolyInterpolation::new(ZpeX, slot_ring_moduli));

        let slot_generator_powers =
            Self::compute_slot_generator_powers(interpolation.poly_ring(), &hypercube_structure, &slot_rings);

        return Self {
            slot_generator_powers,
            hypercube_structure,
            ring,
            e,
    ...
    {
        assert!(
            self.galois_group().get_group() == new_ring.acting_galois_group().get_group(),
            "both rings must have the same galois structure"
        );
        let (p, e) = is_prime_power(&ZZbig, &new_ring.characteristic(&ZZbig).unwrap()).unwrap();
        let d = self.hypercube().d();
        let red_map = ZnReductionMap::new(self.ring().base_ring(), new_ring.base_ring())
            .expect("new ring must have modulus dividing current modulus");
                ...
                let new_slot_ring = FreeAlgebraImpl::new_with_convolution(
                    ...
                    (0..d)
                        .map(|i| {
                            new_ring
                                .base_ring()
                                .negate(new_ring.base_ring().clone_el(poly_ring.coefficient_at(&gen_poly, i)))
                        })
                        .collect::<Vec<_>>(),
                    "𝝵",
                    Global,
                    create_convolution(
                        ...
                        new_ring
                            .base_ring()
                            .integer_ring()
                            .abs_log2_ceil(new_ring.base_ring().modulus())
                            .unwrap(),
                    ),
                );
                let max_ideal_gen = new_slot_ring
                    .inclusion()
                    .map(new_slot_ring.base_ring().coerce(&ZZbig, ZZbig.clone_el(&p)));
        ...
        let Zpe: RingValue<DecoratedBaseRingBase<RNew>> =
            AsLocalPIR::from_zn(RingValue::from(new_ring.base_ring().get_ring().clone())).unwrap();
        let convolution = create_convolution(
            new_ring.rank(),
            Zpe.integer_ring().abs_log2_ceil(Zpe.modulus()).unwrap(),
        );
        ...
        let slot_generator_powers = HypercubeIsomorphism::<RNew>::compute_slot_generator_powers(
            interpolation.poly_ring(),
            self.hypercube(),
            &slot_rings,
        );
    ...
    pub fn allocation_size(&self) -> usize { self.slot_to_ring_interpolation.allocation_size() }

    pub fn hypercube(&self) -> &HypercubeStructure { &self.hypercube_structure }
    ...
    pub fn p(&self) -> &GaloisGroupEl { self.hypercube().p() }

    pub fn e(&self) -> usize { self.e }

    pub fn d(&self) -> usize { self.hypercube_structure.d() }

    pub fn galois_group(&self) -> &Subgroup<CyclotomicGaloisGroup> { self.hypercube_structure.galois_group() }

    pub fn slot_count(&self) -> usize { self.hypercube_structure.element_count() }

    #[instrument(skip_all)]
    pub fn get_slot_value(&self, el: &El<R>, slot_index: &GaloisGroupEl) -> El<SlotRingOf<R>> {
        let el = self
            .ring()
            .apply_galois_action(el, &self.galois_group().inv(slot_index));
        ...
        let poly_modulus = self
            .slot_ring()
            .generating_poly(&poly_ring, self.ring.base_ring().identity());
        let (_, rem) = poly_ring.div_rem_monic(el_as_poly, &poly_modulus);
        self.slot_ring().from_canonical_basis(
            (0..self.d()).map(|i| poly_ring.base_ring().clone_el(poly_ring.coefficient_at(&rem, i))),
        )
    }

    pub fn get_slot_values<'a>(
        &'a self,
        el: &'a El<R>,
    ) -> impl ExactSizeIterator<Item = El<SlotRingOf<R>>> + use<'a, R> {
        self.hypercube_structure
            .element_iter()
            .map(move |g| self.get_slot_value(el, &g))
    }

    #[instrument(skip_all)]
    fn compute_slot_generator_powers(
        poly_ring: &ZpePolyRing<R>,
        hypercube_structure: &HypercubeStructure,
        slot_rings: &[SlotRingOf<R>],
    ) -> Vec<Vec<El<ZpePolyRing<R>>>> {
        let wrap = WrapHom::to_delegate_ring(poly_ring.base_ring().get_ring());
        hypercube_structure
            .element_iter()
            .zip(slot_rings.iter())
            .map(|(g, S)| {
                let image_zeta = S.pow(
                    S.canonical_gen(),
                    hypercube_structure.galois_group().representative(&g) as usize,
                );
                (0..hypercube_structure.d())
                    .scan(S.one(), |current, _| {
        ...
        let result = values_it
            ...
            .map(|(i, a)| {
                ...
                for (c, zeta_pow) in a_wrt_basis.iter().zip(self.slot_generator_powers[i].iter()) {
                    poly_ring.add_assign(
                        &mut check,
                        poly_ring.inclusion().mul_ref_map(&zeta_pow, &wrap.map_ref(&c)),
                    );
            ...
            .collect::<Vec<_>>();
        assert!(
            values_it.next().is_none(),
            "iterator should only have {} elements",
            self.slot_count()
        );
        return result;
    }
        ...
        I: IntoIterator<Item = El<SlotRingOf<R>>>,
    {
        ...
        if let Some(deg) = poly_ring.degree(&unreduced_result) {
            let result = self.ring().from_canonical_basis_extended(
                (0..(deg + 1)).map(|i| hom.map_ref(poly_ring.coefficient_at(&unreduced_result, i))),
            );
            result
        } else {
            self.ring().zero()
        ...
        BaseRing<R>: SerializableElementRing,
    {
        ...
        assert!(
            result.hypercube_structure == *hypercube_structure,
            "hypercube structure mismatch"
        );
        ...
        R: Clone,
    {
        ...
        assert_el_eq!(
            ...
            result
                .slot_ring()
                .generating_poly(&poly_ring, poly_ring.base_ring().identity())
        );
    ...
    #[instrument(skip_all)]
    fn convert_factor_of_generating_poly_mod_p<P>(
        ring: &R,
        poly_ring: P,
        factor: &El<P>,
    ) -> (FpPolyRing<R>, El<FpPolyRing<R>>)
    where
        P: RingStore,
        P::Type: PolyRing,
        <P::Type as RingExtension>::BaseRing: RingStore<Type = BaseRing<R>>,
        R: Clone,
    {
        ...
        let Fp = RingValue::from(
            <BaseRing<R> as FromModulusCreateableZnRing>::from_modulus::<_, !>(|ZZ| {
        ...
        assert!(
            FpX.divides(&ring.generating_poly(&FpX, &hom), &factor),
            "invalid factor"
        );
        return (FpX, factor);
    }
    ...
    #[instrument(skip_all)]
    fn compute_factor_of_generating_poly_mod_p(
        ring: &R,
        hypercube_structure: &HypercubeStructure,
    ) -> (FpPolyRing<R>, El<FpPolyRing<R>>) {
        let m = ring.acting_galois_group().m() as usize;
        assert!(
            ring.is_one(&ring.pow(ring.canonical_gen(), m)),
            "HypercubeIsomorphism currently assumes that the generator of the ring is an m-th root of unity"
        );
        ...
        let Fq = span!(Level::INFO, "create_galois_field").in_scope(|| {
            GaloisField::new_with_convolution(
                Fp,
                d,
                Global,
                create_convolution(d, Fp.integer_ring().abs_log2_ceil(Fp.modulus()).unwrap()),
            )
        });
        ...
        let root = (0..m)
            ...
            .filter(|x| Fq.is_zero(&FpX.evaluate(&gen_poly, x, Fq.inclusion())))
            .next()
            .unwrap();

        let mut result = FqX.prod((0..d).scan(root, |current_root, _| {
    ...
    #[instrument(skip_all)]
    fn convert_tmp_slot_ring_and_root<'a, P>(
        ring: &'a R,
        poly_ring: P,
        factor: &El<P>,
    ) -> (TmpSlotRingOf<'a, R>, El<TmpSlotRingOf<'a, R>>)
    where
        P: RingStore,
        P::Type: PolyRing,
        <P::Type as RingExtension>::BaseRing: RingStore<Type = BaseRing<R>>,
        R: Clone,
    {
        ...
        assert!(
            S.is_zero(
                &poly_ring.evaluate(
                    &ring.generating_poly(&poly_ring, poly_ring.base_ring().identity()),
                    &root_of_unity,
                    S.inclusion()
                        .compose(WrapHom::to_delegate_ring(S.base_ring().get_ring()))
                )
            ),
            "invalid factor"
        );
    ...
    /// has a generator which is a root of unity.
    #[instrument(skip_all)]
    fn compute_tmp_slot_ring_and_root<'a>(
        ring: &'a R,
        hypercube_structure: &HypercubeStructure,
    ) -> (TmpSlotRingOf<'a, R>, El<TmpSlotRingOf<'a, R>>) {
        let m = ring.acting_galois_group().m() as usize;
        ...
        let Fq = span!(Level::INFO, "create_galois_field")
            .in_scope(|| GaloisField::new_with_convolution(Fp, d, Global, convolution));
        let convolution = create_convolution(
            d,
            ring.base_ring()
                .integer_ring()
                .abs_log2_ceil(ring.base_ring().modulus())
                .unwrap(),
        );
        let S = span!(Level::INFO, "create_galois_ring").in_scope(|| {
            let base_ring: AsLocalPIR<RingRef<BaseRing<R>>> =
                AsLocalPIR::<RingRef<_>>::from_zn(RingRef::new(ring.base_ring().get_ring())).unwrap();
            Fq.get_ring().galois_ring_with(base_ring, Global, convolution)
        });

        let root_of_unity = span!(Level::INFO, "compute_root_of_unity")
            .in_scope(|| hensel_lift_root_of_unity(&S, &Fq, get_prim_root_of_unity(&Fq, m).unwrap(), m));
        ...
        let root = span!(Level::INFO, "find_genpoly_root").in_scope(|| {
            (0..m)
                ...
                .filter(|x| S.is_zero(&ZpeX.evaluate(&gen_poly, x, S.inclusion())))
                .next()
                .unwrap()
        });
    ...
    #[instrument(skip_all)]
    fn compute_slot_ring_moduli_small_slot_ring<'a>(
        ring: &R,
        hypercube_structure: &HypercubeStructure,
        S: TmpSlotRingOf<'a, R>,
        root: El<TmpSlotRingOf<'a, R>>,
    ) -> (ZpePolyRing<R>, Vec<El<ZpePolyRing<R>>>) {
        let m = ring.acting_galois_group().m() as usize;
        assert!(
            ring.is_one(&ring.pow(ring.canonical_gen(), m)),
            "HypercubeIsomorphism currently assumes that the generator of the ring is an m-th root of unity"
        ...
        let Zpe: RingValue<DecoratedBaseRingBase<R>> =
            AsLocalPIR::from_zn(RingValue::from(ring.base_ring().get_ring().clone())).unwrap();
        let convolution = create_convolution(
            ring.rank(),
            ring.base_ring()
                .integer_ring()
        ...
        let slot_ring_moduli = span!(Level::INFO, "factor_cyclotomic_poly").in_scope(|| {
            ...
            for g in hypercube_structure.element_iter() {
                let mut result = SX.prod((0..d).scan(
                    S.pow(
                        S.clone_el(&root),
                        galois_group.representative(&galois_group.inv(&g)) as usize,
                    ),
                    |current_root, _| {
                ...
                let rewrap = WrapHom::to_delegate_ring(ZpeX.base_ring().get_ring())
                    .compose(UnwrapHom::from_delegate_ring(S.base_ring().get_ring()));
    ...
    /// elements as enumerated by `hypercube_structure`.
    #[instrument(skip_all)]
    fn compute_slot_ring_moduli_large_slot_ring(
        ring: &R,
        hypercube_structure: &HypercubeStructure,
        FpX: &FpPolyRing<R>,
        factor: &El<FpPolyRing<R>>,
    ) -> (ZpePolyRing<R>, Vec<El<ZpePolyRing<R>>>) {
        ...
        let gen_poly_mod_pe = ZpeX_undecorated
            .lifted_hom(
                &ZZX,
                ZpeX_undecorated
                    .base_ring()
                    .can_hom(ring.base_ring())
                    .unwrap()
                    .compose(ring.base_ring().can_hom(&ZZX.base_ring()).unwrap()),
            )
            .map_ref(&gen_poly);
        let gen_poly_mod_p = FpX
        ...
        let slot_ring_moduli = span!(Level::INFO, "factor_cyclotomic_poly").in_scope(|| {
            ...
            for g in hypercube_structure.element_iter() {
                let factor_conjugate = FpX.from_terms(FpX.terms(&factor).map(|(c, i)| {
                    (
                        FpX.base_ring().clone_el(c),
                        Zm.smallest_positive_lift(Zm.mul(
                            *ring.number_ring().galois_group().as_ring_el(&g),
                            Zm.coerce(&ZZi64, i as i64),
                        )) as usize,
                    )
                }));

                let ZZ = IntegersWithLocalZnQuotient::<BaseRing<R>>::new(
                    Zpe.integer_ring(),
                    Zpe.integer_ring().clone_el(&p),
                );
                ...
                let [lifted_factor, _] = hensel_lift_factorization(
                    &reduction_map,
                    &ZpeX_undecorated,
                    &FpX,
                    &gen_poly_mod_pe,
                ...
                result.push(
                    ZpeX.lifted_hom(&ZpeX_undecorated, WrapHom::to_delegate_ring(Zpe.get_ring()))
                        .map(lifted_factor),
                );
            }
            return result;
    ...
    BaseRing<R>: NiceZn,
{
    fn deserialize_with_data<'de, D: serde::Deserializer<'de>>(ring: R, deserializer: D) -> Result<Self, D::Error> {
        ...
        fn derive_multiple_poly_deserializer<'de, 'a, R>(
            deserializer: &'a DeserializeSeedHypercubeIsomorphismData<R>,
        ) -> impl use<'a, 'de, R> + DeserializeSeed<'de, Value = Vec<El<R>>>
        where
            R: RingStore,
            R::Type: PolyRing + SerializableElementRing,
        {
        ...
        assert!(
            ring.acting_galois_group().get_group() == deserialized.hypercube_structure.galois_group().get_group(),
            "ring mismatch"
        );
        assert!(
            ZZbig.eq_el(&ring.characteristic(ZZbig).unwrap(), &deserialized.characteristic),
            "ring mismatch"
        );

        let hypercube_structure = deserialized.hypercube_structure;
        let slot_ring_moduli = deserialized.slot_ring_moduli;
        let result = HypercubeIsomorphism::create(ring, hypercube_structure, ZpeX, slot_ring_moduli);
        return Ok(result);
    }
    ...
    fn serialize_with_data<S: serde::Serializer>(&self, _: &R, serializer: S) -> Result<S::Ok, S::Error> {
        ...
        let decorated_base_ring =
            AsLocalPIR::from_zn(RingValue::from(self.ring().base_ring().get_ring().clone())).unwrap();
        ...
        SerializableHypercubeIsomorphismData {
            ...
            slot_ring_moduli: (0..self.slot_count())
                .map(|i| SerializeOwnedWithRing::new(self.slot_ring_at(i).generating_poly(&ZpeX, &hom), &ZpeX))
                .collect(),
        }
    ...
    BaseRing<R>: NiceZn + SerializableElementRing,
{
        ...
        S: serde::Serializer,
    {
        SerializableNewtypeStruct::new(
            "HypercubeIsomorphism",
            (self.ring(), SerializeSerializableWithData::new(self.ring(), self)),
        )
        .serialize(serializer)
    }
}
        ...
        D: serde::Deserializer<'de>,
    {
        DeserializeSeedNewtypeStruct::new(
            ...
            DeserializeSeedDependentTuple::new(PhantomData::<R>, |ring| {
                DeserializeSeedDeserializableWithData::new(ring)
            }),
        )
...
#[cfg(test)]
fn test_ring1() -> (
    NumberRingQuotientByInt<Pow2CyclotomicNumberRing, zn_64::Zn>,
    HypercubeStructure,
) {
...
fn test_ring2() -> (
    ...
    let galois_group = CyclotomicGaloisGroupBase::new(32);
    let gs = vec![
        galois_group.from_representative(5),
        galois_group.from_representative(-1),
    ];
...
#[cfg(test)]
fn test_ring3() -> (
    NumberRingQuotientByInt<TensorProductNumberRing, zn_64::Zn>,
    HypercubeStructure,
) {
    let galois_group = CyclotomicGaloisGroupBase::new(11 * 13);
    let p = galois_group.from_representative(3);
    let gs = vec![
        galois_group.from_representative(79),
        galois_group.from_representative(67),
    ];
    let hypercube_structure = HypercubeStructure::new(galois_group.into().full_subgroup(), p, 15, vec![2, 4], gs);
...
#[cfg(test)]
fn test_ring4() -> (
    NumberRingQuotientByIdeal<Pow2CyclotomicNumberRing, zn_64::Zn>,
    HypercubeStructure,
) {
    ...
    let acting_galois_group = galois_group
        .get_group()
        .clone()
        .subgroup([galois_group.from_representative(17)]);
...
fn test_hypercube_isomorphism_from_to_slot_vector() {
        ...
        DecoratedBaseRingBase<R>: CanIsoFromTo<BaseRing<R>>,
    {
        ...
        for _ in 0..10 {
            ...
            let expected = (0..isomorphism.slot_count())
                .map(|_| slot_ring.random_element(|| rng.rand_u64()))
                .collect::<Vec<_>>();
...
fn test_hypercube_isomorphism_is_isomorphic() {
        ...
        DecoratedBaseRingBase<R>: CanIsoFromTo<BaseRing<R>>,
    {
        ...
        for _ in 0..10 {
            let slot_ring = isomorphism.slot_ring();
            let lhs = (0..isomorphism.slot_count())
                .map(|_| slot_ring.random_element(|| rng.rand_u64()))
                .collect::<Vec<_>>();
            let rhs = (0..isomorphism.slot_count())
                .map(|_| slot_ring.random_element(|| rng.rand_u64()))
                .collect::<Vec<_>>();
            let expected = (0..isomorphism.slot_count())
                .map(|i| slot_ring.mul_ref(&lhs[i], &rhs[i]))
                .collect::<Vec<_>>();
...
fn test_hypercube_isomorphism_rotation() {
        ...
        DecoratedBaseRingBase<R>: CanIsoFromTo<BaseRing<R>>,
    {
        ...
        for _ in 0..10 {
            ...
            let mut input = (0..isomorphism.slot_count())
                .map(|_| slot_ring.zero())
                .collect::<Vec<_>>();
            input[0] = slot_ring.clone_el(&a);
            let input = isomorphism.from_slot_values(input.into_iter());

            let mut expected = (0..isomorphism.slot_count())
                .map(|_| slot_ring.zero())
                .collect::<Vec<_>>();
            expected[(hypercube.dim_length(0) - 1) * hypercube.element_count() / hypercube.dim_length(0)] =
                slot_ring.clone_el(&a);

            let actual = ring.apply_galois_action(
                &input,
                &hypercube.galois_group().pow(
                    hypercube.dim_generator(0),
                    &int_cast(hypercube.dim_length(0) as i64 - 1, ZZbig, ZZi64),
                ),
            );
...
fn test_serialization() {
        ...
        BaseRing<R>: NiceZn + SerializableElementRing + CanIsoFromTo<zn_64::ZnBase>,
    {
        ...
        assert_el_eq!(
            hypercube.ring(),
            hypercube
                .from_slot_values((0..hypercube.slot_count()).map(|i| hypercube.slot_ring().int_hom().map(i as i32))),
            deserialized_hypercube.from_slot_values(
                (0..deserialized_hypercube.slot_count())
                    .map(|i| deserialized_hypercube.slot_ring().int_hom().map(i as i32))
            )
        );
