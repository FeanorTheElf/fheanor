    ...
    <<P::Type as RingExtension>::BaseRing as RingStore>::Type: NiceZn,
{
    ...
    let mod_f_ring = FreeAlgebraImpl::new(
        poly_ring.base_ring(),
        deg_f,
        (0..deg_f)
            .map(|i| {
    ...
    let g_mod_f = mod_f_ring.from_canonical_basis(
        (0..deg_f).map(|i| poly_ring.base_ring().clone_el(poly_ring.coefficient_at(&g_mod_f, i))),
    );

    let normalization_factor = invert_over_local_zn(RingRef::new(mod_f_ring.get_ring()), &g_mod_f);

    assert!(
        normalization_factor.is_some(),
        "crt unit vector modulo {} and {} does not exist",
        poly_ring.format(f),
        poly_ring.format(g)
    );
    debug_assert!(mod_f_ring.is_one(&mod_f_ring.mul_ref(normalization_factor.as_ref().unwrap(), &g_mod_f)));
    let g_mod_f_inv = mod_f_ring.poly_repr(
        &poly_ring,
        &normalization_factor.unwrap(),
    ...
    <<P::Type as RingExtension>::BaseRing as RingStore>::Type: NiceZn,
{
        ...
        <<PNew::Type as RingExtension>::BaseRing as RingStore>::Type: NiceZn,
    {
        ...
        let unit_vectors = self
            .unit_vectors
            .iter()
            .map(|list| {
                list.iter()
                    .map(|(e0, e1)| (lifted_red_map.map_ref(e0), lifted_red_map.map_ref(e1)))
                    .collect()
            })
            .collect();
    ...
    pub fn allocation_size(&self) -> usize {
        self.unit_vectors
            ...
            .map(|vec| {
                vec.iter()
                    .map(|(l, r)| {
                        (self.poly_ring().degree(l).unwrap_or(0) + self.poly_ring().degree(r).unwrap_or(0) + 2)
                            * size_of::<El<<P::Type as RingExtension>::BaseRing>>()
                    })
                    .sum::<usize>()
            })
            .sum()
    ...
    pub fn poly_ring(&self) -> &P { &self.poly_ring }

    pub fn final_modulus(&self) -> &El<P> { &self.final_modulus }
    ...
    pub fn interpolate_unreduced(&self, remainders: Vec<El<P>>) -> El<P> {
        ...
        for i in 0..self.unit_vectors.len() {
            ...
            for ([f0, f1], (e0, e1)) in chunks.iter().zip(self.unit_vectors[i].iter()) {
                new.push(
                    self.poly_ring
                        .add(self.poly_ring.mul_ref(f0, e0), self.poly_ring.mul_ref(f1, e1)),
                );
            }
        ...
        debug_assert!(
            self.poly_ring.degree(&result).unwrap_or(0) < (self.input_degree << (self.unit_vectors.len() + 1))
        );
        return result;
    }
}
...
fn test_interpolate() {
    ...
    let remainders = poly_ring
        .with_wrapped_indeterminate(|X| [-1 * X + 3, -5 * X.pow_ref(2) + 21 * X - 12, -728 * X + 16, 720896 * X]);
    ...
    for i in 0..4 {
        assert_el_eq!(
            &poly_ring,
            &remainders[i],
            poly_ring.div_rem_monic(poly_ring.clone_el(&result), &moduli[i]).1
        );
    }

    let moduli =
        poly_ring.with_wrapped_indeterminate(|X| [X.pow_ref(2) - 1, X.pow_ref(3) + X - 1, X.pow_ref(2) - X + 2]);
    let remainders =
        poly_ring.with_wrapped_indeterminate(|X| [-1 * X + 3, -5 * X.pow_ref(2) + 21 * X - 12, -728 * X + 16]);
    let interpolation = FastPolyInterpolation::new(&poly_ring, moduli.iter().map(|f| poly_ring.clone_el(f)).collect());
    let result = interpolation.interpolate_unreduced(remainders.iter().map(|f| poly_ring.clone_el(f)).collect());
    for i in 0..3 {
        assert_el_eq!(
