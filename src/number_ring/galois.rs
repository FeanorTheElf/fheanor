...
use feanor_math::algorithms::discrete_log::{Subgroup, multiplicative_order};
...
use feanor_math::group::{
    AbelianGroupBase, AbelianGroupStore, GroupValue, MultGroup, MultGroupEl, SerializableElementGroup,
};
...
impl CyclotomicGaloisGroupBase {
    pub fn new(m: u64) -> GroupValue<Self> {
        ...
        return GroupValue::from(Self { order, group });
    }

    pub fn subgroup<I>(self, generators: I) -> Subgroup<CyclotomicGaloisGroup>
    where
        I: IntoIterator<Item = GaloisGroupEl>,
    {
    ...
    pub fn full_subgroup(self) -> Subgroup<CyclotomicGaloisGroup> {
        ...
        return Subgroup::new(
            GroupValue::from(self.clone()),
            order,
            generators
                .into_iter()
                .map(|x| self.from_ring_el(self.underlying_ring().coerce(&ZZi64, x)))
                .collect(),
        );
    }

    /// Returns the ring `Z/mZ` whose units form this group.
    pub fn underlying_ring(&self) -> &Zn { self.group.underlying_ring() }

    /// Interprets a group element as ring element of `Z/mZ`.
    pub fn as_ring_el<'a>(&self, value: &'a GaloisGroupEl) -> &'a ZnEl { self.group.as_ring_el(&value.value) }
    ...
    pub fn from_ring_el(&self, el: ZnEl) -> GaloisGroupEl {
        GaloisGroupEl {
            value: self.group.from_ring_el(el).unwrap(),
        }
    }
    ...
    /// this value of `m`.
    pub fn m(&self) -> u64 { *self.underlying_ring().modulus() as u64 }

    /// Returns the order of a group element, i.e. the smallest `k` such that
    /// `value^k = 1`.
    pub fn element_order(&self, value: &GaloisGroupEl) -> usize {
        multiplicative_order(*self.as_ring_el(value), self.underlying_ring()) as usize
    }
    ...
    pub fn group_order(&self) -> usize { self.order }
}
...
impl CyclotomicGaloisGroupOps for CyclotomicGaloisGroup {
    fn underlying_ring(&self) -> &Zn { self.get_group().underlying_ring() }

    fn as_ring_el<'a>(&self, value: &'a GaloisGroupEl) -> &'a ZnEl { self.get_group().as_ring_el(value) }

    fn from_ring_el(&self, el: ZnEl) -> GaloisGroupEl { self.get_group().from_ring_el(el) }

    fn m(&self) -> u64 { self.get_group().m() }

    fn element_order(&self, value: &GaloisGroupEl) -> usize { self.get_group().element_order(value) }

    fn from_representative(&self, x: i64) -> GaloisGroupEl { self.get_group().from_representative(x) }

    fn representative(&self, x: &GaloisGroupEl) -> u64 { self.get_group().representative(x) }

    fn group_order(&self) -> usize { self.get_group().group_order() }

    fn is_full_cyclotomic_galois_group(&self) -> bool { true }
}

impl CyclotomicGaloisGroupOps for Subgroup<CyclotomicGaloisGroup> {
    fn as_ring_el<'a>(&self, value: &'a GaloisGroupEl) -> &'a ZnEl { self.parent().as_ring_el(value) }

    fn element_order(&self, value: &GaloisGroupEl) -> usize { self.parent().element_order(value) }
    ...
    fn m(&self) -> u64 { self.parent().m() }

    fn group_order(&self) -> usize { int_cast(self.subgroup_order(), ZZi64, ZZbig) as usize }

    fn representative(&self, x: &GaloisGroupEl) -> u64 { self.parent().representative(x) }

    fn underlying_ring(&self) -> &Zn { self.parent().underlying_ring() }

    fn is_full_cyclotomic_galois_group(&self) -> bool {
        ZZbig.eq_el(
            &self.subgroup_order(),
            &int_cast(self.parent().group_order() as i64, ZZbig, ZZi64),
        )
    }
}

impl PartialEq for CyclotomicGaloisGroupBase {
    fn eq(&self, other: &Self) -> bool { self.group.get_group() == other.group.get_group() }
}
...
impl AbelianGroupBase for CyclotomicGaloisGroupBase {
    ...
    fn clone_el(&self, x: &Self::Element) -> Self::Element { x.clone() }

    fn eq_el(&self, lhs: &Self::Element, rhs: &Self::Element) -> bool { self.group.eq_el(&lhs.value, &rhs.value) }

    fn hash<H: std::hash::Hasher>(&self, x: &Self::Element, hasher: &mut H) { self.group.hash(&x.value, hasher) }

    fn inv(&self, x: &Self::Element) -> Self::Element {
        GaloisGroupEl {
            value: self.group.inv(&x.value),
        }
    }

    fn op(&self, lhs: Self::Element, rhs: Self::Element) -> Self::Element {
        GaloisGroupEl {
            value: self.group.op(lhs.value, rhs.value),
        }
    }

    fn identity(&self) -> Self::Element {
        GaloisGroupEl {
            value: self.group.identity(),
        }
    }
}
...
impl SerializableElementGroup for CyclotomicGaloisGroupBase {
    ...
    where
        D: Deserializer<'de>,
    {
        DeserializeSeedNewtypeStruct::new("GaloisGroupEl", PhantomData::<u64>)
            .deserialize(deserializer)
            .map(|x| self.from_representative(x as i64))
    }
}
...
pub fn get_multiplicative_generator(ring: Zn) -> ZnEl {
    ...
    assert!(
        *ring.modulus() % 2 == 1,
        "for even m, Z/mZ* is either equal to Z/(m/2)Z* or not cyclic"
    );
