...
use crate::{DefaultNegacyclicNTT, FheanorAllocator};
...
impl Pow2CyclotomicNumberRing {
    pub fn new(m: u64) -> Self { Self::new_with_ntt(m) }
}

impl<N> Pow2CyclotomicNumberRing<N>
where
    N: FheanorNegacyclicNTT<zn_64::Zn>,
{
...
where
    N: FheanorNegacyclicNTT<zn_64::Zn>,
{
    fn clone(&self) -> Self { Self::new_with_ntt(1 << self.log2_m) }
}

impl<N> PartialEq for Pow2CyclotomicNumberRing<N> {
    fn eq(&self, other: &Self) -> bool { self.log2_m == other.log2_m }
}
    ...
    N: FheanorNegacyclicNTT<zn_64::Zn>,
{
    ...
    fn small_basis_product_expansion_factor(&self) -> f64 { self.rank() as f64 }
    ...
    fn mod_p_required_root_of_unity(&self) -> u64 { return 1 << self.log2_m; }
    ...
    {
        poly_ring.add(
            poly_ring.pow(poly_ring.indeterminate(), 1 << (self.log2_m - 1)),
            poly_ring.one(),
        )
    }

    fn rank(&self) -> usize { 1 << (self.log2_m - 1) }
}
...
where
    ...
    A: FheanorAllocator,
{
    fn eq(&self, other: &Self) -> bool { self.ntt == other.ntt }
}
    ...
    A: FheanorAllocator,
{
        ...
        V2: SwappableVectorViewMut<zn_64::ZnEl>,
    {
        ...
        // the elements of src resp. dst follow an order derived from the bitreversing order of the
        // underlying FFT
        ...
        for i in 0..self.rank() {
            ...
            *dst.at_mut(i) = *src.at(galois_el_to_index(
                (galois_el_repr * index_to_galois_el(i)) % (1 << self.log2_m),
            ));
        }
    }
    ...
    where
        V: SwappableVectorViewMut<zn_64::ZnEl>,
    {
        ...
        self.ntt
            .bitreversed_negacyclic_fft_base::<true>(&mut input[..], &mut output[..]);
        ...
        V: SwappableVectorViewMut<zn_64::ZnEl>,
    {
        ...
        output.resize_with(data.len(), || self.ntt.ring().zero());
        self.ntt
            .bitreversed_negacyclic_fft_base::<false>(&mut input[..], &mut output[..]);
        for (i, x) in output.into_iter().enumerate() {
            *data.at_mut(i) = x;
        }
    }
    ...
    fn rank(&self) -> usize { self.ntt.len() }

    fn base_ring(&self) -> &zn_64::Zn { RingValue::from_ref(self.ntt.ring().into()) }
}
...
fn test_permute_galois_automorphism() {
    ...
    assert_el_eq!(
        R,
        R.pow(R.canonical_gen(), 3),
        R.get_ring()
            .apply_galois_action(&R.canonical_gen(), &R.acting_galois_group().from_representative(3))
    );
    assert_el_eq!(
        R,
        R.pow(R.canonical_gen(), 6),
        R.get_ring().apply_galois_action(
            &R.pow(R.canonical_gen(), 2),
            &R.acting_galois_group().from_representative(3)
        )
    );
}
...
fn bench_permute_galois_action(bencher: &mut test::Bencher) {
    ...
    bencher.iter(|| {
        number_ring_mod_p.permute_galois_action(
            std::hint::black_box(&input),
            &mut output,
            &number_ring.galois_group().from_representative(5),
        );
