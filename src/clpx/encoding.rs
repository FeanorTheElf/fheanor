...
use crate::{FheanorAllocator, NiceZn, ZZbig, ZZi64};
...
pub type CLPXPlaintextRing<NumberRing, ZnTy, A = Global, C = KaratsubaAlgorithm> =
    RingValue<CLPXPlaintextRingBase<NumberRing, ZnTy, A, C>>;
    ...
    C: ConvolutionAlgorithm<ZnTy::Type>,
{
    ...
    pub fn create(
        ...
        let norm = span!(Level::INFO, "resultant").in_scope(|| {
            ZZbig.abs(<_ as ComputeResultantRing>::resultant(
                &poly_ring,
                poly_ring.clone_el(&gen_poly),
                poly_ring.clone_el(&t),
            ))
        });
        let rest = if let Some(rest) = ZZbig.checked_div(
            &norm,
            &ZZbig.pow(
                base_ring.characteristic(ZZbig).unwrap(),
                int_cast(acting_galois_group.subgroup_order(), ZZi64, ZZbig) as usize,
            ),
        ) {
            rest
        } else {
            panic!(
                "the given ideal has norm {}, which is not divisible by {}^{}",
                ZZbig.format(&norm),
        ...
        if !ZZbig.is_one(&signed_gcd(rest, base_ring.characteristic(ZZbig).unwrap(), ZZbig)) {
            panic!(
                "the given ideal has norm {}, which is divisible by more than {} (= size of acting Galois group) powers of {}; note that the Galois group must have rank equal to the rank of the quotient ring",
                ZZbig.format(&norm),
                int_cast(acting_galois_group.subgroup_order(), ZZi64, ZZbig),
                ZZbig.format(&base_ring.characteristic(ZZbig).unwrap())
            )
        ...
        let (mut s, _, d) = span!(Level::INFO, "compute_t_inv")
            .in_scope(|| QQX.extended_ideal_gen(&ZZX_to_QQX.map_ref(&t), &ZZX_to_QQX.map_ref(&gen_poly)));
        assert_eq!(0, QQX.degree(&d).unwrap());
        QQX.inclusion().mul_assign_map(
            &mut s,
            QQ.div(&QQ.inclusion().map_ref(&norm), QQX.coefficient_at(&d, 0)),
        );
        let normt_t_inv = poly_ring.from_terms(
            QQX.terms(&s)
                .map(|(c, i)| (ZZbig.checked_div(QQ.num(c), QQ.den(c)).unwrap(), i)),
        );
        ...
        let base = NumberRingQuotientByIdealBase::create(
            number_ring,
            base_ringX,
            ideal_generator,
            acting_galois_group,
    ...
    pub fn t(&self) -> &El<DensePolyRing<BigIntRing>> { &self.t }

    pub fn normt_t_inv(&self) -> &El<DensePolyRing<BigIntRing>> { &self.normt_t_inv }

    pub fn normt(&self) -> &El<BigIntRing> { &self.normt }

    pub fn ZZX(&self) -> &DensePolyRing<BigIntRing> { &self.ZZX }
    ...
    pub fn reduce_mod_t(&self, el: &El<DensePolyRing<BigIntRing>>) -> <Self as RingBase>::Element {
        ...
        self.from_canonical_basis_extended(
            (0..=self.ZZX.degree(el).unwrap()).map(|i| hom.map_ref(self.ZZX.coefficient_at(el, i))),
        )
    }
    ...
    pub fn small_lift(&self, el: &<Self as RingBase>::Element) -> El<DensePolyRing<BigIntRing>> {
        let el_lift = self
            .ZZX
            .from_terms(self.wrt_canonical_basis(el).iter().enumerate().map(|(i, c)| {
                (
                    int_cast(
                        self.base_ring().smallest_lift(c),
                        ZZbig,
        ...
        let (_, normt_el_over_t) = self
            .ZZX
            .div_rem_monic(self.ZZX.mul_ref(&el_lift, &self.normt_t_inv), &self.gen_poly);
        let (_, closest_multiple_of_t) = self.ZZX.div_rem_monic(
            self.ZZX.mul_ref_snd(
                self.ZZX.from_terms(
                    self.ZZX
                        .terms(&normt_el_over_t)
                        .map(|(c, i)| (ZZbig.rounded_div(ZZbig.clone_el(c), &self.normt), i)),
                ),
                &self.t,
            ),
            &self.gen_poly,
        ...
        <<S::Type as RingExtension>::BaseRing as RingStore>::Type: ZnRing + CanHomFrom<BigIntRingBase>,
    {
        ...
        return target.from_canonical_basis((0..target.rank()).map(|i| {
            hom.map(ZZbig.rounded_div(
                ZZbig.mul_ref(&Q, self.ZZX.coefficient_at(&normt_el_over_t, i)),
                &self.normt,
            ))
        }));
    }
        ...
        <<S::Type as RingExtension>::BaseRing as RingStore>::Type: ZnRing,
    {
        ...
        let el_lift = self
            .ZZX
            .from_terms(from.wrt_canonical_basis(el).iter().enumerate().map(|(i, c)| {
                (
                    int_cast(
                        from.base_ring().smallest_lift(c),
                        ZZbig,
                        from.base_ring().integer_ring(),
                    ),
                    i,
                )
            }));
        let (_, el_t) = self
            .ZZX
            .div_rem_monic(self.ZZX.mul_ref(&el_lift, &self.t), &self.gen_poly);
        let hom = self.base_ring().can_hom(&ZZbig).unwrap();
        let Q = int_cast(
            from.base_ring().integer_ring().clone_el(from.base_ring().modulus()),
            ZZbig,
            from.base_ring().integer_ring(),
        );
        return self.from_canonical_basis_extended(
            (0..=self.ZZX.degree(&el_t).unwrap())
                .map(|i| hom.map(ZZbig.rounded_div(ZZbig.clone_el(self.ZZX.coefficient_at(&el_t, i)), &Q))),
        );
    }
}
...
impl<NumberRing, ZnTy1, ZnTy2, A1, A2, C1, C2> CanHomFrom<CLPXPlaintextRingBase<NumberRing, ZnTy2, A2, C2>>
    for CLPXPlaintextRingBase<NumberRing, ZnTy1, A1, C1>
where
    NumberRing: NumberRingDescriptor,
    ZnTy1: RingStore,
...
{
    type Homomorphism = <NumberRingQuotientByIdealBase<NumberRing, ZnTy1, A1, C1> as CanHomFrom<
        NumberRingQuotientByIdealBase<NumberRing, ZnTy2, A2, C2>,
    >>::Homomorphism;
    ...
    fn map_in(
        &self,
        from: &CLPXPlaintextRingBase<NumberRing, ZnTy2, A2, C2>,
        el: <CLPXPlaintextRingBase<NumberRing, ZnTy2, A2, C2> as RingBase>::Element,
    ...
    fn mul_assign_map_in(
        ...
        rhs: <CLPXPlaintextRingBase<NumberRing, ZnTy2, A2, C2> as RingBase>::Element,
        hom: &Self::Homomorphism,
    ) {
        self.base
            .get_ring()
            .mul_assign_map_in(from.base.get_ring(), lhs, rhs, hom)
    }
    ...
    fn mul_assign_map_in_ref(
        ...
        self.base
            .get_ring()
            .mul_assign_map_in_ref(from.base.get_ring(), lhs, rhs, hom)
    }
    ...
    fn fma_map_in(
        ...
        self.base
            .get_ring()
            .fma_map_in(from.base.get_ring(), lhs, rhs, summand, hom)
    }
}

impl<NumberRing, ZnTy1, ZnTy2, A1, A2, C1, C2> CanIsoFromTo<CLPXPlaintextRingBase<NumberRing, ZnTy2, A2, C2>>
    for CLPXPlaintextRingBase<NumberRing, ZnTy1, A1, C1>
where
    NumberRing: NumberRingDescriptor,
    ZnTy1: RingStore,
    ...
    C2: ConvolutionAlgorithm<ZnTy2::Type>,
{
    type Isomorphism = <NumberRingQuotientByIdealBase<NumberRing, ZnTy1, A1, C1> as CanIsoFromTo<
        NumberRingQuotientByIdealBase<NumberRing, ZnTy2, A2, C2>,
    >>::Isomorphism;
    ...
    ) -> <CLPXPlaintextRingBase<NumberRing, ZnTy2, A2, C2> as RingBase>::Element {
    ...
    C2: ConvolutionAlgorithm<ZnTy2::Type>,
{
    ...
    fn mul_assign_map_in(
        ...
        from: &NumberRingQuotientByIdealBase<NumberRing, ZnTy2, A2, C2>,
        lhs: &mut Self::Element,
        rhs: <NumberRingQuotientByIdealBase<NumberRing, ZnTy2, A2, C2> as RingBase>::Element,
        hom: &Self::Homomorphism,
    ) {
...
fn test_rns_base() -> zn_rns::Zn<zn_64::Zn, BigIntRing> {
    zn_rns::Zn::create_from_primes(
        vec![
            5598412801, 5665259521, 5698682881, 5715394561, 5732106241, 5771100161, 5821235201, 5899223041, 5921505281,
            5966069761, 6032916481, 6155468801,
        ],
        ZZbig,
    )
}
...
#[cfg(test)]
fn test_ring1() -> (
    CLPXPlaintextRing<Pow2CyclotomicNumberRing, zn_64::Zn>,
    Vec<El<CLPXPlaintextRing<Pow2CyclotomicNumberRing, zn_64::Zn>>>,
) {
    ...
    let result = CLPXPlaintextRingBase::create(
        number_ring,
        zn_64::Zn::new(65537),
        ZZX,
        t,
...
fn test_ring3() -> (
    ...
    let elements = (0..31)
        .map(|i| result.int_hom().map(1 << i))
        .chain((0..31).map(|i| result.mul(result.canonical_gen(), result.int_hom().map(1 << i))))
        .collect();
    return (result, elements);
}
...
#[cfg(test)]
fn test_ring4() -> (
    CLPXPlaintextRing<OddSquarefreeCyclotomicNumberRing, zn_64::Zn>,
    Vec<El<CLPXPlaintextRing<OddSquarefreeCyclotomicNumberRing, zn_64::Zn>>>,
) {
    let ZZX = DensePolyRing::new(ZZbig, "X");
    ...
    let acting_galois_group = number_ring
        ...
        .get_group()
        .clone()
        .subgroup([number_ring.galois_group().from_representative(18)]);
    let result = CLPXPlaintextRingBase::create(
        number_ring,
        zn_64::Zn::new(131071),
        ZZX,
    ...
    let elements = (0..15)
        .flat_map(|i| {
            from_fn::<_, 4, _>(|j| result.mul(result.pow(result.canonical_gen(), j), result.int_hom().map(1 << i)))
                .into_iter()
        })
        .collect();
    return (result, elements);
...
fn test_clpx_plaintext_ring_create() {
    ...
    assert_el_eq!(
        ...
        ZZX.evaluate(
            &t,
            &ring.canonical_gen(),
            ring.inclusion().compose(ring.base_ring().can_hom(&ZZbig).unwrap())
        )
    );
...
fn test_clpx_plaintext_ring_small_lift() {
    ...
    for a in &elements {
        ...
        assert!(
            ZZX.terms(&ring.get_ring().small_lift(a))
                .all(|(c, _)| int_cast(ZZbig.clone_el(c), ZZi64, ZZbig).abs() <= 1)
        );
    }
