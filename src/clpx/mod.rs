...
use crate::bfv::{CompositeBFV, Pow2BFV, default_impl_lift_to_Cmul, force_double_rns_repr};
...
use crate::number_ring::hypercube::isomorphism::{BaseRing, DecoratedBaseRingBase, HypercubeIsomorphism};
...
pub trait CLPXInstantiation {
    ...
    fn create_ciphertext_rings(
        &self,
        log2_q: Range<usize>,
        log2_t_coeff_inf_norm: usize,
    ) -> (CiphertextRing<Self>, CiphertextRing<Self>);
    ...
    #[instrument(skip_all)]
    fn gen_sk<R: Rng + CryptoRng>(
        C: &CiphertextRing<Self>,
        mut rng: R,
        distr: SecretKeyDistribution,
    ) -> SecretKey<Self> {
        match distr {
    ...
    fn dec_println_slots(
        ...
        DecoratedBaseRingBase<PlaintextRing<Self>>: CanIsoFromTo<BaseRing<PlaintextRing<Self>>>,
    {
    ...
    fn hom_mul_plain(
        ...
        let m_in_C = C.from_canonical_basis(
            (0..C.rank())
                .map(|i| ZZbig.clone_el(P.get_ring().ZZX().coefficient_at(&m, i)))
                .map(|c| mod_Q.map(c)),
        );
    ...
    fn noise_budget(
        ...
        let log2_size_of_noise: usize = (0..C.rank())
            .map(|i| {
                C.base_ring()
                    .integer_ring()
                    .abs_log2_ceil(&C.base_ring().smallest_lift(noise_coeffs.at(i)))
                    .unwrap_or(0)
            })
            .max()
            .unwrap();
        let log2_can_norm_t_estimate = P
            .get_ring()
            .ZZX()
            .terms(P.get_ring().t())
            .map(|(c, _)| ZZbig.abs_log2_ceil(c).unwrap())
            .max()
            .unwrap()
            + C.number_ring().small_basis_product_expansion_factor().log2().ceil() as usize;
        return ZZbig
            .abs_log2_ceil(C.base_ring().modulus())
            .unwrap()
            .saturating_sub(log2_size_of_noise + log2_can_norm_t_estimate);
    }
    ...
    /// of the relinearization key. Thus, a larger value for `digits` will result in lower
    /// (additive) noise growth during relinearization, at the cost of higher performance.
    #[instrument(skip_all)]
    ...
    fn hom_mul(
        ...
        let [mut lifted0, mut lifted1, mut lifted2] = C_mul
            .get_ring()
        ...
        let (s0, s1) = rk;
        return (
            C.add_ref(&res0, &op.gadget_product(s0, C.get_ring())),
            C.add_ref(&res1, &op.gadget_product(s1, C.get_ring())),
        );
    }
    ...
    fn hom_square(
        ...
        let [mut lifted0, mut lifted1, mut lifted2] =
            C_mul.get_ring().two_by_two_convolution_square([c0_lifted, c1_lifted]);
    ...
    /// Using a key-switch key, computes an encryption encrypting the same message as the given
    /// ciphertext under a different secret key.
    #[instrument(skip_all)]
    ...
    fn create_t_in_C_mul(P: &PlaintextRing<Self>, C_mul: &CiphertextRing<Self>) -> (El<CiphertextRing<Self>>, usize) {
        ...
        let result = C_mul.from_canonical_basis(
            (0..C_mul.rank())
                .map(|i| {
                    if ZZbig.is_zero(P.get_ring().ZZX().coefficient_at(P.get_ring().t(), i)) {
                        0
                    } else {
                        let result = int_cast(
                            ZZbig.clone_el(P.get_ring().ZZX().coefficient_at(P.get_ring().t(), i)),
                            ZZi64,
                            ZZbig,
                        );
...
impl<A: FheanorAllocator, C: FheanorNegacyclicNTT<Zn>> CLPXInstantiation for Pow2CLPX<A, C> {
    type CiphertextRing = ManagedDoubleRNSRingBase<Pow2CyclotomicNumberRing<C>, A>;

    fn number_ring(&self) -> &NumberRing<Self> { crate::bfv::BFVInstantiation::number_ring(self) }
    ...
    fn create_ciphertext_rings(
        ...
        let Cmul_modulus_size = 2 * ZZbig.abs_log2_ceil(C.base_ring().modulus()).unwrap()
            + 2 * number_ring.small_basis_product_expansion_factor().log2().ceil() as usize
            + log2_t_coeff_inf_norm
            + 3;
...
impl<A: FheanorAllocator> CLPXInstantiation for CompositeCLPX<A> {
    type CiphertextRing = ManagedDoubleRNSRingBase<TensorProductNumberRing, A>;

    fn number_ring(&self) -> &TensorProductNumberRing { crate::bfv::BFVInstantiation::number_ring(self) }
...
pub fn default_impl_rescale_to_C<'a, Inst: ?Sized + CLPXInstantiation>(
    ...
    // strictly speaking, this is not quite true since we actually multiply c (with small basis inf
    // norm) by t (with coeff basis inf norm) and care about the small basis inf norm of the result;
    // however, since the function currently isn't there, let's just pretend that
    // `log2_t_coeff_inf_norm` is also an upper bound to `log2 | t |_sbinf`
    assert!(
        ZZbig.is_gt(
            C_mul.base_ring().modulus(),
            &ZZbig.mul(
                ZZbig.mul_ref(C.base_ring().modulus(), C.base_ring().modulus()),
                ZZbig.power_of_two(
                    3 + log2_t_coeff_inf_norm
                        + C_mul.number_ring().small_basis_product_expansion_factor().log2().ceil() as usize
                )
            )
        ),
...
#[cfg(test)]
pub fn test_setup_clpx<Params: CLPXInstantiation>(
    params: Params,
) -> (
    PlaintextRing<Params>,
    CiphertextRing<Params>,
    CiphertextRing<Params>,
    SecretKey<Params>,
    RelinKey<Params>,
    El<PlaintextRing<Params>>,
    Ciphertext<Params>,
) {
    ...
    let p = factor(ZZbig, norm)
        .into_iter()
        .filter(|(_, e)| *e == 1)
        .map(|(p, _)| p)
        .max_by(|l, r| ZZbig.cmp(l, r))
        .unwrap();
    ...
    let P = params.create_plaintext_ring(
        ...
        t,
        p,
        params.number_ring().galois_group().get_group().clone().subgroup([]),
    );
...
fn test_composite_clpx_mul() {
    ...
    assert_el_eq!(
        &P,
        &m,
        &CompositeCLPX::dec(&P, &C, CompositeCLPX::clone_ct(&C, &ct), &sk)
    );
    ...
    assert_el_eq!(
        &P,
        P.pow(m, 2),
        &CompositeCLPX::dec(&P, &C, CompositeCLPX::clone_ct(&C, &sqr_ct), &sk)
    );
    ...
    let acting_galois_group = params
        .number_ring()
        .galois_group()
        .get_group()
        .clone()
        .subgroup([params.number_ring().galois_group().from_representative(18)]);
    let P = params.create_plaintext_ring(ZZX.clone(), t, p, acting_galois_group);
    let (C, C_mul) = params.create_ciphertext_rings(400..420, 10);
...
fn test_pow2_clpx_mul() {
    ...
    assert_el_eq!(
        ZZbig,
        ZZbig.power_of_two(71),
        &P.base_ring().smallest_positive_lift(P.wrt_canonical_basis(&res).at(0))
    );
}
...
fn test_pow2_clpx_hom_galois() {
    ...
    let acting_galois_group = params
        ...
        .get_group()
        .clone()
        .subgroup([params.number_ring().galois_group().from_representative(65)]);
    let P = params.create_plaintext_ring(ZZX.clone(), t, p, acting_galois_group);
...
fn test_clpx_hom_mul_overflow_mul_by_t() {
    ...
    let acting_galois_group = params
        ...
        .get_group()
        .clone()
        .subgroup([params.number_ring().galois_group().from_representative(225)]);
    let P = params.create_plaintext_ring(ZZX.clone(), t, p, acting_galois_group);
    let C = ManagedDoubleRNSRingBase::new(
        P.number_ring().clone(),
        zn_rns::Zn::new(
            [
                495617, 496897, 497153, 498689, 499969, 503297, 505601, 506113, 507137, 509441,
            ]
            .into_iter()
            .map(zn_64::Zn::new)
            .collect(),
    ...
    let C_mul = ManagedDoubleRNSRingBase::new(
        ...
        zn_rns::Zn::new(
            [
                495617, 496897, 497153, 498689, 499969, 503297, 505601, 506113, 507137, 509441, 331777, 281857, 278017,
                275201, 257281, 285697, 299521, 326657, 325889, 285953, 355841,
            ]
            .into_iter()
            .map(zn_64::Zn::new)
            .collect(),
    ...
    // the parameters are very carefully chosen; on the one hand, the extended modulus is large enough
    // to accomodate c0 * c1' + c1 * c0', where each ci, cj' are ring elements with coefficients at
    // most 2 * q
    assert!(ZZbig.is_gt(
        C_mul.base_ring().modulus(),
        &ZZbig.int_hom().mul_map(
            ZZbig.mul_ref(C.base_ring().modulus(), C.base_ring().modulus()),
            128 * 2 * 2 * 2
        )
    ));
    // on the other hand, the extended modulus is not large enough to store the ci * cj' * t, for ci,
    // cj' roughly q/2
    assert!(ZZbig.is_lt(
        C_mul.base_ring().modulus(),
        &ZZbig.int_hom().mul_map(
    ...
    assert_el_eq!(
        P.base_ring(),
        P.base_ring().zero(),
        P.wrt_canonical_basis(&Pow2CLPX::dec(&P, &C, Pow2CLPX::clone_ct(&C, &ct), &sk))
            .at(0)
    );
    ...
    assert_el_eq!(
        P.base_ring(),
        P.base_ring().zero(),
        P.wrt_canonical_basis(&Pow2CLPX::dec(&P, &C, Pow2CLPX::clone_ct(&C, &ct_res), &sk))
            .at(0)
    );
}
...
fn measure_time_composite_clpx() {
    ...
    let acting_galois_group = params
        ...
        .get_group()
        .clone()
        .subgroup([params.number_ring().galois_group().from_representative(25276)]);

    let P = log_time("CreateEncoding", |[]| {
        params.create_plaintext_ring(ZZX, t, p, acting_galois_group)
    });
    let int_to_P = P
        .inclusion()
        .compose(P.base_ring().can_hom(&StaticRing::<i128>::RING).unwrap());
    let (C, C_mul) = log_time("CreateCtxtRing", |[]| params.create_ciphertext_rings(790..800, 10));
    ...
    let ct = log_time("EncSym", |[]| CompositeCLPX::enc_sym(&P, &C, &mut rng, &m, &sk, 3.2));
    ...
    let res = log_time("HomMul", |[]| {
        CompositeCLPX::hom_mul(
            &P,
            &C,
            &C_mul,
            CompositeCLPX::clone_ct(&C, &ct),
            CompositeCLPX::clone_ct(&C, &ct),
            &rk,
        )
    });
