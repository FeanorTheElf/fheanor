...
impl<Params: CLPXInstantiation> AsCLPXPlaintext<Params> for StaticRingBase<i64> {
    fn hom_add_to(
        ...
        Params::hom_add_plain(
            P,
            C,
            &P.inclusion().compose(P.base_ring().can_hom(&ZZi64).unwrap()).map(*m),
            ct,
        )
    }
    ...
    fn hom_mul_to(
        ...
        Params::hom_mul_plain(
            P,
            C,
            &P.inclusion().compose(P.base_ring().can_hom(&ZZi64).unwrap()).map_ref(m),
            ct,
        )
    }
}
...
impl<Params: CLPXInstantiation> AsCLPXPlaintext<Params> for BigIntRingBase {
    fn hom_add_to(
        ...
        Params::hom_add_plain(
            P,
            C,
            &P.inclusion().compose(P.base_ring().can_hom(&ZZbig).unwrap()).map_ref(m),
            ct,
        )
    }
...
impl<'a, 'b, R: ?Sized + AsCLPXPlaintext<Inst>, Inst: CLPXInstantiation> CircuitEvaluator<'b, Ciphertext<Inst>, R>
    for CLPXEvaluator<'a, R, Inst>
{
        ...
        Ciphertext<Inst>: 'c,
    {
        if let Some((coeff, ciphertext)) = data.next() {
            let mut result = if let Coefficient::One = coeff {
                ...
                <BigIntRingBase as AsCLPXPlaintext<Inst>>::hom_mul_to(
                    ZZbig.get_ring(),
                    self.P,
                    self.C,
                    &int,
                ...
                self.ring
                    .hom_mul_to(self.P, self.C, coeff, Inst::clone_ct(self.C, ciphertext))
            } else {
                unreachable!()
            };
            for (coeff, ciphertext) in data {
                if let Coefficient::One = coeff {
                    ...
                    result = <BigIntRingBase as AsCLPXPlaintext<Inst>>::hom_fma(
                        ZZbig.get_ring(),
                        self.P,
                        self.C,
                        result,
...
#[cfg(test)]
use feanor_math::rings::poly::{PolyRingStore, dense_poly::DensePolyRing};
...
fn test_hom_evaluate_circuit() {
    ...
    let res = circuit
        .evaluate_clpx::<Pow2CLPX, _>(ZZbig, &P, &C, Some(&C_mul), &[ct], Some(&rk), &[], None)
        .into_iter()
        .next()
        .unwrap();
    assert_el_eq!(
        &P,
        P.inclusion()
            .map(FpX.evaluate(&f, &P.wrt_canonical_basis(&m).at(0), FpX.base_ring().identity())),
        &Pow2CLPX::dec(&P, &C, res, &sk)
    );
}
