...
{
    fn new(ring: R, max_log2_len: usize) -> Self {
        assert!(ring.integer_ring().is_one(&ring.integer_ring().euclidean_rem(
            ring.integer_ring().clone_el(ring.modulus()),
            &ring.integer_ring().power_of_two(max_log2_len)
        )));
        NTTConvolution::new(ring)
    }

    fn ring(&self) -> RingRef<'_, R::Type> { NTTConvolution::ring(self) }
}
...
impl FheanorConvolution<Zn> for NTTConvolution<ZnBase, ZnFastmulBase, CanHom<ZnFastmul, Zn>> {
    ...
    fn ring(&self) -> RingRef<'_, ZnBase> { NTTConvolution::ring(self) }
}
...
impl FheanorConvolution<zn_64::Zn> for feanor_math_hexl::conv::HEXLConvolution {
    fn new(ring: zn_64::Zn, max_log2_len: usize) -> Self {
        assert!(ring.integer_ring().is_one(&ring.integer_ring().euclidean_rem(
            ring.integer_ring().clone_el(ring.modulus()),
            &ring.integer_ring().power_of_two(max_log2_len + 1)
        )));
    ...
    R: RingStore,
{
    ...
    ///
    /// Concretely, the `i`-th element of `output` should store the evaluation of `input`
    /// (interpreted as a polynomial) at `𝝵^(bitrev(i) * 2 + 1)`.
    ///
    ...
    R::Type: ZnRing,
{
    ...
    fn ring(&self) -> RingRef<'_, R::Type> { RingRef::new(self.ring.get_ring()) }

    fn len(&self) -> usize { self.fft_table.len() }
...
impl FheanorNegacyclicNTT<Zn> for feanor_math_hexl::hexl::HEXLNegacyclicNTT {
    ...
    fn len(&self) -> usize { feanor_math_hexl::hexl::HEXLNegacyclicNTT::n(self) }
...
impl<R_main, R_twiddle> FheanorGeneralNTT<R_main>
    for BluesteinFFT<R_main::Type, R_twiddle, CanHom<RingValue<R_twiddle>, R_main>>
where
    R_main: RingStore + Clone,
    ...
    R_main::Type: FiniteRing + CanHomFrom<R_twiddle>,
{
    fn fft_base<const INV: bool>(&self, input: &mut [El<R_main>], output: &mut [El<R_main>]) {
        ...
        if INV {
            <BluesteinFFT<_, _, _> as FFTAlgorithm<R_main::Type>>::inv_fft(
                &self,
                &mut *input,
                FheanorGeneralNTT::ring(self),
            );
        } else {
            <BluesteinFFT<_, _, _> as FFTAlgorithm<R_main::Type>>::fft(
                &self,
                &mut *input,
                FheanorGeneralNTT::ring(self),
            );
    ...
    fn ring(&self) -> RingRef<'_, R_main::Type> { unimplemented!() }

    fn len(&self) -> usize { <BluesteinFFT<_, _, _> as FFTAlgorithm<R_main::Type>>::len(self) }

    fn new(ring: R_main, m: usize) -> Self {
        let p = ring.characteristic(BigIntRing::RING).unwrap();
        let twiddle_ring = RingValue::from(
            R_twiddle::from_modulus(|ZZ| Ok(int_cast(p, RingRef::new(ZZ), BigIntRing::RING))).unwrap_or_else(no_error),
        );
