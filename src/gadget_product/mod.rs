...
pub struct RNSGadgetProductLhsOperand<R: PreparedMultiplicationRing> {
    ...
    /// We store the element once as `PreparedMultiplicant` for fast computation of gadget products,
    /// and once as the element itself, since there currently is no way of getting the ring
    /// element out of a `PreparedMultiplicant`
...
impl<R: NumberRingRNSQuotient> RNSGadgetProductLhsOperand<R> {
    ...
    /// Creates a [`RNSGadgetProductLhsOperand`] w.r.t. the RNS gadget vector that has `digits`
    /// digits. For an explanation of gadget products, see
    /// [`RNSGadgetProductLhsOperand::gadget_product()`].
    pub fn from_element(ring: &R, el: &R::Element, digits: usize) -> Self {
        Self::from_element_with(
            ring,
            el,
            &RNSGadgetVectorDigitIndices::select_digits(digits, ring.base_ring().len()),
        )
    }
    ...
    pub fn scale_up_from_element_with(
        el_ring: &R,
        el: &R::Element,
        main_ring: &R,
        ...
        let scale_by_factors =
            RNSFactorIndexList::missing_from_subset(el_ring.base_ring(), main_ring.base_ring()).unwrap();
        let scaled_el = RingRef::new(main_ring).inclusion().mul_map(
            main_ring.add_rns_factor_element(el_ring, &scale_by_factors, el),
            main_ring.base_ring().coerce(
                &ZZbig,
                ZZbig.prod(
                    scale_by_factors
                        .iter()
                        .map(|i| int_cast(*main_ring.base_ring().at(*i).modulus(), ZZbig, ZZi64)),
                ),
            ),
        );
        let mut gadget_decomposition = gadget_decompose(
            ...
            digits
                .iter()
                .filter(|digit| scale_by_factors.num_within(digit) < digit.end - digit.start),
            main_ring,
        )
        .into_iter();
        let result = digits
            .iter()
            .map(|digit| {
                if scale_by_factors.num_within(&digit) < digit.end - digit.start {
    ...
    A: FheanorAllocator,
{
    /// Creates a [`RNSGadgetProductLhsOperand`] w.r.t. the gadget vector given by `digits`.
    /// For an explanation of gadget products, see [`RNSGadgetProductLhsOperand::gadget_product()`].
    pub fn from_double_rns_ring_with(
        ring: &DoubleRNSRingBase<NumberRing, A>,
        el: &SmallBasisEl<NumberRing, A>,
        digits: &RNSGadgetVectorDigitIndices,
    ) -> Self {
        assert!(digits.iter().all(|digit| digit.end > digit.start));
        let decomposition = gadget_decompose_doublerns(ring, el, digits.iter(), ring)
            .into_iter()
            .map(Some)
            .collect();
        return Self {
    ...
    /// digits. For an explanation of gadget products, see
    /// [`RNSGadgetProductLhsOperand::gadget_product()`].
    pub fn from_double_rns_ring(
        ring: &DoubleRNSRingBase<NumberRing, A>,
        el: &SmallBasisEl<NumberRing, A>,
        digits: usize,
    ) -> Self {
        Self::from_double_rns_ring_with(
            ring,
            el,
            &RNSGadgetVectorDigitIndices::select_digits(digits, ring.base_ring().len()),
        )
...
impl<R: PreparedMultiplicationRing> RNSGadgetProductLhsOperand<R> {
        ...
        R: NumberRingQuotient,
    {
        ...
        result.resize_with(gs.len(), || RNSGadgetProductLhsOperand {
            element_decomposition: Vec::new(),
        });
            ...
            if let Some((prepared_el, el)) = el {
                ...
                for (i, (new_el, g)) in new_els.into_iter().zip(gs.iter()).enumerate() {
                    if ring.acting_galois_group().is_identity(g) {
                        result[i]
                            .element_decomposition
                            .push(Some((prepared_el.take().unwrap(), new_el)));
                    } else {
                        result[i]
                            .element_decomposition
                            .push(Some((ring.prepare_multiplicant(&new_el), new_el)));
                    }
                }
            } else {
                for i in 0..gs.len() {
    ...
    /// ```
    /// To demonstrate how this keeps small error terms small, consider the following variation of
    ...
    /// let mut create_small_error = ||
    /// ring.get_ring().from_canonical_basis((0..ring.rank()).map(|i|
    /// ring.base_ring().int_hom().map((rng.rand_u64() % 3) as i32 - 1))); for i in 0..3 {
    ///     // set the i-th component to `gadget_vector(i) * rhs`, with possibly some error included
    ///     let component_at_i = ring.inclusion().mul_ref_map(&rhs,
    /// &rhs_op.gadget_vector(ring.get_ring()).at(i));     rhs_op.set_component(ring.get_ring(),
    /// i, ring.add(component_at_i, create_small_error())); }
    ///
    /// // compute the gadget product
    /// let lhs = ring.random_element(|| rng.rand_u64());
    /// let lhs_op = RNSGadgetProductLhsOperand::from_element(ring.get_ring(), &lhs, digits);
    ...
    /// assert!((0..8).all(|i| int_cast(ring.base_ring().smallest_lift(error_coefficients.at(i)),
    /// StaticRing::<i64>::RING, BigIntRing::RING).abs() <= max_allowed_error)); ```
    pub fn gadget_product(&self, rhs: &RNSGadgetProductRhsOperand<R>, ring: &R) -> R::Element {
        assert_eq!(
            self.element_decomposition.len(),
            rhs.scaled_element.len(),
            "Gadget product operands created w.r.t. different digit sets"
        );
        return ring.inner_product_prepared(
            self.element_decomposition
                .iter()
                .zip(rhs.scaled_element.iter())
                .filter_map(|(lhs, rhs)| {
                    rhs.as_ref().and_then(|(rhs_prep, rhs)| {
                        lhs.as_ref()
                            .map(|(lhs_prep, lhs)| (lhs, Some(lhs_prep), rhs, Some(rhs_prep)))
                    })
                })
                .collect::<Vec<_>>(),
        );
...
#[instrument(skip_all)]
fn gadget_decompose<R, S, I>(
    ring: &R,
    el: &R::Element,
    digits: I,
    out_ring: &S,
) -> Vec<(S::PreparedMultiplicant, S::Element)>
where
    R: NumberRingRNSQuotient,
...
#[instrument(skip_all)]
fn gadget_decompose_doublerns<NumberRing, A, I>(
    ring: &DoubleRNSRingBase<NumberRing, A>,
    el: &SmallBasisEl<NumberRing, A>,
    digits: I,
    out_ring: &DoubleRNSRingBase<NumberRing, A>,
) -> Vec<((), El<DoubleRNSRing<NumberRing, A>>)>
where
    NumberRing: NumberRingDescriptor,
    A: FheanorAllocator,
    I: Iterator<Item = Range<usize>>,
...
impl<R: PreparedMultiplicationRing> RNSGadgetProductRhsOperand<R> {
    pub fn clone(&self, ring: &R) -> Self {
        Self {
            scaled_element: self
                ...
                .map(|el| {
                    el.as_ref()
                        .map(|el| (ring.prepare_multiplicant(&el.1), ring.clone_el(&el.1)))
                })
                .collect(),
    ...
    {
        self.gadget_vector_digits().map_fn(|digit| {
            ring.base_ring()
                .get_ring()
                .from_congruence((0..ring.base_ring().get_ring().len()).map(|i| {
                    if digit.contains(&i) {
    ...
    pub fn gadget_vector_digits<'b>(&'b self) -> &'b RNSGadgetVectorDigitIndices { &self.digits }
    ...
    /// Creates a [`RNSGadgetProductRhsOperand`] representing `0` w.r.t. the RNS-based gadget vector
    /// that has `digits` digits.
    ///
    /// For an explanation of gadget products, see [`RNSGadgetProductLhsOperand::gadget_product()`].
    ...
    where
        ...
        R::BaseRing: RingStore<Type = zn_rns::ZnBase<zn_64::Zn, BigIntRing>>,
    {
        Self::new_with_digits(
            ring,
            RNSGadgetVectorDigitIndices::select_digits(digits, ring.base_ring().get_ring().len()),
        )
    }

    /// Creates a [`RNSGadgetProductRhsOperand`] representing `0` w.r.t. the RNS-based gadget vector
    /// given by `digits`. For the exact description how the gadget vector is constructed based
    /// on `digits`, see [`RNSGadgetProductRhsOperand::gadget_vector_digits()`].
    ///
    /// For an explanation of gadget products, see [`RNSGadgetProductLhsOperand::gadget_product()`].
...
impl<R: NumberRingRNSQuotient> RNSGadgetProductRhsOperand<R> {
    ...
    pub fn modulus_switch(&self, to: &R, dropped_rns_factors: &RNSFactorIndexList, from: &R) -> Self {
        assert_eq!(
            to.base_ring().get_ring().len() + dropped_rns_factors.len(),
            from.base_ring().get_ring().len()
        );
        ...
        for (digit, scaled_el) in self.digits.iter().zip(self.scaled_element.iter()) {
            ...
            if let Some((scaled_el_prepared, scaled_el)) = scaled_el {
                ...
                result_scaled_el.push(Some((
                    to.drop_rns_factor_prepared_element(from, dropped_rns_factors, scaled_el_prepared),
                    new_scaled_el,
                )));
            } else {
...
fn test_gadget_decomposition() {
    ...
    let ring = SingleRNSRingBase::<_, Global, DefaultConvolution>::new(
        number_ring,
        zn_rns::Zn::create_from_primes(vec![17, 97, 113], BigIntRing::RING),
    );
    let rns_base = ring.base_ring();
    let from_congruence =
        |data: &[i32]| rns_base.from_congruence(data.iter().enumerate().map(|(i, c)| rns_base.at(i).int_hom().map(*c)));
    ...
    assert_el_eq!(
        ring,
        ring.inclusion().map(hom_i32.map(1000)),
        lhs.gadget_product(&rhs, ring.get_ring())
    );
}
...
fn test_modulus_switch() {
    ...
    let smaller_ring = SingleRNSRingBase::<_, Global, DefaultConvolution>::new(
        number_ring.clone(),
        zn_rns::Zn::create_from_primes(vec![17, 113], BigIntRing::RING),
    );
    let rhs = rhs.modulus_switch(
        smaller_ring.get_ring(),
        RNSFactorIndexList::from_ref(&[1], rns_base.len()),
        ring.get_ring(),
    );
    ...
    assert_el_eq!(
        &smaller_ring,
        smaller_ring.int_hom().map(1000),
        lhs.gadget_product(&rhs, smaller_ring.get_ring())
    );

    let ring = SingleRNSRingBase::<_, Global, DefaultConvolution>::new(
        number_ring.clone(),
        zn_rns::Zn::create_from_primes(vec![17, 97, 113, 193, 241], BigIntRing::RING),
    );
    let rns_base = ring.base_ring();
    let from_congruence =
        |data: &[i32]| rns_base.from_congruence(data.iter().enumerate().map(|(i, c)| rns_base.at(i).int_hom().map(*c)));
    ...
    rhs.set_component(
        ring.get_ring(),
        0,
        ring.inclusion().map(from_congruence(&[1000, 1000, 0, 0, 0])),
    );
    rhs.set_component(
        ring.get_ring(),
        1,
        ring.inclusion().map(from_congruence(&[0, 0, 1000, 1000, 0])),
    );
    rhs.set_component(
        ring.get_ring(),
        2,
        ring.inclusion().map(from_congruence(&[0, 0, 0, 0, 1000])),
    );

    let smaller_ring = SingleRNSRingBase::<_, Global, DefaultConvolution>::new(
        number_ring,
        zn_rns::Zn::create_from_primes(vec![17, 193, 241], BigIntRing::RING),
    );
    let rhs = rhs.modulus_switch(
        smaller_ring.get_ring(),
        RNSFactorIndexList::from_ref(&[1, 2], rns_base.len()),
        ring.get_ring(),
    );
    ...
    assert_el_eq!(
        &smaller_ring,
        smaller_ring.int_hom().map(1000000),
        lhs.gadget_product(&rhs, smaller_ring.get_ring())
    );
}
