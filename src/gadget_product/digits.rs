...
impl RNSGadgetVectorDigitIndices {
    fn from_unchecked(digit_boundaries: Box<[usize]>) -> Box<Self> { unsafe { std::mem::transmute(digit_boundaries) } }

    /// Creates the list of digits, each containing the RNS factors whose indices are within the
    /// corresponding range. This requires the ranges to exactly cover a contiguous interval `{
    /// 0, 1, ..., k - 1 }`, otherwise the function will panic.
        ...
        V: VectorFn<Range<usize>>,
    {
        ...
        for _ in 0..digits.len() {
            let mut it = digits
                .iter()
                .filter(|digit| digit.start == *result.last().unwrap_or(&0));
    ...
    pub fn rns_base_len(&self) -> usize { *self.digit_boundaries.last().unwrap_or(&0) }
    ...
    pub fn select_digits(digits: usize, rns_base_len: usize) -> Box<Self> {
        assert!(
            digits <= rns_base_len,
            "the number of gadget product digits may not exceed the number of RNS factors"
        );
...
impl VectorFn<Range<usize>> for RNSGadgetVectorDigitIndices {
    fn len(&self) -> usize { self.digit_boundaries.len() }
