...
impl Deref for RNSFactorIndexList {
    ...
    fn deref(&self) -> &Self::Target { &self.rns_factor_indices }
}
...
impl RNSFactorIndexList {
    fn from_unchecked(indices: Box<[usize]>) -> Box<Self> { unsafe { std::mem::transmute(indices) } }

    fn from_ref_unchecked<'a>(indices: &'a [usize]) -> &'a Self { return unsafe { std::mem::transmute(indices) }; }

    fn check_valid(indices: &[usize], rns_base_len: usize) {
        for i in indices {
            assert!(
                *i < rns_base_len,
                "all indices must be valid for an RNS base of length {}, but found {}",
                rns_base_len,
                *i
            );
        }
        for (i0, j0) in indices.iter().enumerate() {
            for (i1, j1) in indices.iter().enumerate() {
                assert!(
                    i0 == i1 || j0 != j1,
                    "all indices must be distinct, but found indices[{}] == {} == indices[{}]",
                    i0,
                    j0,
                    i1
                );
    ...
    pub fn contains(&self, i: usize) -> bool { self.rns_factor_indices.binary_search(&i).is_ok() }

    pub fn contains_all(&self, subset: &Self) -> bool { subset.iter().all(|i| self.contains(*i)) }
    ...
    /// assert_eq!(
    ///     1,
    ///     RNSFactorIndexList::from_ref(&[2, 5], 8).num_within(&(0..5))
    /// );
    /// ```
    pub fn num_within(&self, range: &Range<usize>) -> usize {
        match (
            self.rns_factor_indices.binary_search(&range.start),
            self.rns_factor_indices.binary_search(&range.end),
        ) {
            (Ok(i), Ok(j)) | (Ok(i), Err(j)) | (Err(i), Ok(j)) | (Err(i), Err(j)) => j - i,
        }
    }
    ...
    /// [`RNSFactorIndexList::subtract()`] - relative to the RNS base that has `context` already
    /// removed. This assumes that `context` is a subset of `self`.
    ///
    ...
    /// ```
    ///
    /// **Note for mathematicians**: This has nothing to do with the category-theoretical
    /// pushforward
    ///
    /// # Example
    /// ```
    ...
    /// assert_eq!(
    ///     &[1usize, 3, 5][..],
    ///     &RNSFactorIndexList::from_ref(&[1, 2, 4, 5, 7], 8)
    ///         .pushforward(RNSFactorIndexList::from_ref(&[2, 5], 8)) as &[usize]
    /// )
    /// ```
    ...
    /// assert_eq!(
    ///     &[1, 2, 3, 5, 6][..],
    ///     &RNSFactorIndexList::from_ref(&[1, 2, 4], 6)
    ///         .pullback(RNSFactorIndexList::from_ref(&[2, 5], 8)) as &[usize]
    /// )
    /// ```
    ...
    pub fn union(&self, other: &Self) -> Box<Self> {
        let mut result = self
            ...
            .iter()
            .copied()
            .chain(other.rns_factor_indices.iter().copied().filter(|i| !self.contains(*i)))
            .collect::<Box<[usize]>>();
        result.sort_unstable();
        return Self::from_unchecked(result);
    }

    pub fn empty() -> Box<Self> { Self::from_unchecked(Box::new([])) }

    pub fn empty_ref() -> &'static Self { Self::from_ref_unchecked(&[]) }
...
impl Clone for Box<RNSFactorIndexList> {
    fn clone(&self) -> Self { RNSFactorIndexList::to_owned(&self) }
}
    ...
    R::Type: ZnRing,
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "the set of RNS factors {:?} is not a subset of {:?}",
            self.not_a_subset
                .as_iter()
                .map(|ring| int_cast(ring.integer_ring().clone_el(ring.modulus()), ZZbig, ring.integer_ring()))
                .collect::<Vec<_>>()
                .iter()
                .map(|x| ZZbig.format(x))
                .collect::<Vec<_>>(),
            self.superset
                .as_iter()
                .map(|ring| int_cast(ring.integer_ring().clone_el(ring.modulus()), ZZbig, ring.integer_ring()))
...
fn test_missing_from_subset() {
    ...
    assert_eq!(
        &expected,
        &**RNSFactorIndexList::missing_from_subset(&subset, &master).unwrap()
    );
