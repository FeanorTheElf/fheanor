...
use crate::number_ring::{
    NumberRingDescriptor, NumberRingQuotient, extend_sampled_primes, largest_prime_leq_congruent_to_one,
};
...
impl ManagedDoubleRNSElRepresentationKind {
    fn joint(self, other: Self) -> Self {
        match (self, other) {
            ...
            (ManagedDoubleRNSElRepresentationKind::SmallBasis, ManagedDoubleRNSElRepresentationKind::DoubleRNS) => {
                ManagedDoubleRNSElRepresentationKind::Sum
            }
            (ManagedDoubleRNSElRepresentationKind::DoubleRNS, ManagedDoubleRNSElRepresentationKind::SmallBasis) => {
                ManagedDoubleRNSElRepresentationKind::Sum
            }
            (ManagedDoubleRNSElRepresentationKind::DoubleRNS, ManagedDoubleRNSElRepresentationKind::DoubleRNS) => {
                ManagedDoubleRNSElRepresentationKind::DoubleRNS
            }
            (ManagedDoubleRNSElRepresentationKind::SmallBasis, ManagedDoubleRNSElRepresentationKind::SmallBasis) => {
                ManagedDoubleRNSElRepresentationKind::SmallBasis
            }
    ...
    A: FheanorAllocator,
{
    Sum(
        Boo<
            'a,
            (SmallBasisEl<NumberRing, A>, DoubleRNSEl<NumberRing, A>),
            MappedRwLockReadGuardType<(SmallBasisEl<NumberRing, A>, DoubleRNSEl<NumberRing, A>)>,
        >,
    ),
    SmallBasis(Boo<'a, SmallBasisEl<NumberRing, A>>),
    DoubleRNS(Boo<'a, DoubleRNSEl<NumberRing, A>>),
    Both(
        Boo<'a, SmallBasisEl<NumberRing, A>>,
        Boo<'a, DoubleRNSEl<NumberRing, A>>,
    ),
    Zero,
}
    ...
    A: FheanorAllocator,
{
    fn get_kind(&self) -> ManagedDoubleRNSElRepresentationKind {
        match self {
            ...
            ManagedDoubleRNSElRepresentation::Both(..) => ManagedDoubleRNSElRepresentationKind::Both,
    ...
    A: FheanorAllocator,
{
    fn get_repr<'a>(&'a self) -> ManagedDoubleRNSElRepresentation<'a, NumberRing, A> {
        ...
        if sum_repr.is_some() {
            return ManagedDoubleRNSElRepresentation::Sum(Boo::GuardedBorrowed(RwLockReadGuard::map(
                sum_repr,
                |sum_repr: &Option<_>| sum_repr.as_ref().unwrap(),
            )));
        }
        // we can unlock `sum_repr` here, since if `sum_repr` was empty previously, it will always remain
        // empty
        drop(sum_repr);
        if let Some(small_basis_repr) = self.small_basis_repr.get() {
            if let Some(double_rns_repr) = self.double_rns_repr.get() {
                return ManagedDoubleRNSElRepresentation::Both(
                    Boo::Borrowed(small_basis_repr),
                    Boo::Borrowed(double_rns_repr),
                );
            } else {
    ...
    fn into_repr<'a>(self) -> ManagedDoubleRNSElRepresentation<'a, NumberRing, A> {
        ...
        if let Some(small_basis_repr) = self.small_basis_repr.into_inner() {
            if let Some(double_rns_repr) = self.double_rns_repr.into_inner() {
                return ManagedDoubleRNSElRepresentation::Both(
                    Boo::Owned(small_basis_repr),
                    Boo::Owned(double_rns_repr),
                );
            } else {
    ...
    where
        F: for<'a> FnOnce(ManagedDoubleRNSElRepresentation<'a, NumberRing, A>) -> R,
    {
    ...
    A: FheanorAllocator,
{
    ...
    pub fn to_small_basis<'a>(
        &self,
        value: &'a ManagedDoubleRNSEl<NumberRing, A>,
    ) -> Option<&'a SmallBasisEl<NumberRing, A>> {
        match value.internal.get_repr() {
            ManagedDoubleRNSElRepresentation::Sum(sum_repr) => {
                ...
                self.base
                    .add_assign_non_fft(&mut result, &self.base.undo_fft(sum_repr.1));
                // no other thread is able to update this, since the previous call to `get_repr()` would alway
                // return `Sum` (or run after we unlocked `sum_repr_lock`)
                value.internal.small_basis_repr.set(result).ok().unwrap();
                // keep the `sum_repr_lock` until we initialized `small_basis_repr`, otherwise threads querying this
                // in the meantime might think the element stores zero
                drop(sum_repr_lock);
            ...
            ManagedDoubleRNSElRepresentation::DoubleRNS(double_rns_repr) => {
                ...
                // here another thread might have set `small_basis_repr` in the meantime, so this might return
                // `Err(_)`
    ...
    pub fn element_allocation_size(&self) -> usize { 2 * self.unmanaged_ring().get_ring().element_allocation_size() }
    ...
    pub fn into_small_basis(&self, value: ManagedDoubleRNSEl<NumberRing, A>) -> Option<SmallBasisEl<NumberRing, A>> {
        if self.to_small_basis(&value).is_some() {
            value.internal.with_repr(|repr| match repr {
                ManagedDoubleRNSElRepresentation::SmallBasis(small_basis)
                | ManagedDoubleRNSElRepresentation::Both(small_basis, _) => {
    ...
    /// Returns the representation of the given element w.r.t. the multiplicative basis, possibly
    /// computing this representation if it is not available. If the element is zero, `None` is
    /// returned.
    pub fn into_doublerns(&self, value: ManagedDoubleRNSEl<NumberRing, A>) -> Option<DoubleRNSEl<NumberRing, A>> {
        if self.to_doublerns(&value).is_some() {
            value.internal.with_repr(|repr| match repr {
                ManagedDoubleRNSElRepresentation::DoubleRNS(double_rns)
                | ManagedDoubleRNSElRepresentation::Both(_, double_rns) => {
    ...
    /// computing this representation if it is not available. If the element is zero, `None` is
    /// returned.
    pub fn to_doublerns<'a>(
        &self,
        value: &'a ManagedDoubleRNSEl<NumberRing, A>,
    ) -> Option<&'a DoubleRNSEl<NumberRing, A>> {
        match value.internal.get_repr() {
            ManagedDoubleRNSElRepresentation::Sum(sum_repr) => {
                drop(sum_repr);
                let mut sum_repr_lock = value.internal.sum_repr.write().unwrap();
            ...
            ManagedDoubleRNSElRepresentation::SmallBasis(small_basis_repr) => {
                let result = self
                    .base
                    .do_fft(small_basis_repr.to_owned(|x| self.base.clone_el_non_fft(x)));
                // here another thread might have set `double_rns_repr` in the meantime, so this might return
                // `Err(_)`
    ...
    fn from_sum(
        &self,
        small_basis_part: SmallBasisEl<NumberRing, A>,
        double_rns_part: DoubleRNSEl<NumberRing, A>,
    ...
    fn from_both(
        &self,
        small_basis_repr: SmallBasisEl<NumberRing, A>,
        double_rns_repr: DoubleRNSEl<NumberRing, A>,
    ) -> ManagedDoubleRNSEl<NumberRing, A> {
        ManagedDoubleRNSEl {
            internal: Arc::new(DoubleRNSElInternal {
                small_basis_repr: {
    ...
    fn negate_internal(
        &self,
        value: Arc<DoubleRNSElInternal<NumberRing, A>>,
    ) -> Arc<DoubleRNSElInternal<NumberRing, A>> {
        self.apply_linear_operation(
            self.zero().internal,
            value,
    ...
    where
        F_coeff_bin: FnOnce(&mut SmallBasisEl<NumberRing, A>, &SmallBasisEl<NumberRing, A>),
        ...
        F_doublerns_un: FnOnce(&mut DoubleRNSEl<NumberRing, A>),
    {
        ...
        lhs.with_repr(|lhs| {
            rhs.with_repr(|rhs| match (lhs, rhs) {
                (_, ManagedDoubleRNSElRepresentation::Zero) => unreachable!(),
                (
                    ManagedDoubleRNSElRepresentation::Zero,
                    ManagedDoubleRNSElRepresentation::DoubleRNS(rhs_double_rns_repr),
                ) => {
                ...
                (
                    ManagedDoubleRNSElRepresentation::Zero,
                    ManagedDoubleRNSElRepresentation::SmallBasis(rhs_small_basis_repr),
                ) => {
                ...
                (
                    ManagedDoubleRNSElRepresentation::Zero,
                    ManagedDoubleRNSElRepresentation::Both(rhs_small_basis_repr, rhs_double_rns_repr),
                ) => {
                    let mut result_fft = rhs_double_rns_repr.to_owned(|x| self.base.clone_el(x));
                    let mut result_coeff = rhs_small_basis_repr.to_owned(|x| self.base.clone_el_non_fft(x));
                    f3(&mut result_coeff);
                ...
                (
                    ManagedDoubleRNSElRepresentation::Sum(lhs_sum_repr),
                    ManagedDoubleRNSElRepresentation::Sum(rhs_sum_repr),
                ) => {
                ...
                (
                    ManagedDoubleRNSElRepresentation::Sum(lhs_sum_repr),
                    ManagedDoubleRNSElRepresentation::Both(rhs_small_basis_repr, _),
                ) => {
                    let mut result_coeff = self.base.clone_el_non_fft(&lhs_sum_repr.0);
                    let result_fft = self.base.clone_el(&lhs_sum_repr.1);
                    f1(&mut result_coeff, &rhs_small_basis_repr);
                ...
                (
                    ManagedDoubleRNSElRepresentation::SmallBasis(lhs_small_basis_repr),
                    ManagedDoubleRNSElRepresentation::Sum(rhs_sum_repr),
                ) => {
                    let mut result_fft = self.base.clone_el(&rhs_sum_repr.1);
                ...
                | (
                    ManagedDoubleRNSElRepresentation::Both(lhs_small_basis_repr, _),
                    ManagedDoubleRNSElRepresentation::SmallBasis(rhs_small_basis_repr),
                ) => {
                    let mut result_coeff = lhs_small_basis_repr.to_owned(|x| self.base.clone_el_non_fft(x));
                    f1(&mut result_coeff, &rhs_small_basis_repr);
                ...
                (
                    ManagedDoubleRNSElRepresentation::DoubleRNS(lhs_double_rns_repr),
                    ManagedDoubleRNSElRepresentation::Sum(rhs_sum_repr),
                ) => {
                ...
                | (
                    ManagedDoubleRNSElRepresentation::DoubleRNS(lhs_double_rns_repr),
                    ManagedDoubleRNSElRepresentation::Both(_, rhs_double_rns_repr),
                )
                | (
                    ManagedDoubleRNSElRepresentation::Both(_, lhs_double_rns_repr),
                    ManagedDoubleRNSElRepresentation::DoubleRNS(rhs_double_rns_repr),
                ) => {
                    let mut result_fft = lhs_double_rns_repr.to_owned(|x| self.base.clone_el(x));
                    f2(&mut result_fft, &rhs_double_rns_repr);
                ...
                (
                    ManagedDoubleRNSElRepresentation::Both(lhs_small_basis_repr, lhs_double_rns_repr),
                    ManagedDoubleRNSElRepresentation::Both(rhs_small_basis_repr, rhs_double_rns_repr),
                ) => {
                    let mut result_fft = lhs_double_rns_repr.to_owned(|x| self.base.clone_el(x));
                    let mut result_coeff = lhs_small_basis_repr.to_owned(|x| self.base.clone_el_non_fft(x));
    ...
    A: FheanorAllocator,
{
    ...
    where
        I: Iterator<Item = super::RNSFactorCongruence<'a, Self, Self::Element>>,
        Self: 'a,
    {
        ...
        let output_format = rns_factors
            .iter()
            .filter_map(|congruence| {
                if let RNSFactorCongruence::CongruentTo(_, _, el) = congruence {
                    Some(el.internal.get_repr())
                } else {
                    None
                }
            })
            ...
            .fold(
                ManagedDoubleRNSElRepresentationKind::Zero,
                ManagedDoubleRNSElRepresentationKind::joint,
            );
        ...
        match output_format {
            ...
            ManagedDoubleRNSElRepresentationKind::SmallBasis => {
                self.from_small_basis(self.base.collect_rns_factors_non_fft(congruences_small_basis))
            }
            ManagedDoubleRNSElRepresentationKind::DoubleRNS => {
                self.from_doublerns(self.base.collect_rns_factors(congruences_doublerns))
            }
            ManagedDoubleRNSElRepresentationKind::Sum => {
                // collect reprs here because we need refs to the RwMappedLockGuard's content
                let reprs = rns_factors
                    .iter()
                    .map(|congruence| match congruence {
                    ...
                    .collect::<Vec<_>>();
                let congruences_small_basis =
                    rns_factors
                        .iter()
                        .zip(reprs.iter())
                        .map(|(congruence, repr)| match (congruence, repr) {
                            (_, ManagedDoubleRNSElRepresentation::Zero)
                            | (_, ManagedDoubleRNSElRepresentation::DoubleRNS(_)) => RNSFactorCongruence::Zero,
                            (
                                RNSFactorCongruence::CongruentTo(other_ring, other_i, _),
                                ManagedDoubleRNSElRepresentation::Both(small_basis_part, _),
                            )
                            | (
                                RNSFactorCongruence::CongruentTo(other_ring, other_i, _),
                                ManagedDoubleRNSElRepresentation::SmallBasis(small_basis_part),
                            ) => RNSFactorCongruence::CongruentTo(
                                &other_ring.base,
                                *other_i,
                                small_basis_part.unwrap_borrowed(),
                            ),
                            (
                                RNSFactorCongruence::CongruentTo(other_ring, other_i, _),
                                ManagedDoubleRNSElRepresentation::Sum(parts),
                            ) => RNSFactorCongruence::CongruentTo(&other_ring.base, *other_i, &parts.0),
                            (RNSFactorCongruence::Zero, _) => unreachable!(),
                        });
                let congruences_doublerns =
                    rns_factors
                        .iter()
                        .zip(reprs.iter())
                        .map(|(congruence, repr)| match (congruence, repr) {
                            ...
                            | (_, ManagedDoubleRNSElRepresentation::Both(..)) => RNSFactorCongruence::Zero,
                            (
                                RNSFactorCongruence::CongruentTo(other_ring, other_i, _),
                                ManagedDoubleRNSElRepresentation::DoubleRNS(doublerns_part),
                            ) => RNSFactorCongruence::CongruentTo(
                                &other_ring.base,
                                *other_i,
                                doublerns_part.unwrap_borrowed(),
                            ),
                            (
                                RNSFactorCongruence::CongruentTo(other_ring, other_i, _),
                                ManagedDoubleRNSElRepresentation::Sum(parts),
                            ) => RNSFactorCongruence::CongruentTo(&other_ring.base, *other_i, &parts.1),
                            (RNSFactorCongruence::Zero, _) => unreachable!(),
                        });
                self.from_sum(
                    self.base.collect_rns_factors_non_fft(congruences_small_basis),
    ...
    where
        I: Iterator<Item = super::RNSFactorCongruence<'a, Self, Self::PreparedMultiplicant>>,
        Self: 'a,
    {
        ()
    }
    ...
    fn two_by_two_convolution_square(&self, val: [Self::Element; 2]) -> [Self::Element; 3] {
        let res = self
            .base
            .two_by_two_convolution_square(val.map(|x| self.into_doublerns(x).unwrap_or_else(|| self.base.zero())));
        return res.map(|x| self.from_doublerns(x));
    }
}
    ...
    A: FheanorAllocator,
{
    ...
    fn number_ring(&self) -> &Self::NumberRing { &self.base.number_ring() }

    fn acting_galois_group(&self) -> &Subgroup<CyclotomicGaloisGroup> { self.base.acting_galois_group() }
    ...
    A: FheanorAllocator,
{
    ...
    fn eq_el(&self, lhs: &Self::Element, rhs: &Self::Element) -> bool {
        ...
        match (lhs_repr, rhs_repr) {
            (ManagedDoubleRNSElRepresentationKind::Zero, _) | (_, ManagedDoubleRNSElRepresentationKind::Zero) => {
                self.is_zero(lhs)
            }
            (ManagedDoubleRNSElRepresentationKind::SmallBasis, _)
            | (_, ManagedDoubleRNSElRepresentationKind::SmallBasis) => self
                .base
                .eq_el_non_fft(&*self.to_small_basis(lhs).unwrap(), &*self.to_small_basis(rhs).unwrap()),
            _ => self
                .base
                .eq_el(&*self.to_doublerns(lhs).unwrap(), &*self.to_doublerns(rhs).unwrap()),
        }
    }
    ...
    fn is_zero(&self, value: &Self::Element) -> bool {
        ...
        match val_repr {
            ...
            ManagedDoubleRNSElRepresentationKind::Sum
            | ManagedDoubleRNSElRepresentationKind::SmallBasis
            | ManagedDoubleRNSElRepresentationKind::Both => self
                .base
                .eq_el_non_fft(&*self.to_small_basis(value).unwrap(), &self.zero),
    ...
    fn fma(&self, lhs: &Self::Element, rhs: &Self::Element, summand: Self::Element) -> Self::Element {
        if let (Some(lhs), Some(rhs)) = (self.to_doublerns(lhs), self.to_doublerns(rhs)) {
            match summand.internal.get_repr() {
                ManagedDoubleRNSElRepresentation::DoubleRNS(double_rns_repr) => {
                    return self.from_doublerns(self.unmanaged_ring().fma(
                        lhs,
                        rhs,
                        double_rns_repr.to_owned(|x| self.unmanaged_ring().clone_el(x)),
                    ));
                }
                ManagedDoubleRNSElRepresentation::Sum(parts) => {
                    return self.from_sum(
                        ...
                        self.unmanaged_ring()
                            .fma(lhs, rhs, self.unmanaged_ring().clone_el(&parts.1)),
                    );
                }
                ManagedDoubleRNSElRepresentation::Zero => {
    ...
    fn sub_assign(&self, lhs: &mut Self::Element, rhs: Self::Element) { take_mut::take(lhs, |x| self.sub(x, rhs)); }

    fn mul_assign_int(&self, lhs: &mut Self::Element, rhs: i32) { take_mut::take(lhs, |x| self.mul_int(x, rhs)); }
    ...
    fn mul_ref_fst(&self, lhs: &Self::Element, rhs: Self::Element) -> Self::Element { self.mul_ref(lhs, &rhs) }

    fn mul_ref_snd(&self, lhs: Self::Element, rhs: &Self::Element) -> Self::Element { self.mul_ref(&lhs, rhs) }

    fn mul(&self, lhs: Self::Element, rhs: Self::Element) -> Self::Element { self.mul_ref(&lhs, &rhs) }
}
    ...
    A: FheanorAllocator,
{
    ...
    default fn inner_product_ref<'a, I: Iterator<Item = (&'a Self::Element, &'a Self::Element)>>(
        &self,
        els: I,
    ) -> Self::Element
    where
        Self: 'a,
    {
        self.from_doublerns(<_ as ComputeInnerProduct>::inner_product_ref(
            &self.base,
            els.filter(|(l, r)| {
                l.internal.get_repr().get_kind() != ManagedDoubleRNSElRepresentationKind::Zero
                    && r.internal.get_repr().get_kind() != ManagedDoubleRNSElRepresentationKind::Zero
            })
            .map(|(l, r)| (self.to_doublerns(l).unwrap(), self.to_doublerns(r).unwrap())),
        ))
    }

    default fn inner_product_ref_fst<'a, I: Iterator<Item = (&'a Self::Element, Self::Element)>>(
        &self,
        els: I,
    ) -> Self::Element
    where
    ...
    A: FheanorAllocator,
{
    ...
    fn base_ring<'a>(&'a self) -> &'a Self::BaseRing { self.base.base_ring() }
    ...
    A: FheanorAllocator,
{
    ...
    fn rank(&self) -> usize { self.base.rank() }

    fn wrt_canonical_basis<'a>(&'a self, el: &'a Self::Element) -> Self::VectorRepresentation<'a> {
        if let Some(result) = self.to_small_basis(el) {
            self.base
                .wrt_canonical_basis_non_fft(self.base.clone_el_non_fft(&result))
        } else {
            self.base
                .wrt_canonical_basis_non_fft(self.base.clone_el_non_fft(&self.zero))
        }
    }
    ...
    A: FheanorAllocator,
{
    type ElementsIter<'a>
        = std::iter::Map<
        <DoubleRNSRingBase<NumberRing, A> as FiniteRing>::ElementsIter<'a>,
        fn(DoubleRNSEl<NumberRing, A>) -> ManagedDoubleRNSEl<NumberRing, A>,
    >
    where
        Self: 'a;
    ...
    A: FheanorAllocator,
{
        ...
        S: serde::Serializer,
    {
        if serializer.is_human_readable() {
            return SerializableNewtypeStruct::new(
                "ManagedDoubleRNSEl",
                &SerializableSmallBasisElWithRing::new(&self.base, self.to_small_basis(el).unwrap_or(&self.zero)),
            )
        ...
        if let ManagedDoubleRNSElRepresentation::DoubleRNS(double_rns_repr) = el.internal.get_repr() {
            serializer.serialize_newtype_variant(
                ...
                0,
                "DoubleRNS",
                &SerializeWithRing::new(&*double_rns_repr, RingRef::new(&self.base)),
            )
            ...
            serializer.serialize_newtype_variant(
                ...
                1,
                "SmallBasis",
                &SerializableSmallBasisElWithRing::new(&self.base, small_basis_repr),
            )
        } else {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        if deserializer.is_human_readable() {
            return DeserializeSeedNewtypeStruct::new(
                "ManagedDoubleRNSEl",
                DeserializeSeedSmallBasisElWithRing::new(&self.base),
            )
            .deserialize(deserializer)
            .map(|small_basis_repr| self.from_small_basis(small_basis_repr));
        }
            ...
            A: FheanorAllocator,
        {
            ...
            fn expecting(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
                write!(
                    f,
                    "either ManagedDoubleRNSEl::DoubleRNS, ManagedDoubleRNSEl::SmallBasis or ManagedDoubleRNSEl::Zero"
                )
            }
                ...
                E: EnumAccess<'de>,
            {
                ...
                impl<'de> serde::de::Visitor<'de> for DiscriminantVisitor {
                    ...
                    fn expecting(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
                        write!(
                            f,
                            "one of the enum discriminants `DoubleRNS` = 0, `SmallBasis` = 1 or `Zero` = 2"
                        )
                    }
                ...
                match discriminant {
                    Discriminant::DoubleRNS => variant
                        .newtype_variant_seed(DeserializeWithRing::new(self.ring.unmanaged_ring()))
                        .map(|double_rns_repr| self.ring.from_doublerns(double_rns_repr)),
                    Discriminant::SmallBasis => variant
                        .newtype_variant_seed(DeserializeSeedSmallBasisElWithRing::new(
                            self.ring.unmanaged_ring().get_ring(),
                        ))
                        .map(|small_basis_repr| self.ring.from_small_basis(small_basis_repr)),
...
impl<NumberRing, A1, A2, C> CanHomFrom<SingleRNSRingBase<NumberRing, A1, C>>
    for ManagedDoubleRNSRingBase<NumberRing, A2>
where
    NumberRing: NumberRingDescriptor,
    A1: FheanorAllocator,
    A2: FheanorAllocator,
...
{
    type Homomorphism =
        <DoubleRNSRingBase<NumberRing, A2> as CanHomFrom<SingleRNSRingBase<NumberRing, A1, C>>>::Homomorphism;
    ...
    fn map_in(
        &self,
        from: &SingleRNSRingBase<NumberRing, A1, C>,
        el: <SingleRNSRingBase<NumberRing, A1, C> as RingBase>::Element,
        hom: &Self::Homomorphism,
    ) -> Self::Element {
        if from.is_zero(&el) {
            return self.zero();
...
{
    type Homomorphism =
        <DoubleRNSRingBase<NumberRing, A2> as CanHomFrom<DoubleRNSRingBase<NumberRing, A2>>>::Homomorphism;
    ...
    fn map_in(
        &self,
        from: &DoubleRNSRingBase<NumberRing, A1>,
        el: <DoubleRNSRingBase<NumberRing, A1> as RingBase>::Element,
        hom: &Self::Homomorphism,
    ) -> Self::Element {
        if from.is_zero(&el) {
            return self.zero();
...
impl<NumberRing, A1, A2> CanHomFrom<ManagedDoubleRNSRingBase<NumberRing, A1>>
    for ManagedDoubleRNSRingBase<NumberRing, A2>
where
    NumberRing: NumberRingDescriptor,
    A1: FheanorAllocator,
    A2: FheanorAllocator,
{
    ...
    fn map_in_ref(
        &self,
        from: &ManagedDoubleRNSRingBase<NumberRing, A1>,
        el: &<ManagedDoubleRNSRingBase<NumberRing, A1> as RingBase>::Element,
        hom: &Self::Homomorphism,
    ) -> Self::Element {
...
impl<NumberRing, A1, A2, C> CanIsoFromTo<SingleRNSRingBase<NumberRing, A1, C>>
    for ManagedDoubleRNSRingBase<NumberRing, A2>
where
    NumberRing: NumberRingDescriptor,
    A1: FheanorAllocator,
...
{
    type Isomorphism =
        <DoubleRNSRingBase<NumberRing, A2> as CanIsoFromTo<SingleRNSRingBase<NumberRing, A1, C>>>::Isomorphism;
    ...
    fn map_out(
        &self,
        to: &SingleRNSRingBase<NumberRing, A1, C>,
        el: Self::Element,
        iso: &Self::Isomorphism,
    ) -> <SingleRNSRingBase<NumberRing, A1, C> as RingBase>::Element {
        if let Some(el) = self.to_small_basis(&el) {
            self.base
                .map_out_to_singlerns(to, self.base.clone_el_non_fft(&*&el), iso)
        } else {
            to.zero()
        }
    }
...
{
    type Isomorphism =
        <DoubleRNSRingBase<NumberRing, A2> as CanIsoFromTo<DoubleRNSRingBase<NumberRing, A2>>>::Isomorphism;
    ...
    fn map_out(
        &self,
        to: &DoubleRNSRingBase<NumberRing, A1>,
        el: Self::Element,
        iso: &Self::Isomorphism,
    ) -> <DoubleRNSRingBase<NumberRing, A1> as RingBase>::Element {
...
impl<NumberRing, A1, A2> CanIsoFromTo<ManagedDoubleRNSRingBase<NumberRing, A1>>
    for ManagedDoubleRNSRingBase<NumberRing, A2>
where
    NumberRing: NumberRingDescriptor,
    A1: FheanorAllocator,
...
{
    type Isomorphism =
        <DoubleRNSRingBase<NumberRing, A1> as CanHomFrom<DoubleRNSRingBase<NumberRing, A2>>>::Homomorphism;
    ...
    fn map_out(
        &self,
        to: &ManagedDoubleRNSRingBase<NumberRing, A1>,
        el: Self::Element,
        iso: &Self::Isomorphism,
    ) -> <ManagedDoubleRNSRingBase<NumberRing, A1> as RingBase>::Element {
...
#[cfg(test)]
fn ring_and_elements() -> (
    ManagedDoubleRNSRing<Pow2CyclotomicNumberRing>,
    Vec<El<ManagedDoubleRNSRing<Pow2CyclotomicNumberRing>>>,
) {
    ...
    assert_el_eq!(
        ...
        ring.sum(
            elements
                ...
                .map(|(l, r)| ring.mul_ref(l, r))
        ),
        ring.get_ring()
            .inner_product_ref(elements.iter().zip(elements.iter().skip(2)))
    );
}
...
fn test_thread_safe() {
    ...
    let test_element = Arc::new(
        ring.get_ring().from_sum(
            ring.get_ring()
                .base
                .from_non_fft(ring.get_ring().base.base_ring().int_hom().map(1)),
            ring.get_ring()
                .base
                .from(ring.get_ring().base.base_ring().int_hom().map(10)),
        ),
    );
    ...
    for _ in 0..n {
        ...
        threads.push(std::thread::spawn(move || {
            ...
            assert_el_eq!(
                ring,
                ring.int_hom().map(121),
                ring.pow(ring.clone_el(&*test_element), 2)
            );
        }))
    }
...
fn test_canonical_hom_from_doublerns() {
    ...
    let elements = vec![
        ...
        doublerns_ring
            .int_hom()
            .mul_map(doublerns_ring.pow(doublerns_ring.canonical_gen(), 15), 17),
    ...
    feanor_math::ring::generic_tests::test_hom_axioms(
        doublerns_ring,
        &ring,
        elements.iter().map(|x| doublerns_ring.clone_el(x)),
    );
}
...
fn test_canonical_hom_from_singlerns() {
    ...
    let elements = vec![
        ...
        singlerns_ring
            .int_hom()
            .mul_map(singlerns_ring.pow(singlerns_ring.canonical_gen(), 15), 97),
    ...
    feanor_math::ring::generic_tests::test_hom_axioms(
        &singlerns_ring,
        &ring,
        elements.iter().map(|x| singlerns_ring.clone_el(x)),
    );
}
...
fn test_add_result_independent_of_repr() {
    ...
    let reprs_of_11: [Box<dyn Fn() -> ManagedDoubleRNSEl<_, _>>; 4] = [
        Box::new(|| {
            ring.get_ring()
                .from_small_basis(base.from_non_fft(base.base_ring().int_hom().map(11)))
        }),
        Box::new(|| {
            ring.get_ring()
                .from_doublerns(base.from(base.base_ring().int_hom().map(11)))
        }),
        Box::new(|| {
            ring.get_ring().from_sum(
                base.from_non_fft(base.base_ring().int_hom().map(10)),
                base.from(base.base_ring().int_hom().map(1)),
            )
        }),
        Box::new(|| {
            ring.get_ring().from_both(
                base.from_non_fft(base.base_ring().int_hom().map(11)),
                base.from(base.base_ring().int_hom().map(11)),
            )
        }),
    ];
    let reprs_of_102: [Box<dyn Fn() -> ManagedDoubleRNSEl<_, _>>; 4] = [
        Box::new(|| {
            ring.get_ring()
                .from_small_basis(base.from_non_fft(base.base_ring().int_hom().map(102)))
        }),
        Box::new(|| {
            ring.get_ring()
                .from_doublerns(base.from(base.base_ring().int_hom().map(102)))
        }),
        Box::new(|| {
            ring.get_ring().from_sum(
                base.from_non_fft(base.base_ring().int_hom().map(100)),
                base.from(base.base_ring().int_hom().map(2)),
            )
        }),
        Box::new(|| {
            ring.get_ring().from_both(
                base.from_non_fft(base.base_ring().int_hom().map(102)),
                base.from(base.base_ring().int_hom().map(102)),
            )
        }),
    ];
    for a in &reprs_of_11 {
        for b in &reprs_of_102 {
            ...
            assert_el_eq!(
                RingRef::new(base),
                base.from_int(22),
                ring.get_ring().to_doublerns(&ring.add_ref(&x, &x)).unwrap()
            );
            ...
            assert_el_eq!(
                RingRef::new(base),
                base.from_int(113),
                ring.get_ring().to_doublerns(&ring.add_ref(&x, &y)).unwrap()
            );

            let x = a();
            let y = b();
            assert!(base.eq_el_non_fft(
                &base.from_non_fft(base.base_ring().int_hom().map(113)),
                &*ring.get_ring().to_small_basis(&ring.add_ref(&x, &y)).unwrap()
            ));
            ...
            assert_el_eq!(
                RingRef::new(base),
                base.from_int(-91),
                ring.get_ring().to_doublerns(&ring.sub_ref(&x, &y)).unwrap()
            );

            let x = a();
            let y = b();
            assert!(base.eq_el_non_fft(
                &base.from_non_fft(base.base_ring().int_hom().map(-91)),
                &*ring.get_ring().to_small_basis(&ring.sub_ref(&x, &y)).unwrap()
            ));
            ...
            assert_el_eq!(
                RingRef::new(base),
                base.from_int(121),
                ring.get_ring().to_doublerns(&ring.mul_ref(&x, &x)).unwrap()
            );
            ...
            assert_el_eq!(
                RingRef::new(base),
                base.from_int(1122),
                ring.get_ring().to_doublerns(&ring.mul_ref(&x, &y)).unwrap()
            );

            let x = a();
            let y = b();
            assert!(base.eq_el_non_fft(
                &base.from_non_fft(base.base_ring().int_hom().map(1122)),
                &*ring.get_ring().to_small_basis(&ring.mul_ref(&x, &y)).unwrap()
            ));
        }
    }
}
...
fn test_serialization() {
    ...
    for a in &elements {
        ...
        let a_small_basis = ring.get_ring().from_small_basis(
            ring.get_ring()
                .unmanaged_ring()
                .get_ring()
                .clone_el_non_fft(ring.get_ring().to_small_basis(a).unwrap()),
        );
        ...
        let a_doublerns = ring.get_ring().from_doublerns(
            ring.get_ring()
                .unmanaged_ring()
                .clone_el(ring.get_ring().to_doublerns(a).unwrap()),
        );
        let serializer = serde_assert::Serializer::builder().is_human_readable(true).build();
