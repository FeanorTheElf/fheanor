        ...
        A: Allocator + Clone
{
    ...
    el_wrt_mult_basis: Vec<El<zn_64::Zn>, A>
}
        ...
        A: Allocator + Clone
{
    number_ring: PhantomData<NumberRing>,
    allocator: PhantomData<A>,
    el_wrt_small_basis: Vec<El<zn_64::Zn>, A>
}
        ...
        A: Allocator + Clone
{
    ...
    /// 
    #[instrument(skip_all)]
    pub fn new_with_alloc(number_ring: NumberRing, rns_base: zn_rns::Zn<zn_64::Zn, BigIntRing>, allocator: A) -> RingValue<Self> {
    ...
    /// 
    pub fn as_matrix_wrt_small_basis<'a>(&self, element: &'a SmallBasisEl<NumberRing, A>) -> Submatrix<'a, AsFirstElement<El<zn_64::Zn>>, El<zn_64::Zn>> {
    ...
    /// Note that the actual choice of the "small basis" is up to the used [`AbstractNumberRing`].
    /// 
    pub fn as_matrix_wrt_small_basis_mut<'a>(&self, element: &'a mut SmallBasisEl<NumberRing, A>) -> SubmatrixMut<'a, AsFirstElement<El<zn_64::Zn>>, El<zn_64::Zn>> {
    ...
    ///
    pub fn as_matrix_wrt_mult_basis<'a>(&self, element: &'a DoubleRNSEl<NumberRing, A>) -> Submatrix<'a, AsFirstElement<El<zn_64::Zn>>, El<zn_64::Zn>> {
    ...
    /// Note that the actual choice of the "multiplicative basis" is up to the used [`AbstractNumberRing`].
    ///
    pub fn as_matrix_wrt_mult_basis_mut<'a>(&self, element: &'a mut DoubleRNSEl<NumberRing, A>) -> SubmatrixMut<'a, AsFirstElement<El<zn_64::Zn>>, El<zn_64::Zn>> {
    ...
    /// 
    #[instrument(skip_all)]
    pub fn undo_fft_partial<V>(&self, element: &DoubleRNSEl<NumberRing, A>, required_rns_factors: &[usize], mut output: SubmatrixMut<V, El<zn_64::Zn>>)
    ...
    #[instrument(skip_all)]
    pub fn as_representation_wrt_small_generating_set_non_fft<V>(&self, x: &SmallBasisEl<NumberRing, A>, output: SubmatrixMut<V, El<zn_64::Zn>>)
        where V: AsPointerToSlice<El<zn_64::Zn>>
    {
    ...
    #[instrument(skip_all)]
    pub fn from_representation_wrt_small_generating_set_non_fft<V>(&self, data: Submatrix<V, El<zn_64::Zn>>) -> SmallBasisEl<NumberRing, A>
        where V: AsPointerToSlice<El<zn_64::Zn>>
    {
        ...
        A: Allocator + Clone
{
    ...
    fn zero(&self) -> Self::Element {
        let mut result = Vec::with_capacity_in(self.element_len(), self.allocator.clone());
        result.extend(self.base_ring().as_iter().flat_map(|rns_factor| (0..self.rank()).map(|_| rns_factor.zero())));
        return DoubleRNSEl {
            el_wrt_mult_basis: result,
            number_ring: PhantomData,
            allocator: PhantomData
...
pub struct DoubleRNSRingBaseElVectorRepresentation<'a, NumberRing, A> 
    where NumberRing: AbstractNumberRing,
        A: Allocator + Clone
{
    el_wrt_coeff_basis: Vec<El<zn_64::Zn>, A>,
        ...
        A: Allocator + Clone
{
        ...
        where S: serde::Serializer 
    {
        if serializer.is_human_readable() {
            SerializableNewtypeStruct::new("RingEl", SerializableSeq::new_with_len(self.ring.wrt_canonical_basis_non_fft(self.ring.clone_el_non_fft(self.el)).iter().map(|c| SerializeOwnedWithRing::new(c, self.ring.base_ring())), self.ring.rank())).serialize(serializer)
        } else {
            SerializableNewtypeStruct::new("SmallBasisEl", &serialize_rns_data(self.ring.base_ring(), self.ring.as_matrix_wrt_small_basis(self.el))).serialize(serializer)
        }
    }
}
        ...
        A: Allocator + Clone
{
        ...
        where D: Deserializer<'de>
    {
        if deserializer.is_human_readable() {
            ...
            DeserializeSeedNewtypeStruct::new("SmallBasisEl", deserialize_rns_data(self.ring.base_ring(), self.ring.as_matrix_wrt_small_basis_mut(&mut result))).deserialize(deserializer)?;
            return Ok(result);
        }
    }
}
        ...
        A: Allocator + Clone
{
    ...
    {
        if serializer.is_human_readable() {
            SerializableNewtypeStruct::new("RingEl", &SerializableSmallBasisElWithRing::new(self, &self.undo_fft(self.clone_el(el)))).serialize(serializer)
        } else {
            SerializableNewtypeStruct::new("DoubleRNSEl", &serialize_rns_data(self.base_ring(), self.as_matrix_wrt_mult_basis(el))).serialize(serializer)
        }
    }
    ...
    {
        if deserializer.is_human_readable() {
            DeserializeSeedNewtypeStruct::new("RingEl", DeserializeSeedSmallBasisElWithRing::new(self)).deserialize(deserializer).map(|x| self.do_fft(x))
        } else {
            let mut result = self.zero();
...
pub fn test_with_number_ring<NumberRing: Clone + AbstractNumberRing>(number_ring: NumberRing) {
    ...
    let dropped_rns_factor_ring = DoubleRNSRingBase::new(number_ring.clone(), zn_rns::Zn::new(vec![zn_64::Zn::new(p2 as u64)], BigIntRing::RING));
    for a in &elements {
        assert_el_eq!(
        ...
        assert_el_eq!(
            &dropped_rns_factor_ring,
            dropped_rns_factor_ring.from_canonical_basis(ring.wrt_canonical_basis(a).iter().map(|c| dropped_rns_factor_ring.base_ring().from_congruence([*ring.base_ring().get_congruence(&c).at(1)].into_iter()))),
            dropped_rns_factor_ring.get_ring().do_fft(dropped_rns_factor_ring.get_ring().drop_rns_factor_non_fft_element(ring.get_ring(), &RNSFactorIndexList::from([0], ring.base_ring().len()), &ring.get_ring().undo_fft(ring.clone_el(a))))
        );
    }
    for a in &elements {
        ...
        assert_el_eq!(
            &ring,
            ring.from_canonical_basis(ring.wrt_canonical_basis(a).iter().map(|c| ring.base_ring().from_congruence([ring.base_ring().at(0).zero(), *ring.base_ring().get_congruence(&c).at(1)].into_iter()))),
            ring.get_ring().do_fft(ring.get_ring().add_rns_factor_non_fft_element(dropped_rns_factor_ring.get_ring(), &RNSFactorIndexList::from([0], ring.base_ring().len()), &dropped_rns_factor_ring.get_ring().undo_fft(dropped_factor_a)))
        );
    }

    let dropped_rns_factor_ring = DoubleRNSRingBase::new(number_ring.clone(), zn_rns::Zn::new(vec![zn_64::Zn::new(p1 as u64)], BigIntRing::RING));
    for a in &elements {
        assert_el_eq!(
            &dropped_rns_factor_ring,
        ...
        assert_el_eq!(
            &dropped_rns_factor_ring,
            dropped_rns_factor_ring.from_canonical_basis(ring.wrt_canonical_basis(a).iter().map(|c| dropped_rns_factor_ring.base_ring().from_congruence([*ring.base_ring().get_congruence(&c).at(0)].into_iter()))),
            dropped_rns_factor_ring.get_ring().do_fft(dropped_rns_factor_ring.get_ring().drop_rns_factor_non_fft_element(ring.get_ring(), &RNSFactorIndexList::from([1], ring.base_ring().len()), &ring.get_ring().undo_fft(ring.clone_el(a))))
        );
    }
    for a in &elements {
        ...
        assert_el_eq!(
            &ring,
            ring.from_canonical_basis(ring.wrt_canonical_basis(a).iter().map(|c| ring.base_ring().from_congruence([*ring.base_ring().get_congruence(&c).at(0), ring.base_ring().at(1).zero()].into_iter()))),
            ring.get_ring().do_fft(ring.get_ring().add_rns_factor_non_fft_element(dropped_rns_factor_ring.get_ring(), &RNSFactorIndexList::from([1], ring.base_ring().len()), &dropped_rns_factor_ring.get_ring().undo_fft(dropped_factor_a)))
        );
    }
...
fn test_nonprime_rns_factors() {
    ...
    let Zq = zn_rns::Zn::new(vec![zn_64::Zn::new(257 * 257), zn_64::Zn::new(65537)], ZZbig);
