...
/// operations like [`FreeAlgebra::from_canonical_basis()`] and
/// [`FreeAlgebra::wrt_canonical_basis()`] are expensive, and in some cases only supported for
/// elements that have explicitly been converted to [`SmallBasisEl`] via
/// [`DoubleRNSRingBase::undo_fft()`]. This conversion is expensive in terms of performance.
///
...
/// In particular, multiplication of elements refers to component-wise multiplication of these
/// vectors.
    ...
    A: FheanorAllocator,
{
    ...
    /// The number ring modulo each RNS factor `pi`, use for conversion between small and
    ...
    NumberRing: NumberRingDescriptor,
{
    ...
    pub fn new_with_modulus_size(
        number_ring: NumberRing,
        first_primes: Option<&zn_rns::Zn<Zn, BigIntRing>>,
        log2_modulus: Range<usize>,
    ) -> RingValue<Self> {
    ...
    A: FheanorAllocator,
{
    ...
    pub fn new_with_alloc(
        ...
        RingValue::from(Self {
            ...
            ring_decompositions: rns_base
                .as_iter()
                .map(|Fp| Arc::new(number_ring.bases_mod_p(Fp.clone())))
                .collect(),
            number_ring,
            rns_base,
            allocator,
    ...
    #[instrument(skip_all)]
    pub fn new_with_modulus_size_alloc(
        number_ring: NumberRing,
        first_primes: Option<&zn_rns::Zn<Zn, BigIntRing>>,
        log2_modulus: Range<usize>,
        allocator: A,
        ...
        let modulus = extend_sampled_primes(&begin_with, log2_modulus.start, log2_modulus.end, 57, |bound| {
            largest_prime_leq_congruent_to_one(bound, required_root_of_unity_order)
        })
        .unwrap();
    ...
    pub fn ring_decompositions<'a>(
        &'a self,
    ) -> impl VectorFn<&'a <NumberRing as NumberRingDescriptor>::NumberRingQuotientBases> + use<'a, NumberRing, A> {
        self.ring_decompositions.as_fn().map_fn(|x| &**x)
    }

    fn element_len(&self) -> usize { self.rank() * self.base_ring().len() }

    pub fn element_allocation_size(&self) -> usize { self.rank() * self.base_ring().len() * size_of::<El<Zn>>() }

    /// Returns a view on the representation of the given element w.r.t. the small basis.
    /// In particular, the `i`-th row of the returned matrix contains the coefficients of the
    /// element modulo the `i`-th RNS factor w.r.t. the small basis, as specified by the `i`-th
    /// ring decomposition.
    ///
    /// Note that the actual choice of the "small basis" is up to the used [`NumberRingDescriptor`].
    pub fn as_matrix_wrt_small_basis<'a>(
        &self,
        element: &'a SmallBasisEl<NumberRing, A>,
    ) -> Submatrix<'a, AsFirstElement<ZnEl>, ZnEl> {
    ...
    ///
    /// Note that the actual choice of the "small basis" is up to the used [`NumberRingDescriptor`].
    pub fn as_matrix_wrt_small_basis_mut<'a>(
        &self,
        element: &'a mut SmallBasisEl<NumberRing, A>,
    ) -> SubmatrixMut<'a, AsFirstElement<ZnEl>, ZnEl> {
    ...
    /// In particular, the `i`-th row of the returned matrix contains the coefficients of the
    /// element modulo the `i`-th RNS factor w.r.t. the multiplicative basis, as specified by
    /// the `i`-th ring decomposition.
    ///
    /// Note that the actual choice of the "multiplicative basis" is up to the used
    /// [`NumberRingDescriptor`].
    pub fn as_matrix_wrt_mult_basis<'a>(
        &self,
        element: &'a DoubleRNSEl<NumberRing, A>,
    ) -> Submatrix<'a, AsFirstElement<ZnEl>, ZnEl> {
        Submatrix::from_1d(&element.el_wrt_mult_basis, self.base_ring().len(), self.rank())
    }

    /// Returns a mutable view on the representation of the given element w.r.t. the multiplicative
    /// basis. In particular, the `i`-th row of the returned matrix contains the coefficients of
    /// the element modulo the `i`-th RNS factor w.r.t. the multiplicative basis, as specified
    /// by the `i`-th ring decomposition.
    ///
    /// Note that the actual choice of the "multiplicative basis" is up to the used
    /// [`NumberRingDescriptor`].
    pub fn as_matrix_wrt_mult_basis_mut<'a>(
        &self,
        element: &'a mut DoubleRNSEl<NumberRing, A>,
    ) -> SubmatrixMut<'a, AsFirstElement<ZnEl>, ZnEl> {
    ...
    fn rns_parts_mut<'a>(
        &self,
        data: &'a mut [ZnEl],
    ) -> impl IntoIterator<IntoIter = std::slice::ChunksExactMut<'a, ZnEl>, Item = &'a mut [ZnEl]>
    + IntoParallelIterator<Iter = rayon::slice::ChunksExactMut<'a, ZnEl>, Item = &'a mut [ZnEl]> {
    ...
    fn rns_parts<'a>(
        &self,
        data: &'a [ZnEl],
    ) -> impl IntoIterator<IntoIter = std::slice::ChunksExact<'a, ZnEl>, Item = &'a [ZnEl]>
    + IntoParallelIterator<Iter = rayon::slice::ChunksExact<'a, ZnEl>, Item = &'a [ZnEl]> {
    ...
    ///
    /// The coefficients w.r.t. this basis can be accessed using
    /// [`DoubleRNSRingBase::as_matrix_wrt_small_basis()`].
    #[instrument(skip_all)]
    pub fn undo_fft(&self, mut element: DoubleRNSEl<NumberRing, A>) -> SmallBasisEl<NumberRing, A> {
        ...
        CondIterator::new(self.rns_parts_mut(&mut element.el_wrt_mult_basis), is_parallel())
            .enumerate()
            .for_each(|(i, part)| {
                span!(parent: &outer_span, Level::INFO, "undo_fft_block")
                    .in_scope(|| self.ring_decompositions[i].mult_basis_to_small_basis(part))
            });
        SmallBasisEl {
    ...
    /// Like [`DoubleRNSRingBase::undo_fft()`], this converts an element to its representation
    /// w.r.t. the small basis. However, unlike [`DoubleRNSRingBase::undo_fft()`], it only
    /// outputs the result for a subset of RNS factors. Thus, the output cannot be a
    /// [`SmallBasisEl`], but instead this function directly writes the small basis
    /// representation of the output to the given matrix. This representation is the same that
    /// would be returned when calling [`DoubleRNSRingBase::as_matrix_wrt_small_basis()`].
    #[instrument(skip_all)]
    pub fn undo_fft_partial<V>(
        &self,
        element: &DoubleRNSEl<NumberRing, A>,
        required_rns_factors: &[usize],
        mut output: SubmatrixMut<V, ZnEl>,
    ) where
    ...
    pub fn from_non_fft(&self, x: El<<Self as RingExtension>::BaseRing>) -> SmallBasisEl<NumberRing, A> {
        ...
        for (i, Zp) in self.base_ring().as_iter().enumerate() {
            ...
            self.ring_decompositions[i]
                .coeff_basis_to_small_basis(&mut result[(i * self.rank())..((i + 1) * self.rank())]);
        }
        SmallBasisEl {
    ...
    /// You can also access the coefficients w.r.t. the multiplicative basis using
    /// [`DoubleRNSRingBase::as_matrix_wrt_mult_basis()`].
    #[instrument(skip_all)]
    pub fn do_fft(&self, mut element: SmallBasisEl<NumberRing, A>) -> DoubleRNSEl<NumberRing, A> {
        ...
        let outer_span = Span::current();
        CondIterator::new(self.rns_parts_mut(&mut element.el_wrt_small_basis), is_parallel())
            .enumerate()
            .for_each(|(i, part)| {
                span!(parent: &outer_span, Level::INFO, "do_fft_block")
                    .in_scope(|| self.ring_decompositions[i].small_basis_to_mult_basis(part))
            });
        DoubleRNSEl {
    ...
    #[instrument(skip_all)]
    pub fn sample_from_coefficient_distribution<G: FnMut() -> i32>(
        &self,
        mut distribution: G,
    ...
    pub fn eq_el_non_fft(&self, lhs: &SmallBasisEl<NumberRing, A>, rhs: &SmallBasisEl<NumberRing, A>) -> bool {
        ...
        for i in 0..self.base_ring().len() {
            for j in 0..self.rank() {
                if !self.base_ring().at(i).eq_el(
                    &lhs.el_wrt_small_basis[i * self.rank() + j],
                    &rhs.el_wrt_small_basis[i * self.rank() + j],
                ) {
                    return false;
                }
            }
    ...
    pub fn negate_inplace_non_fft(&self, val: &mut SmallBasisEl<NumberRing, A>) {
        assert_eq!(self.element_len(), val.el_wrt_small_basis.len());
        let outer_span = Span::current();
        CondIterator::new(self.rns_parts_mut(&mut val.el_wrt_small_basis), is_parallel())
            .enumerate()
            .for_each(|(i, part)| {
                span!(parent: &outer_span, Level::INFO, "negate_block")
                    .in_scope(|| part.iter_mut().for_each(|x| self.rns_base.at(i).negate_inplace(x)))
            });
    }
    ...
    pub fn sub_assign_non_fft(&self, lhs: &mut SmallBasisEl<NumberRing, A>, rhs: &SmallBasisEl<NumberRing, A>) {
        ...
        assert_eq!(self.element_len(), rhs.el_wrt_small_basis.len());
        let outer_span = Span::current();
        CondIterator::new(self.rns_parts_mut(&mut lhs.el_wrt_small_basis), is_parallel())
            .zip(self.rns_parts(&rhs.el_wrt_small_basis))
            .enumerate()
            .for_each(|(i, (lhs_part, rhs_part))| {
                span!(parent: &outer_span, Level::INFO, "sub_block").in_scope(|| {
                    lhs_part
                        .iter_mut()
                        .zip(rhs_part)
                        .for_each(|(l, r)| self.rns_base.at(i).sub_assign_ref(l, r))
                })
            });
    }

    #[instrument(skip_all)]
    pub fn mul_scalar_assign_non_fft(
        &self,
        lhs: &mut SmallBasisEl<NumberRing, A>,
        rhs: &El<zn_rns::Zn<Zn, BigIntRing>>,
    ) {
        assert_eq!(self.element_len(), lhs.el_wrt_small_basis.len());
        ...
        CondIterator::new(self.rns_parts_mut(&mut lhs.el_wrt_small_basis), is_parallel())
            .enumerate()
            .for_each(|(i, lhs_part)| {
                span!(parent: &outer_span, Level::INFO, "mul_scalar_block").in_scope(|| {
                    lhs_part
                        .iter_mut()
                        .for_each(|x| self.rns_base.at(i).mul_assign_ref(x, rhs_congruence.at(i)))
                })
            });
    }
    ...
    pub fn add_assign_non_fft(&self, lhs: &mut SmallBasisEl<NumberRing, A>, rhs: &SmallBasisEl<NumberRing, A>) {
        ...
        CondIterator::new(self.rns_parts_mut(&mut lhs.el_wrt_small_basis), is_parallel())
            ...
            .enumerate()
            .for_each(|(i, (lhs_part, rhs_part))| {
                span!(parent: &outer_span, Level::INFO, "add_block").in_scope(|| {
                    lhs_part
                        .iter_mut()
                        .zip(rhs_part)
                        .for_each(|(l, r)| self.rns_base.at(i).add_assign_ref(l, r))
                })
            });
    }
    ...
    where
        V: IntoIterator<Item = El<<Self as RingExtension>::BaseRing>>,
    {
        let mut result = self.zero_non_fft().el_wrt_small_basis;
        ...
        let outer_span = Span::current();
        CondIterator::new(self.rns_parts_mut(&mut result), is_parallel())
            .enumerate()
            .for_each(|(i, part)| {
                span!(parent: &outer_span, Level::INFO, "coeff_to_small_basis_block")
                    .in_scope(|| self.ring_decompositions[i].coeff_basis_to_small_basis(part))
            });
        return SmallBasisEl {
            el_wrt_small_basis: result,
            allocator: PhantomData,
    ...
    #[instrument(skip_all)]
    pub fn as_representation_wrt_small_generating_set_non_fft<'a>(
        &'a self,
        x: &'a SmallBasisEl<NumberRing, A>,
    ) -> impl 'a + AsSubmatrix<El<Zn>> {
        self.as_matrix_wrt_small_basis(x)
    }

    #[instrument(skip_all)]
    pub fn from_representation_wrt_small_generating_set_non_fft<F>(&self, initializer: F) -> SmallBasisEl<NumberRing, A>
    where
        F: FnOnce(
    ...
    #[instrument(skip_all)]
    pub fn wrt_canonical_basis_non_fft<'a>(
        &'a self,
        el: SmallBasisEl<NumberRing, A>,
    ) -> DoubleRNSRingBaseElVectorRepresentation<'a, NumberRing, A> {
        let mut result = el.el_wrt_small_basis;
        let outer_span = Span::current();
        CondIterator::new(self.rns_parts_mut(&mut result), is_parallel())
            .enumerate()
            .for_each(|(i, part)| {
                span!(parent: &outer_span, Level::INFO, "small_to_coeff_basis_block")
                    .in_scope(|| self.ring_decompositions[i].small_basis_to_coeff_basis(part))
            });
    ...
    #[instrument(skip_all)]
    pub fn map_in_from_singlerns<A2, C>(
        &self,
        from: &SingleRNSRingBase<NumberRing, A2, C>,
        mut el: El<SingleRNSRing<NumberRing, A2, C>>,
        hom: &<Self as CanHomFrom<SingleRNSRingBase<NumberRing, A2, C>>>::Homomorphism,
    ) -> SmallBasisEl<NumberRing, A>
    where
        A2: FheanorAllocator,
        C: ConvolutionAlgorithm<ZnBase>,
    {
        ...
        for (i, Zp) in self.base_ring().as_iter().enumerate() {
            for j in 0..self.rank() {
                result.push(Zp.get_ring().map_in_ref(
                    from.base_ring().at(i).get_ring(),
                    el_as_matrix.at(i, j),
                    &hom[i],
                ));
            }
    ...
    #[instrument(skip_all)]
    pub fn map_out_to_singlerns<A2, C>(
        &self,
        to: &SingleRNSRingBase<NumberRing, A2, C>,
        el: SmallBasisEl<NumberRing, A>,
        iso: &<Self as CanIsoFromTo<SingleRNSRingBase<NumberRing, A2, C>>>::Isomorphism,
    ) -> El<SingleRNSRing<NumberRing, A2, C>>
    where
        A2: FheanorAllocator,
        C: ConvolutionAlgorithm<ZnBase>,
    {
        ...
        let outer_span = Span::current();
        CondIterator::new(self.rns_parts_mut(&mut el_coeff), is_parallel())
            .enumerate()
            .for_each(|(i, part)| {
                span!(parent: &outer_span, Level::INFO, "small_to_coeff_basis_block")
                    .in_scope(|| self.ring_decompositions[i].small_basis_to_coeff_basis(part))
        ...
        for (i, Zp) in self.base_ring().as_iter().enumerate() {
            for j in 0..self.rank() {
                *result_matrix.at_mut(i, j) = Zp.get_ring().map_out(
                    to.base_ring().at(i).get_ring(),
                    Zp.clone_el(&el_coeff[i * self.rank() + j]),
                    &iso[i],
                );
            }
        }
    ...
    pub fn drop_rns_factor_non_fft_element(
        &self,
        from: &Self,
        dropped_rns_factors: &RNSFactorIndexList,
        value: &SmallBasisEl<NumberRing, A>,
    ) -> SmallBasisEl<NumberRing, A> {
        self.collect_rns_factors_non_fft(drop_rns_factor_list_of_congruences(from, dropped_rns_factors, value))
    }

    pub fn add_rns_factor_non_fft_element(
        &self,
        from: &Self,
    ...
    where
        I: Iterator<Item = RNSFactorCongruence<'a, Self, SmallBasisEl<NumberRing, A>>>,
        Self: 'a,
    {
        ...
        for (i, congruence) in rns_factors.iter().enumerate() {
            match congruence {
                RNSFactorCongruence::CongruentTo(other_ring, other_i, other_el) => {
                    ...
                    result.extend(
                        other_ring
                            .as_matrix_wrt_small_basis(other_el)
                            .row_at(*other_i)
                            .iter()
                            .copied(),
                    );
    ...
    A: FheanorAllocator,
{
    ...
    where
        I: Iterator<Item = RNSFactorCongruence<'a, Self, DoubleRNSEl<NumberRing, A>>>,
        Self: 'a,
    {
        let rns_factors = rns_factors.collect::<Vec<_>>();
        assert_eq!(self.base_ring().len(), rns_factors.len());
        ...
        for (i, congruence) in rns_factors.iter().enumerate() {
            match congruence {
                RNSFactorCongruence::CongruentTo(other_ring, other_i, other_el) => {
                    ...
                    result.extend(
                        other_ring
                            .as_matrix_wrt_mult_basis(other_el)
                            .row_at(*other_i)
                            .iter()
                            .copied(),
                    );
    ...
    where
        I: Iterator<Item = RNSFactorCongruence<'a, Self, Self::PreparedMultiplicant>>,
        Self: 'a,
    {
        ()
    }
    ...
    fn drop_rns_factor(&self, drop_rns_factors: &RNSFactorIndexList) -> Self {
        Self {
            ring_decompositions: self
                .ring_decompositions
                .iter()
                .enumerate()
                .filter(|(i, _)| !drop_rns_factors.contains(*i))
                .map(|(_, x)| x.clone())
                .collect(),
            number_ring: self.number_ring().clone(),
            rns_base: zn_rns::Zn::new(
                self.base_ring()
                    .as_iter()
                    .enumerate()
                    .filter(|(i, _)| !drop_rns_factors.contains(*i))
    ...
    fn small_generating_set_len(&self) -> usize { self.rank() }
    ...
    {
        let result = self.from_representation_wrt_small_generating_set_non_fft(initializer);
    ...
    fn two_by_two_convolution(&self, lhs: [Self::Element; 2], rhs: [Self::Element; 2]) -> [Self::Element; 3] {
        ...
        CondIterator::new(self.rns_parts_mut(&mut lhs0.el_wrt_mult_basis), is_parallel())
            ...
            .for_each(|(i, (((lhs0, lhs1), rhs0), rhs1))| {
                span!(parent: &outer_span, Level::INFO, "conv_block").in_scope(|| {
                    lhs0.iter_mut()
                        .zip(lhs1)
                        .zip(rhs0)
                        .zip(rhs1)
                        .for_each(|(((l0, l1), r0), r1)| {
    ...
    fn two_by_two_convolution_square(&self, lhs: [Self::Element; 2]) -> [Self::Element; 3] {
        ...
        CondIterator::new(self.rns_parts_mut(&mut lhs0.el_wrt_mult_basis), is_parallel())
            ...
            .for_each(|(i, ((lhs0, lhs1), res))| {
                span!(parent: &outer_span, Level::INFO, "conv_square_block").in_scope(|| {
                    lhs0.iter_mut().zip(lhs1).zip(res).for_each(|((l0, l1), r)| {
                        let Zp = &self.rns_base.at(i);
                        let l0_val = *l0;
    ...
    A: FheanorAllocator,
{
    ...
    fn add_assign_ref(&self, lhs: &mut Self::Element, rhs: &Self::Element) {
        ...
        let outer_span = Span::current();
        CondIterator::new(self.rns_parts_mut(&mut lhs.el_wrt_mult_basis), is_parallel())
            .zip(self.rns_parts(&rhs.el_wrt_mult_basis))
            .enumerate()
            .for_each(|(i, (lhs_part, rhs_part))| {
                span!(parent: &outer_span, Level::INFO, "add_block").in_scope(|| {
                    lhs_part
    ...
    fn sub_assign(&self, lhs: &mut Self::Element, rhs: Self::Element) { self.sub_assign_ref(lhs, &rhs) }
    ...
    fn negate_inplace(&self, lhs: &mut Self::Element) {
        ...
        CondIterator::new(self.rns_parts_mut(&mut lhs.el_wrt_mult_basis), is_parallel())
            ...
            .for_each(|(i, lhs_part)| {
                span!(parent: &outer_span, Level::INFO, "negate_block")
                    .in_scope(|| lhs_part.iter_mut().for_each(|l| self.rns_base.at(i).negate_inplace(l)))
            });
    }
    ...
    fn mul_assign_ref(&self, lhs: &mut Self::Element, rhs: &Self::Element) {
        ...
        CondIterator::new(self.rns_parts_mut(&mut lhs.el_wrt_mult_basis), is_parallel())
            ...
            .enumerate()
            .for_each(|(i, (lhs_part, rhs_part))| {
                span!(parent: &outer_span, Level::INFO, "mul_block").in_scope(|| {
                    lhs_part
                        .iter_mut()
                        .zip(rhs_part)
                        .for_each(|(l, r)| self.rns_base.at(i).mul_assign_ref(l, r))
                })
            });
    }
    ...
    fn fma(&self, lhs: &Self::Element, rhs: &Self::Element, mut summand: Self::Element) -> Self::Element {
        ...
        let outer_span = Span::current();
        CondIterator::new(self.rns_parts_mut(&mut summand.el_wrt_mult_basis), is_parallel())
            .zip(self.rns_parts(&lhs.el_wrt_mult_basis))
            .zip(self.rns_parts(&rhs.el_wrt_mult_basis))
            .enumerate()
            .for_each(|(i, ((summand_part, lhs_part), rhs_part))| {
                span!(parent: &outer_span, Level::INFO, "fma_block").in_scope(|| {
                    summand_part
                        .iter_mut()
                        .zip(lhs_part)
                        .zip(rhs_part)
                        .for_each(|((x, l), r)| *x = self.rns_base.at(i).fma(l, r, *x))
                })
            });
        return summand;
    }
    ...
    fn mul_assign_int(&self, lhs: &mut Self::Element, rhs: i32) {
        ...
        CondIterator::new(self.rns_parts_mut(&mut lhs.el_wrt_mult_basis), is_parallel())
            ...
            .for_each(|(i, lhs_part)| {
                ...
                span!(parent: &outer_span, Level::INFO, "mul_int_block").in_scope(|| {
                    lhs_part
                        .iter_mut()
                        .for_each(|l| self.rns_base.at(i).mul_assign_ref(l, &rhs_mod_p))
                })
            });
    }
    ...
    fn zero(&self) -> Self::Element {
        ...
        result.extend(
            self.base_ring()
                .as_iter()
                .flat_map(|Zp| (0..self.rank()).map(|_| Zp.zero())),
        );
        return DoubleRNSEl {
            el_wrt_mult_basis: result,
            number_ring: PhantomData,
    ...
    fn square(&self, value: &mut Self::Element) {
        ...
        let outer_span = Span::current();
        CondIterator::new(self.rns_parts_mut(&mut value.el_wrt_mult_basis), is_parallel())
            .enumerate()
            .for_each(|(i, lhs_part)| {
                span!(parent: &outer_span, Level::INFO, "square_block")
                    .in_scope(|| lhs_part.iter_mut().for_each(|l| self.rns_base.at(i).square(l)))
            });
    }
    ...
    A: FheanorAllocator,
{
    ...
    fn acting_galois_group(&self) -> &Subgroup<CyclotomicGaloisGroup> { &self.galois_group }
    ...
    fn apply_galois_action(&self, el: &Self::Element, g: &GaloisGroupEl) -> Self::Element {
        ...
        let outer_span = Span::current();
        CondIterator::new(self.rns_parts_mut(&mut result.el_wrt_mult_basis), is_parallel())
            .zip(self.rns_parts(&el.el_wrt_mult_basis))
            .enumerate()
            .for_each(|(i, (dst_part, src_part))| {
                span!(parent: &outer_span, Level::INFO, "galois_block")
                    .in_scope(|| self.ring_decompositions[i].permute_galois_action(src_part, dst_part, g))
            });
        return result;
    }
}
    ...
    A: FheanorAllocator,
{
    ...
    fn checked_left_div(&self, lhs: &Self::Element, rhs: &Self::Element) -> Option<Self::Element> {
        ...
        CondIterator::new(self.rns_parts_mut(&mut result), is_parallel())
            ...
            .zip(self.rns_parts(&rhs.el_wrt_mult_basis))
            .enumerate()
            .for_each(|(i, ((dst_part, lhs_part), rhs_part))| {
                span!(parent: &outer_span, Level::INFO, "div_block").in_scope(|| {
                    dst_part.iter_mut().zip(lhs_part).zip(rhs_part).for_each(|((x, l), r)| {
                        if let Some(res) = self.rns_base.at(i).checked_div(l, r) {
                            *x = res;
                        } else {
    ...
    fn is_unit(&self, x: &Self::Element) -> bool {
        x.el_wrt_mult_basis
            .iter()
            .enumerate()
            .all(|(index, c)| self.base_ring().at(index / self.rank()).is_unit(c))
    }
}
...
impl<'a, NumberRing, A> VectorFn<El<zn_rns::Zn<Zn, BigIntRing>>>
    for DoubleRNSRingBaseElVectorRepresentation<'a, NumberRing, A>
where
    NumberRing: NumberRingDescriptor,
    A: FheanorAllocator,
{
    fn len(&self) -> usize { self.ring.rank() }

    fn at(&self, i: usize) -> El<zn_rns::Zn<Zn, BigIntRing>> {
        assert!(i < self.len());
        self.ring.base_ring().from_congruence(
            self.el_wrt_coeff_basis[i..]
                .iter()
                .step_by(self.ring.rank())
                .enumerate()
                .map(|(i, x)| self.ring.base_ring().at(i).clone_el(x)),
        )
    }
}
    ...
    A: FheanorAllocator,
{
    ...
    fn rank(&self) -> usize { self.ring_decompositions[0].rank() }
    ...
    A: FheanorAllocator,
{
    ...
    fn base_ring<'a>(&'a self) -> &'a Self::BaseRing { &self.rns_base }
    ...
    fn fma_base(&self, lhs: &Self::Element, rhs: &El<Self::BaseRing>, mut summand: Self::Element) -> Self::Element {
        ...
        for (i, Zp) in self.base_ring().as_iter().enumerate() {
            for j in 0..self.rank() {
                summand.el_wrt_mult_basis[i * self.rank() + j] = Zp.fma(
                    &lhs.el_wrt_mult_basis[i * self.rank() + j],
                    x_congruence.at(i),
    ...
    A: FheanorAllocator,
{
    ...
    where
        I: IntoIterator<Item = (&'a Self::Element, Option<&'a ()>, &'a Self::Element, Option<&'a ()>)>,
        Self: 'a,
    {
...
where
    ...
    A: FheanorAllocator,
{
    fn clone(&self) -> Self { Self { ring: self.ring } }
}

impl<'a, 'b, NumberRing, A> Fn<(&'b [El<zn_rns::Zn<Zn, BigIntRing>>],)>
    for WRTCanonicalBasisElementCreator<'a, NumberRing, A>
where
    NumberRing: NumberRingDescriptor,
    A: FheanorAllocator,
{
...
impl<'a, 'b, NumberRing, A> FnMut<(&'b [El<zn_rns::Zn<Zn, BigIntRing>>],)>
    for WRTCanonicalBasisElementCreator<'a, NumberRing, A>
where
    NumberRing: NumberRingDescriptor,
    A: FheanorAllocator,
...
impl<'a, 'b, NumberRing, A> FnOnce<(&'b [El<zn_rns::Zn<Zn, BigIntRing>>],)>
    for WRTCanonicalBasisElementCreator<'a, NumberRing, A>
where
    NumberRing: NumberRingDescriptor,
    A: FheanorAllocator,
    ...
    A: FheanorAllocator,
{
        ...
        I::Type: IntegerRing,
    {
        ...
        if ZZ.get_ring().representable_bits().is_none()
            || ZZ.get_ring().representable_bits().unwrap() >= self.rank() * ZZ.abs_log2_ceil(&modulus).unwrap()
        {
        ...
        if serializer.is_human_readable() {
            SerializableNewtypeStruct::new(
                ...
                SerializableSeq::new_with_len(
                    self.ring
                        .wrt_canonical_basis_non_fft(self.ring.clone_el_non_fft(self.el))
                        .iter()
                        .map(|c| SerializeOwnedWithRing::new(c, self.ring.base_ring())),
                    self.ring.rank(),
                ),
            )
            ...
            SerializableNewtypeStruct::new(
                "SmallBasisEl",
                &serialize_rns_data(self.ring.base_ring(), self.ring.as_matrix_wrt_small_basis(self.el)),
            )
            .serialize(serializer)
        }
    }
...
where
    ...
    A: FheanorAllocator,
{
    pub fn new(ring: &'a DoubleRNSRingBase<NumberRing, A>) -> Self { Self { ring } }
}
    ...
    A: FheanorAllocator,
{
        ...
        D: Deserializer<'de>,
    {
        if deserializer.is_human_readable() {
            let data = DeserializeSeedNewtypeStruct::new(
                "RingEl",
                DeserializeSeedSeq::new(
                    (0..(self.ring.rank() + 1)).map(|_| DeserializeWithRing::new(self.ring.base_ring())),
            ...
            if data.len() != self.ring.rank() {
                return Err(serde::de::Error::invalid_length(
                    data.len(),
                    &format!("expected a sequence of {} elements of Z/qZ", self.ring.rank()).as_str(),
                ));
            }
            ...
            DeserializeSeedNewtypeStruct::new(
                "SmallBasisEl",
                deserialize_rns_data(
                    self.ring.base_ring(),
                    self.ring.as_matrix_wrt_small_basis_mut(&mut result),
                ),
            )
            .deserialize(deserializer)?;
            return Ok(result);
    ...
    A: FheanorAllocator,
{
        ...
        S: serde::Serializer,
    {
        if serializer.is_human_readable() {
            SerializableNewtypeStruct::new(
                "RingEl",
                &SerializableSmallBasisElWithRing::new(self, &self.undo_fft(self.clone_el(el))),
            )
            .serialize(serializer)
        } else {
            SerializableNewtypeStruct::new(
                "DoubleRNSEl",
                &serialize_rns_data(self.base_ring(), self.as_matrix_wrt_mult_basis(el)),
            )
            .serialize(serializer)
        }
    }
    ...
    {
        if deserializer.is_human_readable() {
            DeserializeSeedNewtypeStruct::new("RingEl", DeserializeSeedSmallBasisElWithRing::new(self))
                .deserialize(deserializer)
                .map(|x| self.do_fft(x))
        } else {
            let mut result = self.zero();
            DeserializeSeedNewtypeStruct::new(
                "DoubleRNSEl",
                deserialize_rns_data(self.base_ring(), self.as_matrix_wrt_mult_basis_mut(&mut result)),
            )
            .deserialize(deserializer)?;
            return Ok(result);
        }
    ...
    A2: FheanorAllocator,
{
    ...
    fn has_canonical_hom(&self, from: &DoubleRNSRingBase<NumberRing, A2>) -> Option<Self::Homomorphism> {
        if self.base_ring().len() == from.base_ring().len() && self.number_ring() == from.number_ring() {
            (0..self.base_ring().len())
                .map(|i| {
                    self.base_ring()
                        .at(i)
                        .get_ring()
                        .has_canonical_hom(from.base_ring().at(i).get_ring())
                        .ok_or(())
                })
    ...
    fn map_in(
        ...
        el: <DoubleRNSRingBase<NumberRing, A2> as RingBase>::Element,
    ...
    fn map_in_ref(
        ...
        for (i, Zp) in self.base_ring().as_iter().enumerate() {
            for j in 0..self.rank() {
                result.push(Zp.get_ring().map_in_ref(
                    from.base_ring().at(i).get_ring(),
                    &el.el_wrt_mult_basis[i * self.rank() + j],
                    &hom[i],
                ));
            }
        }
    ...
    C2: ConvolutionAlgorithm<ZnBase>,
{
    ...
    fn map_in(
        &self,
        from: &SingleRNSRingBase<NumberRing, A2, C2>,
        el: <SingleRNSRingBase<NumberRing, A2, C2> as RingBase>::Element,
        hom: &Self::Homomorphism,
    ) -> Self::Element {
    ...
    C2: ConvolutionAlgorithm<ZnBase>,
{
            ...
            (0..self.base_ring().len())
                .map(|i| {
                    self.base_ring()
                        .at(i)
                        .get_ring()
                        .has_canonical_iso(from.base_ring().at(i).get_ring())
                        .ok_or(())
                })
                .collect::<Result<Vec<_>, ()>>()
                .ok()
    ...
    ) -> <SingleRNSRingBase<NumberRing, A2, C2> as RingBase>::Element {
    ...
    A2: FheanorAllocator,
{
    ...
    fn map_out(
        ...
        el: Self::Element,
        iso: &Self::Isomorphism,
    ) -> <DoubleRNSRingBase<NumberRing, A2> as RingBase>::Element {
        let mut result = Vec::with_capacity_in(from.element_len(), from.allocator.clone());
        for (i, Zp) in self.base_ring().as_iter().enumerate() {
            for j in 0..self.rank() {
                result.push(Zp.get_ring().map_out(
                    from.base_ring().at(i).get_ring(),
                    Zp.clone_el(&el.el_wrt_mult_basis[i * self.rank() + j]),
                    &iso[i],
                ));
            }
        }
...
pub fn test_with_number_ring<NumberRing: Clone + NumberRingDescriptor>(number_ring: NumberRing) {
    ...
    let required_root_of_unity = signed_lcm(
        ...
        1 << StaticRing::<i64>::RING
            .abs_log2_ceil(&(number_ring.galois_group().m() as i64))
            .unwrap()
            + 2,
        StaticRing::<i64>::RING,
    );
    ...
    let elements = vec![
        ...
        ring.int_hom()
            .mul_map(ring.pow(ring.canonical_gen(), rank - 1), p1 as i32),
    ...
    feanor_math::ring::generic_tests::test_hom_axioms(
        &ring,
        &single_rns_ring,
        elements.iter().map(|x| ring.clone_el(x)),
    );

    let dropped_rns_factor_ring = DoubleRNSRingBase::new(
        number_ring.clone(),
        zn_rns::Zn::new(vec![Zn::new(p2 as u64)], BigIntRing::RING),
    );
    for a in &elements {
        assert_el_eq!(
            &dropped_rns_factor_ring,
            dropped_rns_factor_ring.from_canonical_basis(ring.wrt_canonical_basis(a).iter().map(|c| {
                dropped_rns_factor_ring
                    .base_ring()
                    .from_congruence([*ring.base_ring().get_congruence(&c).at(1)].into_iter())
            })),
            dropped_rns_factor_ring.get_ring().drop_rns_factor_element(
                ring.get_ring(),
                &RNSFactorIndexList::from([0], ring.base_ring().len()),
                a
            )
        );
        assert_el_eq!(
            ...
            dropped_rns_factor_ring.get_ring().do_fft(
                dropped_rns_factor_ring.get_ring().drop_rns_factor_non_fft_element(
                    ring.get_ring(),
                    &RNSFactorIndexList::from([0], ring.base_ring().len()),
                    &ring.get_ring().undo_fft(ring.clone_el(a))
                )
            )
        );
    }
    for a in &elements {
        let dropped_factor_a = dropped_rns_factor_ring.get_ring().drop_rns_factor_element(
            ring.get_ring(),
            &RNSFactorIndexList::from([0], ring.base_ring().len()),
            a,
        );
        assert_el_eq!(
            &ring,
            ring.from_canonical_basis(ring.wrt_canonical_basis(a).iter().map(|c| {
                ring.base_ring().from_congruence(
                    [
                        ring.base_ring().at(0).zero(),
                        *ring.base_ring().get_congruence(&c).at(1),
                    ]
                    .into_iter(),
                )
            })),
            ring.get_ring().add_rns_factor_element(
                dropped_rns_factor_ring.get_ring(),
                &RNSFactorIndexList::from([0], ring.base_ring().len()),
                &dropped_factor_a
            )
        );
        assert_el_eq!(
            ...
            ring.get_ring().do_fft(ring.get_ring().add_rns_factor_non_fft_element(
                dropped_rns_factor_ring.get_ring(),
                &RNSFactorIndexList::from([0], ring.base_ring().len()),
                &dropped_rns_factor_ring.get_ring().undo_fft(dropped_factor_a)
            ))
        );
    }
    ...
    let dropped_rns_factor_ring = DoubleRNSRingBase::new(
        number_ring.clone(),
        zn_rns::Zn::new(vec![Zn::new(p1 as u64)], BigIntRing::RING),
    );
    for a in &elements {
        assert_el_eq!(
            &dropped_rns_factor_ring,
            dropped_rns_factor_ring.from_canonical_basis(ring.wrt_canonical_basis(a).iter().map(|c| {
                dropped_rns_factor_ring
                    .base_ring()
                    .from_congruence([*ring.base_ring().get_congruence(&c).at(0)].into_iter())
            })),
            dropped_rns_factor_ring.get_ring().drop_rns_factor_element(
                ring.get_ring(),
                &RNSFactorIndexList::from([1], ring.base_ring().len()),
                a
            )
        );
        assert_el_eq!(
    ...
    for a in &elements {
        ...
        assert_el_eq!(
            ...
            ring.from_canonical_basis(ring.wrt_canonical_basis(a).iter().map(|c| {
                ring.base_ring().from_congruence(
                    [
                        *ring.base_ring().get_congruence(&c).at(0),
                        ring.base_ring().at(1).zero(),
                    ]
                    .into_iter(),
                )
            })),
