...
fn test_empty_rns_base_conversion() {
    ...
    table.apply_base(Submatrix::from_1d(&[], from.len(), 1), SubmatrixMut::from_1d(&mut actual, to.len(), 1));
    for j in 0..to.len() {
    ...
    table.apply_base(Submatrix::from_1d(&input, from.len(), 1), SubmatrixMut::from_1d(&mut [], to.len(), 1));
}
...
fn test_rns_base_conversion() {
    ...
    for k in (-q/2)..=(q/2) {
        ...
        table.apply_base(Submatrix::from_1d(&input, from.len(), 1), SubmatrixMut::from_1d(&mut actual, to.len(), 1));
