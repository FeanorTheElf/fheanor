    ...
    R::Type: SerializableElementRing,
{
        ...
        S: serde::Serializer,
    {
        ...
        fn serialize_coefficient<'a, R>(
            c: &'a Coefficient<R::Type>,
            ring: R,
            constants: &'a Constants,
        ) -> SerializableCoefficient<'a, R>
        where
            R: RingStore + Copy,
            R::Type: SerializableElementRing,
        {
            match c {
                ...
                Coefficient::NegOne => {
                    SerializableCoefficient::Integer(SerializeWithRing::new(&constants.neg_one, ZZbig))
                }
        ...
        fn serialize_lin_transform<'a, R: Copy + RingStore>(
            t: &'a LinearCombination<R::Type>,
            ring: R,
            constants: &'a Constants,
        ) -> SerializableLinearCombination<SerializableCoefficient<'a, R>, impl use<'a, R> + Serialize>
        where
            R::Type: SerializableElementRing,
            R: 'a,
        {
            SerializableLinearCombination {
                ...
                factors: SerializableSeq::new_with_len(
                    t.factors.iter().map(move |c| serialize_coefficient(c, ring, constants)),
                    t.factors.len(),
                ),
            }
        }
        SerializablePlaintextCircuitData {
            ...
            gates: SerializableSeq::new_with_len(
                self.circuit.gates.iter().map(|gate| match gate {
                    PlaintextCircuitGate::Mul(lhs, rhs) => {
                        SerializablePlaintextCircuitGate::Mul(SerializablePlaintextCircuitMulGate {
                            lhs: serialize_lin_transform(lhs, self.ring, &constants),
                            rhs: serialize_lin_transform(rhs, self.ring, &constants),
                        })
                    }
                    PlaintextCircuitGate::Gal(gs, val) => {
                        SerializablePlaintextCircuitGate::Gal(SerializablePlaintextCircuitGalGate {
                            automorphisms: SerializableSeq::new_with_len(
                                gs.iter()
                                    .map(|g| SerializeWithGroup::new(g, self.galois_group.unwrap().parent())),
                                gs.len(),
                            ),
                            input: serialize_lin_transform(val, self.ring, &constants),
                        })
                    }
                    PlaintextCircuitGate::Square(val) => {
                        SerializablePlaintextCircuitGate::Square(SerializablePlaintextCircuitSquareGate {
            ...
            output_transforms: SerializableSeq::new_with_len(
                self.circuit
                    .output_transforms
                    .iter()
                    .map(|t| serialize_lin_transform(t, self.ring, &constants)),
                self.circuit.output_transforms.len(),
            ),
        }
    ...
    R::Type: SerializableElementRing,
{
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        fn derive_gal_gate_deserializer<'de, 'a, R>(
            d: &DeserializeSeedPlaintextCircuitGalGate<'a, R>,
        ) -> impl use<'a, 'de, R> + DeserializeSeed<'de, Value = Vec<GaloisGroupEl>>
        where
            R: RingStore + Copy,
            R::Type: SerializableElementRing,
        {
            let galois_group: &'a Subgroup<CyclotomicGaloisGroup> = d
                .galois_group
                .expect("cannot deserialize a circuit with galois gates if no galois group was specified");
            DeserializeSeedSeq::new(
                std::iter::repeat(DeserializeWithGroup::new(galois_group.parent())),
                Vec::new(),
                |mut current, next| {
                    assert!(galois_group.contains(&next));
                    current.push(next);
                    current
                },
            )
        ...
        let result = PlaintextCircuit {
            gates: res
                ...
                .map(|gate| match gate {
                    GateData::Gal((gate, _)) => {
                        PlaintextCircuitGate::Gal(gate.automorphisms, convert_transform(gate.input))
                    }
                    GateData::Mul((gate, _)) => {
                        PlaintextCircuitGate::Mul(convert_transform(gate.lhs), convert_transform(gate.rhs))
                    }
