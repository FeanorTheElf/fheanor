...
use feanor_math::integer::{IntegerRingStore, int_cast};
use feanor_math::ring::*;
use feanor_math::rings::zn::ZnRingStore;
use tracing::{Level, event};

use super::*;
use crate::bfv::eval::{AsBFVPlaintext, EncodedBFVPlaintextRing, EncodedBFVPlaintextRingBase};
...
/// let bootstrapper =
///     ThinBootstrapper::build_pow2(&params, &P, &C, 2, None, 4, &digits, Some("./cache"));
///
/// // creating keys
...
/// let res_ct = bootstrapper.bootstrap_thin(&C, &C_mul, ct, &rk, &gk, Some(&encaps), Some(&sk));
...
impl<Inst: BFVInstantiation> ThinBootstrapper<Inst> {
    ...
    ///  - `slots_to_coeffs_ciphertext_ring` is a intermediate, reduced-modulus ciphertext ring used
    ///    for the Slots-to-Coeffs transform. More concretely, since the result of the
    ///    Slots-to-Coeffs transform does not have to have any noise budget left, the input
    ///    ciphertext can be mod-switched to a lower modulus ciphertext ring before the
    ///    Slots-to-Coeffs transform, which will improve performance of the Slots-to-Coeffs
    ///    transform. Note that the current implementation of BFV does not use hybrid key switching
    ///    (although it pretends to in some cases), and this should be considered when calculating
    ///    how large the modulus of this ring should be.
    ///
    ...
    pub fn create(
        ...
        let plaintext_ring_hierarchy = ((r + 1)..e)
            ...
            .collect();
        let slots_to_coeffs_plaintext_ring =
            EncodedBFVPlaintextRingBase::new(original_plaintext_ring, slots_to_coeffs_ciphertext_ring);
        let intermediate_plaintext_ring = EncodedBFVPlaintextRingBase::new(intermediate_plaintext_ring, C);
        let coeffs_to_slots_thin: PlaintextCircuit<EncodedBFVPlaintextRingBase<Inst>> = coeffs_to_slots_thin
            .change_ring_uniform(|x| {
                x.change_ring(|x| WrapHom::to_delegate_ring(intermediate_plaintext_ring.get_ring()).map(x))
            });
        let slots_to_coeffs_thin: PlaintextCircuit<EncodedBFVPlaintextRingBase<Inst>> = slots_to_coeffs_thin
            .change_ring_uniform(|x| {
        ...
        Inst::CiphertextRing: Clone,
    {
        ...
        let slots_to_coeffs_ciphertext_ring = {
            let (drop_additional, special_modulus) = compute_optimal_special_modulus(
                C.get_ring(),
                RNSFactorIndexList::empty_ref(),
                C.base_ring().len().saturating_sub(slots_to_coeffs_rns_factors),
                gk_digits,
            );
            RingValue::from(
                C.get_ring()
                    .drop_rns_factor(&drop_additional.subtract(&special_modulus)),
            )
        };
        ...
        Inst::CiphertextRing: Clone,
    {
        ...
        // see the corresponding comment in `build_pow2()`
        let min_rns_factor_log2 = C
            .base_ring()
            .as_iter()
            .map(|rns_factor| *rns_factor.modulus() as i64)
    ...
    pub fn p(&self) -> El<BigIntRing> { ZZbig.clone_el(self.digit_extract.p()) }
    ...
    fn perform_slots_to_coefficients(
        ...
        let modswitched_gks = self
            .slots_to_coeffs_thin
            .required_galois_keys(&galois_group)
            .iter()
            .map(|g| {
                if let Some((_, gk)) = gks
                    .iter()
                    .filter(|(provided_g, _)| galois_group.eq_el(g, provided_g))
                    .next()
                {
                    (
                        g.clone(),
                        (
                            gk.0.clone(C.get_ring()).modulus_switch(
                                C_input.get_ring(),
                                &C_to_C_input_drop_factors,
                                C.get_ring(),
                            ),
                            gk.1.clone(C.get_ring()).modulus_switch(
                                C_input.get_ring(),
                                &C_to_C_input_drop_factors,
                                C.get_ring(),
                            ),
                    ...
                    panic!(
                        "missing galois key for {}",
                        galois_group.underlying_ring().format(galois_group.as_ring_el(g))
                    )
                }
            })
            .collect::<Vec<_>>();
        ...
        let sk_input = debug_sk.map(|sk| {
            C_input
                .get_ring()
                .drop_rns_factor_element(C.get_ring(), &C_to_C_input_drop_factors, &sk)
        });
    ...
    fn perform_noisy_expansion(
        ...
        let result = if let Some(sk_encaps_data) = sk_encaps_data {
            let ct_with_sparse_key = {
                ...
                Inst::key_switch(
                    &sk_encaps_data.C_sparse_sk,
                    ct_modswitched,
                    &sk_encaps_data.switch_to_sparse_key,
                )
            };
            if let Some(sk) = &debug_sk {
                Inst::dec_println(
                    ...
                    &sk_encaps_data.C_sparse_sk,
                    &ct_with_sparse_key,
                    &Inst::mod_switch_sk(P, &sk_encaps_data.C_sparse_sk, C, sk),
                );
            }
            ...
            Inst::hom_add_plain(
                ...
                C,
                &c0,
                Inst::hom_mul_plain(P_main, C, &c1, Inst::clone_ct(C, &sk_encaps_data.encapsulated_key)),
            )
        } else {
    ...
    fn perform_digit_extraction(
        ...
        self.digit_extract
            .evaluate_bfv::<_, Inst>(&plaintext_rings, &plaintext_rings, C, C_mul, ct, rk, gks, debug_sk)
            .0
    }
    ...
    ///  - `sparse_key_encapsulation` optionally contains all data required to temporarily switch to
    ///    a sparse secret key before bootstrapping. If used, this can make bootstrapping work with
    ///    significantly smaller parameters.
    ...
    pub fn bootstrap_thin(
        ...
        return self.perform_digit_extraction(C_mul, noisy_decryption_in_slots, rk, gks, debug_sk);
    }
}
...
fn test_digit_extract_homomorphic() {
    ...
    let rk = Pow2BFV::gen_rk(
        ...
        &mut rng,
        &sk,
        &RNSGadgetVectorDigitIndices::select_digits(7, C.base_ring().len()),
        3.2,
    );
    ...
    let (ct_high, ct_low) = digitextract.evaluate_bfv::<_, Pow2BFV>(
...
fn test_pow2_bfv_thin_bootstrapping_17() {
    ...
    let rk = Pow2BFV::gen_rk(&C, &mut rng, &sk, &key_switch_params, 3.2);

    let m = P.int_hom().map(2);
    let ct = Pow2BFV::enc_sym(&P, &C, &mut rng, &m, &sk, 3.2);
    let res_ct = bootstrapper.bootstrap_thin(&C, &C_mul, ct, &rk, &gk, None, Some(&sk));
...
fn test_pow2_bfv_thin_bootstrapping_23() {
    ...
    let m = P.int_hom().map(2);
    let ct = Pow2BFV::enc_sym(&P, &C, &mut rng, &m, &sk, 3.2);
    let res_ct = bootstrapper.bootstrap_thin(&C, &C_mul, ct, &rk, &gk, None, None);
...
fn test_composite_bfv_thin_bootstrapping_2() {
    ...
    let rk = CompositeBFV::gen_rk(&C, &mut rng, &sk, &key_switch_params, 3.2);
...
fn measure_time_double_rns_composite_bfv_thin_bootstrapping() {
    ...
    let rk = CompositeBFV::gen_rk(&C, &mut rng, &sk, &rk_params, 3.2);
    let encaps = SparseKeyEncapsulationKey::new(
        bootstrapper.intermediate_plaintext_ring(),
        &C,
        &sk,
    ...
    let m = P.int_hom().map(2);
    let ct = CompositeBFV::enc_sym(&P, &C, &mut rng, &m, &sk, 3.2);
    let res_ct = bootstrapper.bootstrap_thin(&C, &C_mul, ct, &rk, &gk, Some(&encaps), None);

    println!(
        "final noise budget: {}",
        CompositeBFV::noise_budget(&P, &C, &res_ct, &sk)
    );
    assert_el_eq!(P, P.int_hom().map(2), CompositeBFV::dec(&P, &C, res_ct, &sk));
}
...
fn measure_time_double_rns_pow2_bfv_thin_bootstrapping_t257_sqr() {
    ...
    let rk = Pow2BFV::gen_rk(&C, &mut rng, &sk, &rk_params, 3.2);
    let encaps = SparseKeyEncapsulationKey::new(
        bootstrapper.intermediate_plaintext_ring(),
        &C,
        &sk,
...
fn measure_time_single_rns_composite_bfv_thin_bootstrapping() {
    ...
    let gk = CompositeSingleRNSBFV::gen_gks(
        &C,
        &mut rng,
        &sk,
        bootstrapper.required_galois_keys(&P),
    ...
    let rk = CompositeSingleRNSBFV::gen_rk(&C, &mut rng, &sk, &rk_params, 3.2);
    let encaps = SparseKeyEncapsulationKey::new(
        bootstrapper.intermediate_plaintext_ring(),
        &C,
        &sk,
    ...
    println!(
        "final noise budget: {}",
        CompositeSingleRNSBFV::noise_budget(&P, &C, &res_ct, &sk)
    );
