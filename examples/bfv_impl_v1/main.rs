...
type CiphertextRing = NumberRingQuotientByInt<NumberRing, zn_big::Zn<BigIntRing>>;
...
fn hom_mul_three_component(
    ...
    let lift_ciphertext_ring_el = |x: &El<CiphertextRing>| {
        multiplication_ring.from_canonical_basis(
            ciphertext_ring
                .wrt_canonical_basis(x)
                .iter()
                .map(|c| ciphertext_ring.base_ring().smallest_lift(c)),
        )
    };
    ...
    let product = (
        ...
        multiplication_ring.add(
            multiplication_ring.mul_ref(&lhs_lifted.0, &rhs_lifted.1),
            multiplication_ring.mul_ref(&lhs_lifted.1, &rhs_lifted.0),
        ),
    ...
    let scale_down_multiplication_ring_el = |x: &El<FreeAlgebraImpl<_, [_; 1]>>| {
        ciphertext_ring.from_canonical_basis(multiplication_ring.wrt_canonical_basis(x).iter().map(|c| {
            modulo_q.map(BigIntRing::RING.rounded_div(
...
fn gadget_vector(B: &El<BigIntRing>, digits: usize) -> Vec<El<BigIntRing>> {
    (0..digits)
        .map(|i| BigIntRing::RING.pow(BigIntRing::RING.clone_el(B), i))
        .collect()
}
        ...
        .map(|factor| {
            ...
            return (
                ciphertext_ring.add(
                    ...
                    ciphertext_ring
                        .inclusion()
                        .mul_ref_map(&sk_sqr, &modulo_q.map_ref(factor)),
                ),
                a,
            );
        })
...
fn relinearize(
    ...
    let mut c2_decomposition = (0..digits)
        .map(|_| {
            (0..ciphertext_ring.rank())
                .map(|_| ciphertext_ring.base_ring().zero())
                .collect::<Vec<_>>()
        })
        .collect::<Vec<_>>();
    ...
    for i in 0..c2_wrt_basis.len() {
        let mut coeff_decomposition =
            gadget_decompose(ciphertext_ring.base_ring().smallest_lift(c2_wrt_basis.at(i)), B, digits).into_iter();
        for j in 0..digits {
            c2_decomposition[j][i] = modulo_q.map(coeff_decomposition.next().unwrap());
        }
    }
    let c2_decomposition = c2_decomposition
        .into_iter()
        .map(|coefficients| ciphertext_ring.from_canonical_basis(coefficients))
        .collect::<Vec<_>>();

    return (
        ciphertext_ring.add_ref_fst(
            &three_component_ciphertext.0,
            ciphertext_ring.sum(
                c2_decomposition
                    .iter()
                    .zip(relin_key.iter())
                    .map(|(c, (rk0, _))| ciphertext_ring.mul_ref(c, rk0)),
            ),
        ),
        ciphertext_ring.add_ref_fst(
            &three_component_ciphertext.1,
            ciphertext_ring.sum(
                c2_decomposition
                    .iter()
                    .zip(relin_key.iter())
                    .map(|(c, (_, rk1))| ciphertext_ring.mul_ref(c, rk1)),
            ),
        ),
    );
}
...
fn main() {
    ...
    let end = Instant::now();
    println!(
        "bfv_impl_v1: Multiplication done in dimension N = {} within {} ms",
        C.rank(),
        (end - start).as_millis()
    );
