...
fn main() {
    ...
    let (C, C_for_multiplication): (CiphertextRing<Pow2BFV>, CiphertextRing<Pow2BFV>) =
        params.create_ciphertext_rings(105..110);
    ...
    println!(
        "log2(q') = {}",
        BigIntRing::RING
            .abs_log2_ceil(C_for_multiplication.base_ring().modulus())
            .unwrap()
    );

    let P: PlaintextRing<Pow2BFV> =
        params.create_plaintext_ring(int_cast(17, BigIntRing::RING, StaticRing::<i64>::RING));
    ...
    let x = P.from_canonical_basis((0..(1 << 12)).map(|i| P.base_ring().int_hom().map(i)));
