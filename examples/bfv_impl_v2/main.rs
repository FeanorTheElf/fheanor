...
type RelinKey = (
    RNSGadgetProductRhsOperand<<CiphertextRing as RingStore>::Type>,
    RNSGadgetProductRhsOperand<<CiphertextRing as RingStore>::Type>,
);
...
fn create_plaintext_ring(ring_degree: usize, t: u64) -> PlaintextRing {
    return <PlaintextRing as RingStore>::Type::new(
        Pow2CyclotomicNumberRing::new((ring_degree * 2) as u64),
        zn_64::Zn::new(t),
    );
}
...
fn key_gen(ciphertext_ring: &CiphertextRing) -> El<CiphertextRing> {
    ...
    let sk = ciphertext_ring.from_canonical_basis((0..ciphertext_ring.rank()).map(|_| {
        ciphertext_ring
            .base_ring()
            .int_hom()
            .map((rng.next_u32() as i32 % 3) - 1)
    }));
    return sk;
}
...
fn rlwe_sample(ciphertext_ring: &CiphertextRing, sk: &El<CiphertextRing>) -> (El<CiphertextRing>, El<CiphertextRing>) {
    ...
    let e = ciphertext_ring.from_canonical_basis((0..ciphertext_ring.rank()).map(|_| {
        ciphertext_ring
            .base_ring()
            .int_hom()
            .map((rng.sample::<f64, _>(StandardNormal) * 3.2).round() as i32)
    }));
    return (
        ciphertext_ring.add(e, ciphertext_ring.mul_ref(&a, sk)),
        ciphertext_ring.negate(a),
    );
}
...
fn enc_sym(
    ...
    let t = int_cast(
        *plaintext_ring.base_ring().modulus(),
        BigIntRing::RING,
        StaticRing::<i64>::RING,
    );
    let Δ = ciphertext_ring
        .base_ring()
        .can_hom(&BigIntRing::RING)
        .unwrap()
        .map(BigIntRing::RING.rounded_div(BigIntRing::RING.clone_el(q), &t));
    let x_mod_q = ciphertext_ring.from_canonical_basis(plaintext_ring.wrt_canonical_basis(x).iter().map(|c| {
        ciphertext_ring
            .base_ring()
            .int_hom()
            .map(plaintext_ring.base_ring().smallest_lift(c) as i32)
    }));
    return (
        ciphertext_ring.add(ciphertext_ring.inclusion().mul_map(x_mod_q, Δ), b),
        a,
    );
}
...
fn dec(
    ...
    return plaintext_ring.from_canonical_basis(
        ciphertext_ring
            .wrt_canonical_basis(&decryption_with_noise)
            .iter()
            .map(|c| {
                modulo_t.map(BigIntRing::RING.rounded_div(
...
fn hom_add(
    ...
    return (
        ciphertext_ring.add_ref(&lhs.0, &rhs.0),
        ciphertext_ring.add_ref(&lhs.1, &rhs.1),
    );
}
...
fn create_multiplication_ring(ciphertext_ring: &CiphertextRing) -> CiphertextRing {
    ...
    let min_bits = BigIntRing::RING
        .abs_log2_ceil(ciphertext_ring.base_ring().modulus())
        .unwrap()
        * 2
        + StaticRing::<i64>::RING
            .abs_log2_ceil(&(number_ring.rank() as i64))
            .unwrap()
        + 10;
    let max_bits = min_bits + 57;
    DoubleRNSRingBase::new_with_modulus_size(number_ring, Some(ciphertext_ring.base_ring()), min_bits..max_bits)
}
...
fn hom_mul_three_component(
    ...
    lhs: &(
        SmallBasisEl<Pow2CyclotomicNumberRing>,
        SmallBasisEl<Pow2CyclotomicNumberRing>,
    ),
    rhs: &(
        SmallBasisEl<Pow2CyclotomicNumberRing>,
    ...
    debug_assert!(
        lift_to_multiplication_ring_rnsconv
            .input_rings()
            .iter()
            .zip(ciphertext_ring.base_ring().as_iter())
            .all(|(lhs, rhs)| lhs.get_ring() == rhs.get_ring())
    );
    debug_assert!(
        lift_to_multiplication_ring_rnsconv
            .output_rings()
            .iter()
            .zip(multiplication_ring.base_ring().as_iter())
            .all(|(lhs, rhs)| lhs.get_ring() == rhs.get_ring())
    );
    let lift_to_multiplication_ring = |x: &SmallBasisEl<_, _>| {
        let mut result = multiplication_ring.get_ring().zero_non_fft();
        lift_to_multiplication_ring_rnsconv.apply(
            ciphertext_ring.get_ring().as_matrix_wrt_small_basis(&x),
            multiplication_ring
                .get_ring()
                .as_matrix_wrt_small_basis_mut(&mut result),
        );
        ...
        ciphertext_ring
            ...
            .map(|Zp| {
                multiplication_ring
                    .base_ring()
                    .as_iter()
                    .position(|Zp2| Zp2.modulus() == Zp.modulus())
                    .unwrap()
            })
            .collect::<Vec<_>>(),
    );
    ...
    let scale_down = |x: El<CiphertextRing>| {
        ...
        scale_down_rnsconv.apply(
            multiplication_ring
                .get_ring()
                .as_matrix_wrt_small_basis(&multiplication_ring.get_ring().undo_fft(x)),
            ciphertext_ring.get_ring().as_matrix_wrt_small_basis_mut(&mut result),
        );
        return result;
    };
...
fn gen_relin_key(ciphertext_ring: &CiphertextRing, sk: &El<CiphertextRing>, digits: usize) -> RelinKey {
    let sk_sqr = ciphertext_ring.pow(ciphertext_ring.clone_el(sk), 2);
    let mut result0 = RNSGadgetProductRhsOperand::new(ciphertext_ring.get_ring(), digits);
    let mut result1 = RNSGadgetProductRhsOperand::new(ciphertext_ring.get_ring(), digits);
    ...
    for i in 0..gadget_vector_len {
        ...
        let (key0, key1) = (
            ciphertext_ring.add(b, ciphertext_ring.inclusion().mul_ref_map(&sk_sqr, &factor)),
            a,
        );
...
fn relinearize(
    ...
    let c2_decomposition = RNSGadgetProductLhsOperand::from_double_rns_ring_with(
        ciphertext_ring.get_ring(),
        &three_component_ciphertext.2,
        relin_key.0.gadget_vector_digits(),
    );
    let mut result0 = three_component_ciphertext.0;
    ciphertext_ring.get_ring().add_assign_non_fft(
        &mut result0,
        &ciphertext_ring
            .get_ring()
            .undo_fft(c2_decomposition.gadget_product(&relin_key.0, ciphertext_ring.get_ring())),
    );
    ...
    ciphertext_ring.get_ring().add_assign_non_fft(
        ...
        &ciphertext_ring
            .get_ring()
            .undo_fft(c2_decomposition.gadget_product(&relin_key.1, ciphertext_ring.get_ring())),
    );
    return (result0, result1);
}
...
fn main() {
    ...
    let end = Instant::now();
    println!(
        "bfv_impl_v2: Multiplication done in dimension N = {} within {} ms",
        C.rank(),
        (end - start).as_millis()
    );

    // finally, we have to explicitly change representation back
    let ciphertext_sqr_relin = (
        C.get_ring().do_fft(ciphertext_sqr_relin.0),
        C.get_ring().do_fft(ciphertext_sqr_relin.1),
    );
