...
fn main() {
    ...
    assert!(BigIntRing::RING.is_one(&signed_gcd(
        BigIntRing::RING.clone_el(C_initial.base_ring().modulus()),
        plaintext_modulus,
        BigIntRing::RING
    )));
    ...
    println!(
        "log2(q)  = {}",
        BigIntRing::RING.abs_log2_ceil(C_initial.base_ring().modulus()).unwrap()
    );
    ...
    let rk = Pow2BGV::gen_rk(
        ...
        &mut rng,
        &sk,
        &RNSGadgetVectorDigitIndices::select_digits(2, C_initial.base_ring().len()),
        3.2,
    );

    let x = P.from_canonical_basis((0..(1 << 13)).map(|i| P.base_ring().int_hom().map(i)));
    ...
    println!(
        "log2(q') = {}",
        BigIntRing::RING.abs_log2_ceil(C_new.base_ring().modulus()).unwrap()
    );
