...
fn main() {
    ...
    let (C, C_for_multiplication): (CiphertextRing<Pow2CLPX>, CiphertextRing<Pow2CLPX>) =
        params.create_ciphertext_rings(105..110, 10);
    let N = C.rank();
    println!("N        = {}", N);
    println!("m        = {}", C.acting_galois_group().m());
    println!(
        "log2(q)  = {}",
        BigIntRing::RING.abs_log2_ceil(C.base_ring().modulus()).unwrap()
    );

    let ZZX = DensePolyRing::new(BigIntRing::RING, "X");
    let [t] = ZZX.with_wrapped_indeterminate(|X| [X.pow_ref(16) + 2]);
    let p = BigIntRing::RING
        .get_ring()
        .parse("93461639715357977769163558199606896584051237541638188580280321", 10)
        .unwrap();
    ...
    let rk = Pow2CLPX::gen_rk(
        ...
        &mut rng,
        &sk,
        &RNSGadgetVectorDigitIndices::select_digits(2, C.base_ring().len()),
        3.2,
    );

    let m = P.inclusion().map(
        P.base_ring()
            .coerce(&BigIntRing::RING, BigIntRing::RING.power_of_two(100)),
    );
    ...
    assert_el_eq!(
        BigIntRing::RING,
        BigIntRing::RING.power_of_two(200),
        P.base_ring().smallest_positive_lift(res_constant_coeff)
    );
    println!("done");
}
