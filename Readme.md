...
In short, this crate contains the following:
...
In addition to the API documentation, detailed guides and examples to some parts of this crate can be found in [`crate::examples`].
...
As indicated by the `0.x.y` SemVer version, this crate is still in an alpha phase, and every new release may contain breaking changes.
In fact, most of the version I published recently contained many significant breaking changes.
Hence, it is recommended to fix the used version of this crate, using `version = "=0.x.y"` in your `Cargo.toml`, and only upgrade when you are willing to adjust to the new API changes.
...
This crate is instrumented using the framework defined by the Rust library [`tracing`](https://crates.io/crates/tracing).
Hence, running any functions with an active tracing subscriber will generate corresponding tracing events that the subscriber can use for profiling purposes.
...
This crate is licensed under the [MIT license](https://choosealicense.com/licenses/mit/).